...
class BrowserAudit:
    ...
    def run(self):
        ...
        status, body, _, _ = self.get("/demo/cdn/-/releases?status=candidate")
        self.check(status == 200 and b"No releases match these filters" in body, "status filter excludes non-matching releases")
        status, body, _, _ = self.get("/demo/cdn/-/releases?major=1&status=stable")
        self.check(status == 200 and b'id="release-1.0.0"' in body, "train and status filters keep matching releases")
        ...
        self.chrome.call("Emulation.setDeviceMetricsOverride", {"width": 1440, "height": 1000, "deviceScaleFactor": 1, "mobile": False})
        package_url = "/demo/cdn/-/docs/config-demo/1.0.0/x86_64-linux?release=1.0.0"
        self.navigate(package_url)
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length") == 1, "package guide opens with exactly one persistent card")
        package_entry = self.chrome.evaluate("document.querySelector('[data-doc-browser]').dataset.docPackage")
        self.check(bool(package_entry), "package guide pins its immutable package scope")
        self.chrome.evaluate("window.__packageNavigation = true; document.querySelector('.doc-view-toggle a:last-child').click()")
        self.wait("location.search.includes('view=all') && !document.querySelector('[aria-busy=\"true\"]')", "package Options toggle")
        self.check(self.chrome.evaluate("window.__packageNavigation && document.querySelectorAll('[data-doc-package-card]').length === 1"), "Options keeps the package card during in-place navigation")
        self.check(self.chrome.evaluate("Array.from(document.querySelectorAll('[data-doc-folder] a')).every(a => new URL(a.href).searchParams.get('package_entry') === document.querySelector('[data-doc-browser]').dataset.docPackage)"), "package folder and pagination links retain package scope")
        self.chrome.evaluate("document.querySelector('.doc-view-toggle a:first-child').click()")
        self.wait("!location.search.includes('view=all') && !document.querySelector('[aria-busy=\"true\"]')", "package Children toggle")
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length") == 1, "Children keeps the package card")
        self.chrome.evaluate("history.back()")
        self.wait("location.search.includes('view=all') && !document.querySelector('[aria-busy=\"true\"]')", "Back restores package Options")
        self.chrome.evaluate("history.back()")
        self.wait("location.pathname.includes('/docs/config-demo/') && !document.querySelector('[aria-busy=\"true\"]')", "Back restores the historical package URL")
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length === 1 && !document.querySelector('.doc-view-toggle a:last-child').hasAttribute('aria-current')"), "Back restores the package card and Children view")
        self.chrome.evaluate("document.querySelector('.doc-view-toggle a:last-child').click()")
        self.wait("location.search.includes('view=all') && !document.querySelector('[aria-busy=\"true\"]')", "reopen package Options")
        self.chrome.evaluate("document.querySelector('[data-doc-folder] tbody a').click()")
        self.wait("document.querySelector('.doc-option') !== null && !document.querySelector('[aria-busy=\"true\"]')", "package option selection")
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length") == 1, "option selection retains the package overview")
        self.chrome.evaluate("document.querySelector('.doc-root').click()")
        self.wait("document.querySelector('#doc-folder-title')?.textContent === 'Configuration' && !document.querySelector('[aria-busy=\"true\"]')", "package root breadcrumb")
        self.chrome.evaluate("document.querySelector('#doc-query').value = 'enable'; document.querySelector('.doc-search').requestSubmit()")
        self.wait("document.querySelector('.doc-result') !== null && !document.querySelector('[aria-busy=\"true\"]')", "package search")
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length === 1 && document.querySelector('.doc-search [name=package_entry]').value === document.querySelector('[data-doc-browser]').dataset.docPackage"), "package search preserves the card and immutable scope")
        self.screenshot("package-guide-navigation")
        self.chrome.call("Emulation.setScriptExecutionDisabled", {"value": True})
        self.navigate(package_url)
        options_url = self.chrome.evaluate("document.querySelector('.doc-view-toggle a:last-child').getAttribute('href')")
        self.navigate(options_url)
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length === 1 && document.querySelectorAll('[data-doc-folder] tbody tr').length === 50"), "package Options works without JavaScript and remains bounded")
        children_url = self.chrome.evaluate("document.querySelector('.doc-view-toggle a:first-child').getAttribute('href')")
        self.navigate(children_url)
        self.check(self.chrome.evaluate("document.querySelectorAll('[data-doc-package-card]').length") == 1, "package Children keeps the card without JavaScript")
