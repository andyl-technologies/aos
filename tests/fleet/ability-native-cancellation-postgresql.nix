##! Production cancellation flights for the native PostgreSQL provider.
{
  lib,
  mkSystem,
  pkgs,
...
in
  import ./_ability-cancellation-cohort.nix {
    ...
    name = "ability-native-cancellation-postgresql";
    ...
    domainScript = ''
      ...
      def settle_postgresql(label, include_target, target_version):
          ...
          runtime.succeed(
              ...
              f"--eval-root /run/postgresql-cancel-baseline-{shlex.quote(label)}",
              timeout=1200,
          )
      ...
      for index, cell_id in enumerate(COHORT_CELLS):
          ...
          label = f"postgresql-cancel-{index:03d}"
          ...
          EFFECT_FLIGHT.run_cancellation_flight(
              flight, candidate, CANCELLATION_BUILDER, observe_postgresql
          )
    '';
  }
