    ...
    phases = [
      {
        ...
        script = ''
          ...
          grep -q '^selected_default_basis=provisional_modeled_smallest_quantum_above_throughput_floor$' "$out/result"
          ...
          grep -q '^s11_sim_rerun_green=false$' "$out/result"
          ...
          grep -q '^fallback_adopted=modeled_throughput_default_only_quantum_until_preemption_explorer$' "$out/result"
          grep -q '^s13_complete=false$' "$out/result"
