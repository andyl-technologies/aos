    ...
    phases = [
      {
        ...
        script = ''
          ...
          grep -q '^PASS_WITH_FALLBACK$' "$out/result"
          ...
          grep -q '^selected_default_basis=s11_green_smallest_quantum_above_throughput_floor$' "$out/result"
          ...
          grep -q '^race_yield_source=not_available_s12_preemption_explorer_disabled$' "$out/result"
          grep -q '^s12_decision_entry_consumed=true$' "$out/result"
          grep -q '^decision_preemption_exploration_enabled=false$' "$out/result"
          grep -q '^d25_status=open_until_s12_passes_without_fallback$' "$out/result"
          grep -q '^fallback_adopted=modeled_throughput_default_only_quantum_until_preemption_injection$' "$out/result"
