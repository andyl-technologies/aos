{
  ...
  # Deterministic structural proof that the atomic patch's virtio-rng delivery
  # change is byte-for-byte inert with sim off, closing the E7a async
  ...
  selectedPatchMicrotests =
    if campaignComposition == null
    then patchMicrotests
    else
      import ./phase2-patch-microtests.nix {
        inherit pkgs lib campaignComposition testing;
        qemuPackage = patchedQemu;
        inherit referenceQemu;
        dependencies = patchMicrotests.passthru.dependencies;
      };
  runtimeInputs =
    [
      pkgs.coreutils
  ...
  runtimeScript = ''
    ...
    ${lib.optionalString (campaignComposition != null) ''
      grep -Fxq 'gate=gate:patch-microtests' "$PATCH_MICROTESTS_RESULT" \
        || fail "mode-specific patch-microtests gate identity is absent"
      grep -Fxq 'campaign_mode=${campaignComposition.mode}' "$PATCH_MICROTESTS_RESULT" \
        || fail "mode-specific patch-microtests mode is absent"
      grep -Fxq \
        'campaign_configuration_identity=${campaignComposition.system.config.aos.services.crucibleCampaign._runtimeIdentity}' \
        "$PATCH_MICROTESTS_RESULT" \
        || fail "mode-specific patch-microtests configuration identity is absent"
      grep -Fxq 'campaign_toplevel=${campaignComposition.system.config.system.build.toplevel}' \
        "$PATCH_MICROTESTS_RESULT" \
        || fail "mode-specific patch-microtests toplevel is absent"
    ''}
    ...
    probe_qmp_surface() {
      ...
      for command in query-status query-machines query-block query-migrate query-migrate-capabilities migrate; do
      ...
      if [ "$label" = patched ]; then
        ...
        if grep -F -x -q -f "$TMPDIR/qmp-patched-only-expected.txt" \
          ...
          fail "reference QEMU exposed a Crucible-only QMP command"
        fi
      fi
      ...
      grep -F -x -v -f "$TMPDIR/qmp-patched-only-expected.txt" \
        "$TMPDIR/qmp-command-names-$label.txt" \
    ...
    probe_qmp_surface patched "$PATCHED_QEMU"
    ...
    compare_files qmp-command-set-delta \
      "$TMPDIR/qmp-patched-only-expected.txt" \
      "$TMPDIR/qmp-patched-only-commands.txt"
    ...
    status=complete
    evidence_scope=atomic-patch-sim-off-upstream-equivalent-corpus
