{
  ...
  sourceFile = builtins.toFile "phase0-s4-shmem-visibility.c" source;
  runtimeInputs = [pkgs.coreutils pkgs.gcc pkgs.grep];
  runtimeScript = ''
    set -eu
  ...
  authoritativeGate = pkgs.mkDerivation {
    ...
    meta.description = "Crucible Phase 0 S4 shared-memory visibility discipline spike";
  };
in
  ...
  then
    import ./phase9-campaign-mode-system-gate.nix {
      ...
      gateName = "gate:phase0-s4-shmem-visibility";
      authoritativeAttr = "checks.crucible.phase0.s4ShmemVisibility";
      authoritativeResultIdentity = "check=checks.crucible.phase0.s4ShmemVisibility";
      executionFamily = "qemu-runtime";
      name = "phase0-s4-shmem-visibility";
      runtimeClosures = [sourceFile];
      runtimeEnvironment.sourcePath = sourceFile;
    }
