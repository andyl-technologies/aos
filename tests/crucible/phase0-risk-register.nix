    ...
    phases = [
      {
        ...
        script = ''
          ...
          require_fixed "$risk_doc" "**RISK-8 / RISK-9** were initially mitigated by the historical \`T-RISK-4\`"
          require_fixed "$risk_doc" "That fallback is no longer an implementation path"
          require_fixed "$risk_doc" "there is no runtime fallback from incomplete VMState to thin replay"
          ...
          require_fixed "$decision_doc" "HISTORICAL SPIKE, SUPERSEDED"
          ...
          require_fixed "$decision_doc" "**Current decision:** no fallback"
          require_fixed "$decision_doc" "QemuExactSnapshotPolicy"
