...
#
# Built by M2-Planet from mescc-tools (stage 1). Targets x86 (32-bit) only.
...
#   - mes/module/: Scheme modules (including NYACC C parser)
#   - include/: C headers
#   - lib/: crt1.o and library archives
#
...
#
{
  ...
  mesCpu = "x86";
  m2Arch = "x86";
  m2Defines = "-D __i386__=1 -D __linux__=1";
  baseAddr = "0x08048000";

  # Fetch GNU Mes source tarball
  mesSrc = builtins.derivation {
    name = "mes-source-${version}";
    inherit system;
  ...
  # Fetch NYACC (Guile C99 parser, needed for mescc)
  nyaccSrc = builtins.derivation {
    name = "nyacc-source-1.00.2-lb1";
    inherit system;
    builder = "builtin:fetchurl";
  ...
  mes = builtins.derivation {
    ...
    buildScript = ''
      ...
      # ── Extract sources ────────────────────────────────────────────────
      ungz --file ${mesSrc} --output ''${WORK}/mes.tar
      untar --non-strict --file ''${WORK}/mes.tar
      rm ''${WORK}/mes.tar

      ungz --file ${nyaccSrc} --output ''${WORK}/nyacc.tar
      ...
      cp ''${MES_SRC}/include/linux/${mesCpu}/kernel-stat.h ''${MES_SRC}/include/arch/kernel-stat.h
      cp ''${MES_SRC}/include/linux/${mesCpu}/signal.h ''${MES_SRC}/include/arch/signal.h
      cp ''${MES_SRC}/include/linux/${mesCpu}/syscall.h ''${MES_SRC}/include/arch/syscall.h
      ...
      replace --file ''${BINDIR}/mescc.scm --output ''${BINDIR}/mescc.scm --match-on @VERSION@ --replace-with ${version}
      replace --file ''${BINDIR}/mescc.scm --output ''${BINDIR}/mescc.scm --match-on @mes_cpu@ --replace-with ${mesCpu}
      ...
      # Install Mes-specific Scheme modules — stubs override NYACC files
      # We use the Mes source tree's mes/module/ which has priority stubs
      # and the NYACC module/ which has the real parser files.
      # For kaem (no cp -r), we rely on the source tree layout at runtime.

      # Install NYACC to a separate dir for build-time GUILE_LOAD_PATH
      mkdir ''${out}/share
      mkdir ''${out}/share/nyacc

      # Install Guile-compatible modules
      mkdir ''${out}/share/mes
      mkdir ''${out}/share/mes/module

      # Install C headers — enumerate top-level files and subdirs
      # (kaem has no cp -r, but we need these for stage 3)
      # We'll copy the entire source include tree using catm trick:
      # Actually, for headers we need to enumerate. But the Mes source tree
      # has ~70 header files. Let's use a helper approach: we set up paths
      # so that stage 3 can reference the mes output include dir.

      # For now, install the critical headers individually (matching live-bootstrap)
      mkdir ''${INCDIR}/mes
      mkdir ''${INCDIR}/sys
      mkdir ''${INCDIR}/linux
      mkdir ''${INCDIR}/linux/${mesCpu}
      mkdir ''${INCDIR}/arch
      mkdir ''${INCDIR}/m2
      ...
      cp include/linux/${mesCpu}/signal.h ''${INCDIR}/arch/signal.h

      cp include/linux/syscall.h ''${INCDIR}/linux/syscall.h
      cp include/linux/${mesCpu}/syscall.h ''${INCDIR}/linux/${mesCpu}/syscall.h
      ...
      # Install library source files (needed by mescc to build libc in stage 3)
      mkdir ''${out}/lib/linux
      mkdir ''${out}/lib/linux/${mesCpu}-mes
      mkdir ''${out}/lib/linux/${mesCpu}-mes-mescc
      mkdir ''${out}/lib/linux/${mesCpu}-mes-m2
      mkdir ''${out}/lib/linux/${mesCpu}-mes-gcc
      mkdir ''${out}/lib/linux/m2
      mkdir ''${out}/lib/m2
      mkdir ''${out}/lib/m2/${mesCpu}
      mkdir ''${out}/lib/${mesCpu}-mes
      mkdir ''${out}/lib/${mesCpu}-mes-mescc
      mkdir ''${out}/lib/${mesCpu}-mes-gcc
      mkdir ''${out}/lib/mes
      mkdir ''${out}/lib/ctype
      ...
      # IMPORTANT: MES_PREFIX must point to the SOURCE TREE during the build.
      # GUILE_LOAD_PATH provides the search path for include-from-path.
      ...
      # GUILE_LOAD_PATH: mes stubs first, then module, then NYACC
      GUILE_LOAD_PATH=''${MES_SRC}/mes/module:''${MES_SRC}/module:''${NYACC_DIR}/module

      # mescc invokes M1, hex2, blood-elf as subprocesses
      ...
      DEFS_M1=''${MES_SRC}/lib/m2/${mesCpu}/${mesCpu}_defs.M1
      ARCH_M1=''${MES_SRC}/lib/${mesCpu}-mes/${mesCpu}.M1

      cd ''${MES_SRC}

      # ── crt1.o ──
      echo "==> Building crt1.o..."
      ''${MES} --no-auto-compile -e main ''${MESCC} -- -D HAVE_CONFIG_H=1 -I ''${INCDIR} -I ''${INCDIR}/linux/${mesCpu} -c lib/linux/${mesCpu}-mes-mescc/crt1.c -o lib/${mesCpu}-mes/crt1.o

      # Assemble .s -> .o
      ${mescc-tools}/bin/M1 --little-endian --architecture ${m2Arch} -f ''${DEFS_M1} -f ''${ARCH_M1} -f lib/${mesCpu}-mes/crt1.s -o ''${LIBDIR}/crt1.o
      cp lib/${mesCpu}-mes/crt1.s ''${LIBDIR}/crt1.s

      # ── libc-mini.a ──
      echo "==> Building libc-mini.a..."
      catm ''${TMPDIR}/libc-mini.c lib/mes/__init_io.c lib/mes/eputs.c lib/mes/oputs.c lib/mes/globals.c lib/stdlib/exit.c lib/linux/${mesCpu}-mes-mescc/syscall.c lib/linux/${mesCpu}-mes-mescc/_exit.c lib/linux/${mesCpu}-mes-mescc/_write.c lib/stdlib/puts.c lib/string/strlen.c

      ''${MES} --no-auto-compile -e main ''${MESCC} -- -D HAVE_CONFIG_H=1 -I ''${INCDIR} -I ''${INCDIR}/linux/${mesCpu} -c ''${TMPDIR}/libc-mini.c

      ${mescc-tools}/bin/M1 --little-endian --architecture ${m2Arch} -f ''${DEFS_M1} -f ''${ARCH_M1} -f libc-mini.s -o ''${LIBDIR}/libc-mini.a
      cp libc-mini.s ''${LIBDIR}/libc-mini.s

      # ── libmescc.a ──
      echo "==> Building libmescc.a..."
      catm ''${TMPDIR}/libmescc.c lib/mes/globals.c lib/linux/${mesCpu}-mes-mescc/syscall-internal.c

      ''${MES} --no-auto-compile -e main ''${MESCC} -- -D HAVE_CONFIG_H=1 -I ''${INCDIR} -I ''${INCDIR}/linux/${mesCpu} -c ''${TMPDIR}/libmescc.c

      ${mescc-tools}/bin/M1 --little-endian --architecture ${m2Arch} -f ''${DEFS_M1} -f ''${ARCH_M1} -f libmescc.s -o ''${LIBDIR}/libmescc.a
      cp libmescc.s ''${LIBDIR}/libmescc.s

      # ── libc.a ──
      echo "==> Building libc.a..."
      catm ''${TMPDIR}/libc.c lib/ctype/isnumber.c lib/mes/abtol.c lib/mes/cast.c lib/mes/eputc.c lib/mes/fdgetc.c lib/mes/fdputc.c lib/mes/fdputs.c lib/mes/fdungetc.c lib/mes/itoa.c lib/mes/ltoa.c lib/mes/ltoab.c lib/mes/mes_open.c lib/mes/ntoab.c lib/mes/oputc.c lib/mes/ultoa.c lib/mes/utoa.c lib/ctype/isdigit.c lib/ctype/isspace.c lib/ctype/isxdigit.c lib/mes/assert_msg.c lib/posix/write.c lib/stdlib/atoi.c lib/linux/lseek.c lib/mes/__assert_fail.c lib/mes/__buffered_read.c lib/mes/__mes_debug.c lib/posix/execv.c lib/posix/getcwd.c lib/posix/getenv.c lib/posix/isatty.c lib/posix/open.c lib/posix/buffered-read.c lib/posix/setenv.c lib/posix/wait.c lib/dirent/closedir.c lib/dirent/opendir.c lib/stdio/fgetc.c lib/stdio/fputc.c lib/stdio/fputs.c lib/stdio/getc.c lib/stdio/getchar.c lib/stdio/putc.c lib/stdio/putchar.c lib/stdio/ungetc.c lib/stdlib/calloc.c lib/stdlib/free.c lib/stdlib/realloc.c lib/string/memchr.c lib/string/memcmp.c lib/string/memcpy.c lib/string/memmove.c lib/string/memset.c lib/string/strcmp.c lib/string/strcpy.c lib/string/strncmp.c lib/posix/raise.c lib/linux/access.c lib/linux/brk.c lib/linux/chdir.c lib/linux/chmod.c lib/linux/clock_gettime.c lib/linux/dup.c lib/linux/dup2.c lib/linux/execve.c lib/linux/fork.c lib/linux/fsync.c lib/linux/_getcwd.c lib/linux/gettimeofday.c lib/linux/ioctl3.c lib/linux/malloc.c lib/linux/_open3.c lib/linux/_read.c lib/linux/readdir.c lib/linux/rename.c lib/linux/time.c lib/linux/umask.c lib/linux/uname.c lib/linux/unlink.c lib/linux/utimensat.c lib/linux/wait4.c lib/linux/waitpid.c lib/linux/${mesCpu}-mes-mescc/syscall.c lib/linux/getpid.c lib/linux/kill.c lib/linux/pipe.c lib/linux/stat.c lib/linux/lstat.c lib/linux/mkdir.c lib/linux/rmdir.c lib/linux/link.c lib/linux/symlink.c lib/linux/close.c lib/linux/nanosleep.c lib/linux/fcntl.c lib/linux/fstat.c lib/linux/getdents.c

      ''${MES} --no-auto-compile -e main ''${MESCC} -- -D HAVE_CONFIG_H=1 -I ''${INCDIR} -I ''${INCDIR}/linux/${mesCpu} -c ''${TMPDIR}/libc.c

      # M1 assemble .s -> .a (mescc's internal M1 may be broken in Nix sandbox)
      ${mescc-tools}/bin/M1 --little-endian --architecture ${m2Arch} -f ''${DEFS_M1} -f ''${ARCH_M1} -f libc.s -o ''${TMPDIR}/libc.o
      # libc.a = libc-mini.a + libc.o
      catm ''${LIBDIR}/libc.a ''${LIBDIR}/libc-mini.a ''${TMPDIR}/libc.o
      catm ''${LIBDIR}/libc.s ''${LIBDIR}/libc-mini.s libc.s

      # ── libc+tcc.a ──
      echo "==> Building libc+tcc.a..."
      catm ''${TMPDIR}/libc+tcc.c lib/ctype/islower.c lib/ctype/isupper.c lib/ctype/tolower.c lib/ctype/toupper.c lib/mes/abtod.c lib/mes/dtoab.c lib/mes/search-path.c lib/posix/execvp.c lib/stdio/fclose.c lib/stdio/fdopen.c lib/stdio/ferror.c lib/stdio/fflush.c lib/stdio/fopen.c lib/stdio/fprintf.c lib/stdio/fread.c lib/stdio/fseek.c lib/stdio/ftell.c lib/stdio/fwrite.c lib/stdio/printf.c lib/stdio/remove.c lib/stdio/snprintf.c lib/stdio/sprintf.c lib/stdio/sscanf.c lib/stdio/vfprintf.c lib/stdio/vprintf.c lib/stdio/vsnprintf.c lib/stdio/vsprintf.c lib/stdio/vsscanf.c lib/stdlib/abort.c lib/stdlib/qsort.c lib/stdlib/strtod.c lib/stdlib/strtof.c lib/stdlib/strtol.c lib/stdlib/strtold.c lib/stdlib/strtoll.c lib/stdlib/strtoul.c lib/stdlib/strtoull.c lib/string/memmem.c lib/string/strcat.c lib/string/strchr.c lib/string/strlwr.c lib/string/strncpy.c lib/string/strrchr.c lib/string/strstr.c lib/string/strupr.c lib/stub/sigaction.c lib/stub/ldexp.c lib/stub/mprotect.c lib/stub/localtime.c lib/stub/sigemptyset.c lib/${mesCpu}-mes-mescc/setjmp.c lib/linux/close.c lib/linux/rmdir.c lib/linux/stat.c

      ''${MES} --no-auto-compile -e main ''${MESCC} -- -D HAVE_CONFIG_H=1 -I ''${INCDIR} -I ''${INCDIR}/linux/${mesCpu} -c ''${TMPDIR}/libc+tcc.c

      # M1 assemble .s -> .a
      ${mescc-tools}/bin/M1 --little-endian --architecture ${m2Arch} -f ''${DEFS_M1} -f ''${ARCH_M1} -f libc+tcc.s -o ''${TMPDIR}/libc+tcc.o
      # libc+tcc.a = libc.a + libc+tcc.o
      catm ''${LIBDIR}/libc+tcc.a ''${LIBDIR}/libc.a ''${TMPDIR}/libc+tcc.o
      catm ''${LIBDIR}/libc+tcc.s ''${LIBDIR}/libc.s libc+tcc.s

      # ── Install M1/hex2 architecture files (needed for linking) ───────
      mkdir ''${out}/lib/linux/${mesCpu}-mes
      cp lib/linux/${mesCpu}-mes/elf32-footer-single-main.hex2 ''${out}/lib/linux/${mesCpu}-mes/elf32-footer-single-main.hex2
      cp lib/linux/${mesCpu}-mes/elf32-header.hex2 ''${out}/lib/linux/${mesCpu}-mes/elf32-header.hex2
      cp lib/${mesCpu}-mes/${mesCpu}.M1 ''${LIBDIR}/${mesCpu}.M1

      # ── Install library source files for stage 3 ─────────────────────
      # Stage 3 (TinyCC) needs to recompile the Mes libc from source.
      # Copy individual source files that the stage 3 libcSourceList references.
      ...
      # linux syscalls (arch-specific)
      cp lib/linux/${mesCpu}-mes-gcc/syscall.c ''${out}/lib/linux/${mesCpu}-mes-gcc/syscall.c
      cp lib/linux/${mesCpu}-mes-gcc/_exit.c ''${out}/lib/linux/${mesCpu}-mes-gcc/_exit.c
      cp lib/linux/${mesCpu}-mes-gcc/_write.c ''${out}/lib/linux/${mesCpu}-mes-gcc/_write.c
      cp lib/linux/${mesCpu}-mes-gcc/crt1.c ''${out}/lib/linux/${mesCpu}-mes-gcc/crt1.c
      cp lib/linux/${mesCpu}-mes-gcc/crtn.c ''${out}/lib/linux/${mesCpu}-mes-gcc/crtn.c
      cp lib/linux/${mesCpu}-mes-gcc/crti.c ''${out}/lib/linux/${mesCpu}-mes-gcc/crti.c
      ...
      # arch-specific
      cp lib/${mesCpu}-mes-gcc/setjmp.c ''${out}/lib/${mesCpu}-mes-gcc/setjmp.c

      # libtcc1.c (Mes's pure-C version, needed for stage 3)
      cp lib/libtcc1.c ''${out}/lib/libtcc1.c

      # M1 architecture definitions (needed for stage 3 manual linking)
      cp lib/m2/${mesCpu}/${mesCpu}_defs.M1 ''${out}/lib/m2/${mesCpu}/${mesCpu}_defs.M1
      cp lib/${mesCpu}-mes/${mesCpu}.M1 ''${out}/lib/${mesCpu}-mes/${mesCpu}.M1

      # Scheme modules — needed for mescc to run in stage 3
      # We store these as Nix store references so stage 3 can set GUILE_LOAD_PATH
      # pointing to the source tree paths. Stage 3's GUILE_LOAD_PATH will be:
      # $out/mes/module:$out/share/mes/module:$out/share/nyacc

      echo "Stage 2 complete: GNU Mes ${version} built successfully (x86/i386)"
