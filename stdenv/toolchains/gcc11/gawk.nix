...
builtins.derivation {
  ...
  args = [
    ...
    ''
      ...
      cd "$TMPDIR"
      mkdir gawk-5.1.0 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd gawk-5.1.0 && ${prev.tar}/bin/tar xf -)
      cd gawk-5.1.0
      chmod -R u+w .
...
// {
  meta = {
    description = "GNU awk pattern scanning and processing language, version 5.1.0";
