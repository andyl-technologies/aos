# stdenv/toolchains/gcc11/autoconf.nix — GNU Autoconf 2.71 (RHEL 9)
#
# Built with THIS tier's GCC 11.5.0 + binutils 2.35 + glibc 2.34.
# Autoconf is mostly Perl scripts; needs m4 and perl.
#
{
  prev,
  gcc,
...
let
  src = builtins.fetchTarball {
    ...
    sha256 = "048zp73c7gczz1ms5jx8nphgc8xr6xpyb56dbqixcan5a1psh4hm";
  };
in
builtins.derivation {
  name = "autoconf-2.71";
  ...
  args = [
    ...
    ''
      ...
      export PATH="${texinfo}/bin:${help2man}/bin:${prev.coreutils}/bin:${gcc}/bin:${binutils}/bin:${prev.gnumake}/bin:${prev.sed}/bin:${prev.grep}/bin:${prev.gawk}/bin:${prev.findutils}/bin:${prev.tar}/bin:${prev.gzip}/bin:${prev.diffutils}/bin:${prev.bash}/bin:${prev.patch}/bin:${m4}/bin:${perl}/bin"
      ...
      cd "$TMPDIR"
      mkdir autoconf-2.71 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd autoconf-2.71 && ${prev.tar}/bin/tar xf -)
      cd autoconf-2.71
      chmod -R u+w .
      ...
      M4="${m4}/bin/m4" \
      PERL="${perl}/bin/perl" \
      "$TMPDIR/autoconf-2.71/configure" \
        --prefix="$out" \
      ...
      echo "GNU Autoconf 2.71 installed to $out"
    ''
  ];
}
// {
  meta = {
    description = "GNU Autoconf, version 2.71";
