...
let
  ...
  src = fetchTarball {
    ...
    hash = "0jk31l6w6bd2x067hcqa8zjvx202f85kk5khkidy6pig0p3yx8a8";
  };
in
builtins.derivation {
  name = "binutils-2.35";
  ...
  args = [
    ...
    ''
      ...
      cd "$TMPDIR"
      mkdir binutils-2.35 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd binutils-2.35 && ${prev.tar}/bin/tar xf -)
      cd binutils-2.35
      chmod -R u+w .
...
// {
  meta = {
    description = "GNU tools for manipulating binaries, version 2.35";
