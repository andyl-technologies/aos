  ...
  builtins.derivation {
    ...
    args = [
      ...
      ''
        ...
        cd "$TMPDIR"
        mkdir gzip-1.9 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd gzip-1.9 && ${prev.tar}/bin/tar xf -)
        cd gzip-1.9
        chmod -R u+w .
        ...
        CPPFLAGS="-nostdinc -isystem $GCC_INCDIR -isystem $GCC_INCDIR-fixed -isystem ${glibc}/include -D_IO_ftrylockfile -D_IO_IN_BACKUP=0x100" \
