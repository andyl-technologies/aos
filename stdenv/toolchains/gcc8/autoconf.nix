# stdenv/toolchains/gcc8/autoconf.nix — GNU Autoconf 2.69 (RHEL 8)
#
...
builtins.derivation {
  ...
  args = [
    ...
    ''
      ...
      cd "$TMPDIR"
      mkdir autoconf-2.69 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd autoconf-2.69 && ${prev.tar}/bin/tar xf -)
      cd autoconf-2.69
      chmod -R u+w .
      ...
      "$TMPDIR/autoconf-2.69/configure" \
        --prefix="$out" \
