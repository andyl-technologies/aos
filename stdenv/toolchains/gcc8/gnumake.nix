# stdenv/toolchains/gcc8/gnumake.nix — GNU Make 4.3
#
# Built with THIS tier's GCC 8.5.0 + binutils 2.30 + glibc 2.28.
# Upgraded from 4.2.1 to fix segfault when parsing GCC 11's large Makefiles.
#
{
  prev,
  gcc,
...
builtins.derivation {
  ...
  args = [
    ...
    ''
      ...
      cd "$TMPDIR"
      mkdir make-4.3 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd make-4.3 && ${prev.tar}/bin/tar xf -)
      cd make-4.3
      chmod -R u+w .
...
// {
  meta = {
    description = "GNU Make build automation tool, version 4.3";
