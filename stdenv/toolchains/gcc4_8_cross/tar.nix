# stdenv/toolchains/gcc4_8_cross/tar.nix — Phase 7
#
# Native target-arch GNU tar 1.26, cross-compiled from x86_64.
#
{
  prev,
  crossGccStage2,
...
let
  src = builtins.fetchTarball {
    ...
    sha256 = "05d4g4g0wzfzrbyc4cl7rz1bgh4ahnir38gz400cyrsvyjgmg0nk";
  };
in
builtins.derivation {
  name = "tar-1.26";
  ...
  args = [
    ...
    ''
      ...
      ${prev.sed}/bin/sed -i '/_GL_WARN_ON_USE (gets,/d' "$TMPDIR/src/gnu/stdio.in.h" 2>/dev/null || true
      ...
      echo "GNU tar 1.26 (${hostPlatform.config}) installed to $out"
    ''
  ];
}
