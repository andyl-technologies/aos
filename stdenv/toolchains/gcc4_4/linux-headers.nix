...
in
  builtins.derivation {
    ...
    args = [
      ...
      ''
        set -eu
        export PATH="${prev.coreutils}/bin:${prev.gcc}/bin:${prev.binutils}/bin:${prev.gnumake}/bin:${prev.bash}/bin:${prev.sed}/bin:${prev.grep}/bin:${prev.gawk}/bin:${prev.findutils}/bin:${prev.diffutils}/bin:${prev.tar}/bin:${prev.gzip}/bin:${prev.bzip2}/bin:${prev.patch}/bin:${prev.perl}/bin"
        ...
        # Use the kernel's export rules to remove kernel-only declarations and
        # compiler annotations together. The preceding tier now supplies Perl;
        # Kbuild compiles its own unifdef helper with the static AOS compiler.
        make SHELL="${prev.bash}/bin/bash" \
          CONFIG_SHELL="${prev.bash}/bin/bash" \
          HOSTCC="${prev.gcc}/bin/gcc -static" \
