# stdenv/toolchains/gcc14/flex.nix — flex 2.6.4 (RHEL 10)
#
# Fast lexical analyzer generator built with THIS tier's GCC 14.3.0 + binutils 2.41 + glibc 2.39.
#
{
  prev,
  gcc,
...
builtins.derivation {
  ...
  args = [
    ...
    ''
      ...
      export PATH="${texinfo}/bin:${help2man}/bin:${m4}/bin:${prev.coreutils}/bin:${gcc}/bin:${binutils}/bin:${prev.gnumake}/bin:${prev.sed}/bin:${prev.grep}/bin:${prev.gawk}/bin:${prev.findutils}/bin:${prev.tar}/bin:${prev.gzip}/bin:${prev.diffutils}/bin:${prev.bash}/bin:${prev.patch}/bin"
      export CONFIG_SHELL="${prev.bash}/bin/bash"

      cd "$TMPDIR"
      mkdir flex-2.6.4 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd flex-2.6.4 && ${prev.tar}/bin/tar xf -)
      cd flex-2.6.4
      chmod -R u+w .
      ...
      echo "flex 2.6.4 installed to $out"
    ''
  ];
}
// {
  meta = {
    description = "Fast lexical analyzer generator 2.6.4";
