# stdenv/toolchains/gcc14/perl.nix — Perl 5.38.2 (RHEL 10)
#
# Perl built with THIS tier's GCC 14.3.0 + binutils 2.41 + glibc 2.39.
# Uses Perl's own Configure script (not autoconf).
#
{
  prev,
  gcc,
...
builtins.derivation {
  ...
  args = [
    ...
    ''
      ...
      cd "$TMPDIR"
      mkdir perl-5.38.2 && (cd ${src} && ${prev.tar}/bin/tar cf - .) | (cd perl-5.38.2 && ${prev.tar}/bin/tar xf -)
      cd perl-5.38.2
      chmod -R u+w .
      ...
      echo "Perl 5.38.2 installed to $out"
    ''
  ];
}
// {
  meta = {
    description = "Perl programming language 5.38.2";
