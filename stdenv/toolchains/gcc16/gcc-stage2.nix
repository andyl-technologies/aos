# stdenv/toolchains/gcc16/gcc-stage2.nix - bootstrapped GCC 16.2.0
#
...
in
  mkGcc {
    ...
    '';
    postConfigure = ''
      # GCC 16 fixincludes does not always generate include-fixed/limits.h.
      mkdir -p "$TMPDIR/build/gcc/include-fixed"
      cat > "$TMPDIR/build/gcc/include-fixed/limits.h" <<'LIMITS_EOF'
      /* Generated for GCC 16 bootstrap - chains to system limits.h. */
    ...
    postInstall = ''
      ...
      SPEC_DIR="$out/lib/gcc/${targetPlatform.config}/16.2.0"
    ...
    '';
    finalMessage = "GCC 16.2.0 bootstrapped to $out";
    meta = {
      description = "GNU Compiler Collection 16.2.0 - bootstrapped against tier-own glibc";
