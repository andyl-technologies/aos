# stdenv/toolchains/gcc16/gcc.nix - GCC 16.2.0 stage1 (RHEL 10)
#
# Stage1 is built by GCC 11.5.0 from the previous tier. It is consumed only
# inside the gcc16 tier to build binutils, linux headers, glibc, and the final
# bootstrapped GCC.
{
  prev,
  buildPlatform,
...
in
  mkGcc {
    ...
    configureEnv = [
      ...
      # libstdc++ wrapper headers use include_next to reach the C headers.
      ''CXXFLAGS="-O2 -static -idirafter ${prev.glibc}/include"''
    ...
    '';
    finalMessage = "GCC 16.2.0 installed to $out";
    meta = {
      description = "GNU Compiler Collection 16.2.0 - stage1 compiler with PIE+SSP";
