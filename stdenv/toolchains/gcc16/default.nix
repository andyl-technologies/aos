# stdenv/toolchains/gcc16/default.nix — GCC 16.2.0 toolchain tier (RHEL 10)
#
# Takes the gcc11 toolchain as `prev` and builds the RHEL 10 era toolchain:
#   Phase 1: GCC 16.2.0 (C/C++, in-tree GMP/MPFR/MPC/ISL, PIE+SSP by default)
...
#
{
  ...
  # Phase 1: stage-1 GCC 16.2.0 built with prev.gcc (11.5.0).
  ...
  baseScope = {
    ...
    # Shared mini-stdenv for gcc16's POSIX/autotools tools. Use the wrapped
    # final bootstrapped compiler and this tier's split glibc outputs; the build shell
    # remains previous-tier bash while gcc16 bash is being built.
