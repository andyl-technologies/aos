        ...
        vlanNetworkFiles = lib.mapAttrs' (
          ...
          lib.nameValuePair "systemd/network/10-${name}.network" {
            text = ''
              ...
              [Network]
              ${if vlanCfg.address != "" then "Address=${vlanCfg.address}" else "DHCP=yes"}
            '';
          }
        ) cfg.vlans;

        # Bond netdev and network units.
        bondNetdevFiles = lib.mapAttrs' (
          name: bondCfg:
          lib.nameValuePair "systemd/network/05-${name}.netdev" {
            text = ''
              # /etc/systemd/network/05-${name}.netdev
        ...
        bondNetworkFiles = lib.mapAttrs' (
          ...
          lib.nameValuePair "systemd/network/10-${name}.network" {
            text = ''
              ...
              [Network]
              ${if bondCfg.address != "" then "Address=${bondCfg.address}" else "DHCP=yes"}
            '';
          }
        ) cfg.bonds;
