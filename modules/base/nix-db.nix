...
{pkgs, ...}: {
  config = {
    # Nix's compatibility shell is an explicit AOS dependency. A bundled or
    # host-provided shell must not become an undeclared build input.
    environment.etc."nix/nix.conf" = {
      text = ''
        # Generated by modules/base/nix-db.nix — do not edit manually.
        build-users-group =
        sandbox-paths = /bin/sh=${pkgs.bash}/bin/bash
      '';
    };
    ...
    system.checks.nix-db = {
      ...
      checks = [
        ...
        }
        {
          ...
          script = ''
            config = vm.succeed("cat /etc/nix/nix.conf")
            assert "build-users-group =" in config
            assert "sandbox-paths = /bin/sh=${pkgs.bash}/bin/bash" in config
          '';
        }
        {
          name = "gcroot-bridge";
