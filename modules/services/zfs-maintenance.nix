...
##! reflects it.
{
  ...
  zedConfiguration = ''
    # /etc/zfs/zed.rc — generated by modules/services/zfs-maintenance.nix
...
in {
  ...
  config = lib.mkIf (cfg.enable && zfs.enable) {
    ...
    # zed reads its hooks from /etc/zfs/zed.d. The scripts ship with the
    # package; the generated zed.rc selects the policy.
    environment.etc = lib.mkMerge [
      (lib.mkIf cfg.eventDaemon {
        "zfs/zed.d".source = "${zfs.package}/etc/zfs/zed.d";
        "zfs/zed.rc".text = zedConfiguration;
      })
    ];
