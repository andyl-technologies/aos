...
##!   [security.hardening] enable, sysctl, core_dump
{
  ...
  corePattern =
    ...
    then "|${pkgs.systemd}/lib/systemd/systemd-coredump %P %u %g %s %t %c %h %e"
  ...
  kernelTunables = lib.abilities.interfaces.serviceManagement.forProducer {
    ...
    parameters = {
      values = cfg.sysctl // {"kernel.core_pattern" = corePattern;};
      dependencies = [];
    };
  };
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    system.checks.kernel-security = {
      ...
      checks = [
        ...
        # systemd-sysctl pass racing the /etc overlay.
        {
          name = "aslr";
      ...
      checks = [
        {
          ...
          script = ''
            ...
            # on an early systemd-sysctl pass through the /etc overlay.
            vm.wait_until_succeeds(
    ...
    # The userspace coredump policy remains ordinary immutable configuration;
    # only its kernel-facing values belong to the tunable provider.
    ...
    # Resource limits prevent per-process core dumps when collection is off.
