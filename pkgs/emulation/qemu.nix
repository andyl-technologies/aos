            ...
            then ''
              ...
              checks = [
                  ...
                  ("active host readiness crosses the pre-replay poll seam",
                   ...
                   r'cont_id = live\.qmp_send\("cont"\).*?'
                   r"wait_for_rr_dispatch_event\(startup_deadline\).*?"
                   r"live\.qmp_wait\(cont_id, deadline=startup_deadline\).*?"
                   r"blocked_dispatch_index = 1.*?"
                   r"blocked_icount = RR_SWITCH_QUANTUM \* 2.*?"
                   r"live.control_state\[16:20\] = "
                   r"blocked_dispatch_index\.to_bytes.*?"
                   r"live.control_state\[8:16\] = "
                   r"startup_ceiling\.to_bytes.*?"
                   r"os.eventfd_write\(live.wake_fd, 1\).*?"
                   r'"control", \(RR_SWITCH_QUANTUM, 1\).*?'
                   r"blocked_dispatch == \(\"dispatch\", blocked_icount\).*?"
                   r"wait_for_host_poll_state\(deadline\).*?"
                   r"if poll_timeout == -1:\s*break.*?"
                   r"transient_polls\.append\(.*?"
                   r"len\(transient_polls\) > "
                   r"ACTIVE_HOST_READY_MAX_TRANSIENT_POLLS.*?"
                   r"next_dispatch_index = blocked_dispatch_index \+ 1.*?"
                   r"live.control_state\[16:20\] = "
                   r"next_dispatch_index\.to_bytes.*?"
                   r"live.control_state\[4:8\] = "
                   r"blocked_dispatch_index\.to_bytes.*?"
                   r"blocked_dispatch_index = next_dispatch_index.*?"
                   r"blocked_icount \+= RR_SWITCH_QUANTUM.*?"
                   r"blocked_dispatch == \(\"dispatch\", blocked_icount\).*?"
                   r"dispatch_ceiling = blocked_icount \+ "
                   r"RR_SWITCH_QUANTUM \* 2.*?"
                   r"live.control_state\[20:24\] = "
                   r"blocked_dispatch_index\.to_bytes.*?"
                   r'query_status_id = live\.qmp_send\("query-status"\).*?'
                   r"if poll_pending:.*?"
                   r"wait_for_infinite_host_poll_ready\(\s*"
                   r"prior_generation, deadline\s*\).*?"
                   r"os.eventfd_write\(live.wake_fd, 1\).*?"
                   r"live.control_state\[4:8\] = "
                   r"blocked_dispatch_index\.to_bytes.*?"
                   r"live\.qmp_wait\(query_status_id, deadline=deadline\).*?"
                   r'expected_control = \(\s*"control", '
                   r"\(dispatch_ceiling, 2\)\s*\).*?"
                   r"expected = \[\s*"
                   r"\(\"dispatch\", blocked_icount \+ "
                   r"RR_SWITCH_QUANTUM\),\s*"
                   r"\(\"dispatch\", dispatch_ceiling\),\s*"
                   r"expected_control,\s*\]", 1),
                  ("active host readiness holds the RR boundary",
                   idle_plugin,
                   r"if \(active_host_ready\) \{.*?"
                   r"active_dispatch = atomic_fetch_add_explicit\(\s*"
                   r"&active_host_ready_dispatches, 1,\s*"
                   r"memory_order_relaxed\);.*?"
                   r"active_dispatch != 0.*?"
                   r"disable_after = atomic_load_explicit\(\s*"
                   r"&shared_state->active_host_ready_disable_after,\s*"
                   r"memory_order_acquire\);.*?"
                   r"disable_after == 0 \|\| "
                   r"active_dispatch <= disable_after.*?"
                   r"atomic_load_explicit\(\s*"
                   r"&shared_state->active_host_ready_arm,\s*"
                   r"memory_order_acquire\).*?"
                   r"armed_dispatch < active_dispatch.*?"
                   r"atomic_load_explicit\(\s*"
                   r"&shared_state->active_host_ready_release,\s*"
                   r"memory_order_acquire\) < active_dispatch", 1),
