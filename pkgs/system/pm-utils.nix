...
in
  mkDerivation {
    ...
    buildDeps = [gnumake];
    runtimeDeps =
      [coreutils grep util-linux kmod procps-ng kbd dbus]
      ++ lib.optionals isLinuxCross [bash];
    propagatedDeps = [];
      ...
      }
      {
        ...
        script = ''
          ...
          ${lib.optionalString isLinuxCross ''
            # Build scripts use the native configure shell; installed hooks
            # must retain and execute the target shell on the deployed host.
            find "$out" -type f | while read -r script; do
              first=$(head -n 1 "$script" 2>/dev/null || true)
              case "$first" in
                "#!$CONFIG_SHELL") sed -i "1c#!${bash}/bin/bash" "$script" ;;
              esac
            done
