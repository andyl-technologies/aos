##! aos-boot-transaction-storage-provider - checked ESP journal view
{
  lib,
  stdenv,
  mkCargoPackage,
  ...
  cargoArtifactContract = {
    family = "aos-boot-transaction-storage-provider-static-release-and-test";
    target = targetTriple;
  ...
  cargoArtifacts = mkCargoArtifacts {
    pname = "aos-boot-transaction-storage-provider-static-artifacts";
    inherit version cargoDeps cargoArtifactContract;
    src = mkCargoDummySource {
      srcRoot = ../../crates;
      name = "aos-boot-transaction-storage-provider-dummy-source";
      cargoRoot = "crates";
    };
    cargoRoot = "crates";
    cargoBuildCommands = [
...
in
  mkCargoPackage {
    pname = "aos-boot-transaction-storage-provider";
    qualification.packageProbe = lib.qualification.providerExecutableProbe {
      name = "aos-boot-transaction-storage-provider";
    ...
    postInstall = ''
      ...
      if patchelf --print-interpreter "$out/bin/aos-boot-transaction-storage-provider" \
        ...
        printf '%s unexpectedly has ELF interpreter: ' aos-boot-transaction-storage-provider
