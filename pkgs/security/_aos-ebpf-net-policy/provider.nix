##! Pure controller for selected BPF network policy resources.
{
  config,
  lib,
  packageName,
  ...
  in
    ...
    else throw "a BPF network policy request must have exactly one selected binding";
    ...
    // {
      ...
      realizations =
        builtins.mapAttrs (_: _: {
          schema = "aos.linux.ebpf-net-policy-realization/v1";
          loader = {
            artifact = lib.abilities.packageOutput {};
            entry_point = "libexec/aos-ebpf-net-policy-loader";
            arguments = [];
          };
          pin_directory = "/sys/fs/bpf/aos/network";
        })
        resources;
    };
  transition = context:
