##! aos-registry-hub-cloudflare — the hub binary packaged as a self-contained
##! Cloudflare installer.
##!
##! The base `aos-registry-hub` is a lean server binary; its `cloudflare`
...
##!   `aos-registry-worker-dist`) — the payload `wrangler deploy` uploads, copied
...
##! `aos_registry_hub::cloudflare::Assets::from_env`) and prepends `node` to
...
mkDerivation {
  pname = "aos-registry-hub-cloudflare";
  version = "0.1.0";
  ...
  runtimeDeps = [aos-registry-hub miniflare nodejs bash];
  ...
  phases = [
    {
      ...
      script = ''
        ...
        cp ${aos-registry-worker-dist}/shim.mjs ${aos-registry-worker-dist}/index.wasm \
        ...
        cat > "$out/bin/aos-registry-hub" <<EOF
        #!${bash}/bin/bash
        export AOS_HUB_WORKER_DIST="$out/share/aos-hub/worker"
        export AOS_HUB_WRANGLER="${miniflare}/bin/wrangler"
        export PATH="${nodejs}/bin:\$PATH"
        exec ${aos-registry-hub}/bin/aos-registry-hub "\$@"
        EOF
        chmod +x "$out/bin/aos-registry-hub"
      '';
    }
  ];

  meta = {
    description = "aos-registry-hub packaged with wrangler + the Worker wasm dist as a Cloudflare installer";
