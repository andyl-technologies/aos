##! aos-registry-hub — multi-tenant AOS registry management hub (RFC-0004)
##!
##! Builds the `aos-registry-hub` binary from the shared `crates/` cargo
...
##! runtime and needs no PATH wrapper — `$out/bin/aos-registry-hub` is the
##! complete artifact.
##!
...
in
  mkCargoPackage {
    pname = "aos-registry-hub";
    inherit version src;

    # Build only the hub binary out of the workspace.
    cargoFlags = "-p aos-registry-hub";
    ...
    meta = {
      description = "aos-registry-hub — multi-tenant AOS registry management hub";
