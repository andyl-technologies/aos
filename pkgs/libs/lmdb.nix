##! lmdb — Lightning Memory-Mapped Database
{
  ...
  version = "1.0.1";
  # Upstream's Makefile defaults to ELF soname flags and Linux libdl.
  darwinMakeFlags =
    lib.optionalString stdenv.hostPlatform.isDarwin
    "SOEXT=.dylib VERSION_OPT=-Wl,-current_version,1.0 LDL=";
in
  mkDerivation {
    platformSupport = {
    ...
    phases =
      [
        ...
        }
        {
          name = "build";
          script = ''
            make -j"$NIX_BUILD_CORES" CC="$CC" AR="$AR" ${darwinMakeFlags}
          '';
        }
      ]
      ...
      ++ [
        {
          name = "install";
          script = ''
            make install prefix="$out" CC="$CC" AR="$AR" ${darwinMakeFlags}
