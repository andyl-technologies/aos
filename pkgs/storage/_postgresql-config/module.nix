...
##! the service-owned state directory across package generations.
{
  ...
  hostResources = import ./host-resources.nix {inherit lib;};
...
in {
  ...
  config = {
    # Each retained database allocation receives distinct service, probe, and
    # broker identities. Keeping these declarations with the provider package
    # prevents unrelated systems from reserving PostgreSQL-specific accounts.
    aos.users = {
      inherit (hostResources) users groups;
    };
    ...
    environment.etc = {
      ...
      "tmpfiles.d/aos-ability-postgresql.conf".text = hostResources.tmpfiles;
    };
  };
}
