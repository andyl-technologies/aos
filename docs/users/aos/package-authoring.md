...
Apply the same rule to a compiler-ladder boundary when the previous JDK's
module reader is not concurrency-safe: build OpenJDK 11 with one job because
JDK 10 javac can throw `ConcurrentModificationException` even with its javac
server disabled and a bounded parallel make graph. Do not treat a lower job
count as serialization; use one job until the boot compiler is replaced or the
