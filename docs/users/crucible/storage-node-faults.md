...
| `memory.service` | memory range; `fetch`, `load`, `store`, `dma_read`, `dma_write`, `page_table_walk`, `queue`; `persistent` or `state_machine`; `composite` | `qemu.memory.service.v2` | exact picosecond latency, optional byte/operation rates, sharing scope |
