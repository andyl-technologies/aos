...
| WS-06 | [workstream-06-nix-cache.md](./workstream-06-nix-cache.md) | origin-side `nix-cache-info`/`<storehash>.narinfo`/`nar/` emitter, `References` basename expansion, per-narinfo Ed25519 `Sig:` (Nix-fingerprint over StorePath/NarHash/NarSize/References), NAR blob URL key scheme, origin-as-substituter — reuses the WS-04 signing key |
...
| G28 | no origin nix-cache emitter → **`nix-cache-info`/`<storehash>.narinfo`/`nar/` superset** (`References` basename expansion, per-narinfo Ed25519 `Sig:` over the Nix fingerprint, NAR blob URL key, origin-as-substituter) — **narinfo/nix-cache features now COVERED** | §13,§11 | WS-06 |
...
- **WS-06 lands last and is orthogonal** to the git trust chain: the origin-side
  `nix-cache-info`/narinfo/`nar/` emitter reuses WS-04's one Ed25519 key for the
  per-narinfo `Sig:`, so signing must exist first, but the NAR surface is
