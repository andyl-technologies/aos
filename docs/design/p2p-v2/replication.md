...
Only the `reserved_bytes` field is needed. The replication factor comes from
...
toward `reserved_bytes`. Pinned objects do not count — they are already
...
`aos:store` with a short TTL (1 minute). This record is periodically
refreshed. Non-replicator nodes can query this key to discover how many
replicators are active in the network.

When a replicator's pool is full (`free_reserved = 0`), it stops refreshing
...
5. **Download.** Fetch the manifest and chunks via the normal store transfer
   protocol (`/aos/store/manifest/1.0.0`, `/aos/store/chunk/1.0.0`). The
   manifest's `references` and `closure` hints enable parallel fetching of the
   full closure.
...
7. **Success.** Once the object is fully local, publish `ReplicateSuccess`:
   ...
   The replicator also calls `start_providing` on `aos:store:{store_hash}` so
...
`aos:store:{store_hash}`. If the total provider count (including non-replicator
...
- **Pinned objects** (in an active FUSE view or in `roots_db`): silently
...
- **Replicated objects** (in the replication pool, unpinned): removed from
  ...
  `aos:store:{store_hash}` provider record, letting it expire.
...
Provider record TTLs for `aos:store:{store_hash}` are tiered:

| Object State | TTL | Rationale |
|---|---|---|
| Pinned (FUSE view or roots_db) | Cluster-config interval (e.g., 1 day) | Stable, long-lived. |
