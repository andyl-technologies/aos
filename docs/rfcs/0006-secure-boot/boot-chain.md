...
- `modules/security/verity.nix` (dm-verity, default off) protects rootfs
  *block integrity* — complementary to SB's *boot-binary authenticity*. A
  fully locked appliance enables verity on the read-only root with the
  root hash baked into the (signed) UKI cmdline (`pkgs/boot/aos-uki.nix`
  `rootHashFile`), so the signature chain extends to every rootfs block.
  **Shipped in RFC-0011** (F1) — the EROFS root gets a Merkle tree and the
  signed cmdline carries its root hash; not required for SB enforcement, but
  on by default under measured boot. This same verity-on-root + signed-roothash
  is the precondition for RFC-0012's password-less emergency/recovery shell
