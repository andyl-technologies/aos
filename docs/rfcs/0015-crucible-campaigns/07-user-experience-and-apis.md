...
run, watched, steered, paused, forked, inspected, reproduced, and replicated.
...
[[stage]]
id = "first-disruption"
fork_at = { choice_class = "uplink-disruption" }
...
crucible campaign fork NAME --at SNAPSHOT|CONFIG --name NEW_NAME
```
...
configurations       184211    ready expansions       913
open expansions        7842    attempts running        32
...
`choices` shows domain, admitted values, visit counts, reward/metric summaries,
interval partitions, prior/proposal distributions, and why widening is ready or
waiting. `explain` follows causal links from policy and observation basis through
proposal, selection, execution evidence, and reward credit.
...
| `fork` | Issue an operator proposal at a declared choice point |
...
- **[CAPI-7]** These commands MUST call the same campaign/execution primitives
  rather than maintain separate search, fuzz, or fork state models.
...
- expansion nodes showing untried/admitted values and widening state;
