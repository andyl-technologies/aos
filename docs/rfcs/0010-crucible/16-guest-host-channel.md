...
RFC-0020 §02.7 additionally assigns the same synchronous trap transport a
...
- **[GHC-22]** The doorbell `kind` field MUST enumerate exactly the following
  ...
  - **Measurement messages**, the `measurement_begin`, `metric_sample`, and
    `measurement_end` messages of RFC-0020 §08.4. Each carries a canonical
  ...
  - **Semantic markers**, carrying a canonical marker ID, exact instance key,
    ...
    instance-bearing boundary events required by RFC-0020 §08.3-08.4 without
...
- [x] **T-GHC-3** Implement the readiness heuristic (fixed icount /
  ...
  `ReadyPoint` material and the fixed 1000-tick-per-nanosecond scale for
  conversion, treats same-tick network activity as not yet idle, canonicalizes same-time console
