...
Checklist sync digest: `rfc0010-checklist-v1:65f2cd363e4d1e9a`
...
- CLI (incl. the triage + debug subcommands `T-CLI-17, T-CLI-18`): `T-CLI-1 … T-CLI-21` ([`23`](23-cli.md)).
  ...
  and rejects directories and noncurrent or malformed ledger schemas before triage
  begins.
...
- Time-travel debugging (built on the checkpoint DAG + replay): `T-DBG-1 … T-DBG-12` ([`36`](36-time-travel-debugging.md)).
  ...
  `T-DBG-7`, `T-DBG-8`, and `T-CLI-18` require executable coverage through the
  current authenticated daemon Session owner.
  Controller-leased
...
- Failure triage: `T-TRI-1` is green through `checks.crucible.phase6.failureSignature`,
  ...
  discovery-signature evidence, and rejects directories and noncurrent or
...
fault system passes its per-kind, live-boundary, checkpoint, replay, search,
  documentation, and current-surface proofs). When this is green and the
