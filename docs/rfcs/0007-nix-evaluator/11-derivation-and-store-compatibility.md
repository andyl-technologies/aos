...
- [x] Current `.drv` surface leak canary for internal cache hashes: `internal_cache_hash_canaries_do_not_reach_drv_surfaces` evaluates a static derivation, computes the actual current parse-cache BLAKE3 key for that source plus the evaluator-local xxh3 structural hash for the derivation name string, and asserts the recorded `.drv` ATerm bytes and `.drv` store path do not contain those internal digest renderings or raw digest bytes. This is a selected current-substrate regression canary, not the full type-enforced leak-invariant API or harness ([§9](#9-hashing-policy-three-hashes-three-jobs)) — P1/P2 precursor, `S-15`; gate: focused derivation canary test.
