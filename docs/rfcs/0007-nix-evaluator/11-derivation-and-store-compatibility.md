...
- [x] Current hash-builtin cache-surface canary: `configured_import_cache_preserves_hash_builtin_surface` evaluates `builtins.hashString "sha256" (import file)` with import caching disabled, with configured parse/persist roots on a miss/write path, and with a later persistent-hit path, requires identical SHA-256 hash-string output across all three runs, and scans that Nix-observed hash output for selected internal parse/import/file-content BLAKE3 and hot xxh3 canaries. This is one hash-builtin sample, not full hash/fetch builtin leak-invariant coverage ([§9](#9-hashing-policy-three-hashes-three-jobs)) — P1/P2 precursor, `S-15`; gate: focused hash-builtin cache-surface canary test.
