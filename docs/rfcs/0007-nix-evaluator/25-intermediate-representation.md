...
- [x] Current owned parse-cache artifact decode path: cache entries read versioned little-endian `resolved.bin`/`ir.bin`/`symbols.bin` streams through `cache::parse::{format,codec}` into owned `ResolvedAst`/`Ir` Rust structures, then run structural validation; no borrowed mapped views are claimed (§7.1) — **P1/P2**, `S-19`/`S-11`; gate: parse-cache artifact roundtrip/validation tests.
