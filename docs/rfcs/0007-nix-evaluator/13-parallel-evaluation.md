...
- [x] Current same-root persistent artifact-mapping writer lock precursor:
      independently opened `PersistCache` handles in one process now serialize
      file-artifact and parse-artifact mapping appends plus compaction through
      process-local mutexes keyed by the canonical cache root, so concurrent
      same-root appends preserve complete mapping records and poisoned live
      mapping locks fail before writing the sidecars. This is same-process
      fixed-record coordination only, not cross-process locking/CAS, the final
      LMDB/redb index tables, work-stealing integration, or loom/Miri audit
      (§4.3, [12](12-incremental-evaluation-cache.md) §6.5) — **P3.5**
      precursor, `R-4`/`R-10`.
