...
- [x] Current Chase-Lev-backed `.drv` surface differential precursor:
      ...
      materialization, deep `.drvPath` root contexts, single-output `outPath`
      root contexts, non-string roots with forced derivation materialization but
      empty root context, empty roots, empty worker-count rejection, oversized
      ...
      does not run a full closure, force every lazy derivation attrset, force
      ...
      satisfy the full parallel evaluator parity gate (§4.4) — **P3.5**
      precursor, `C-12`/`S-13`.
...
- [x] Current loom CAS/waiter model precursor:
      ...
      waiter registration. Assertions pin no stranded waiter registration, no
      double body execution, no invalid/torn state-word decode, self-cycle
      progress, and acquire/release visibility of the pre-publish success or
      failure payload. This is still a model precursor: the full unbounded
