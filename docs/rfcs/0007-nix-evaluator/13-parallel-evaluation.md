...
- [x] Current Chase-Lev-backed `.drv` surface differential precursor:
      ...
      ATerm byte stream materialized during root evaluation plus the root string
      context when the root itself evaluates to a string, collates the fragments
      with `parallel_output`, and compares the path-sorted content-only SHA-256
      collation to the serial baseline. It preflights
      ...
      derivation-surface failures with stable task indexes. The standard
      `.drv` convenience harness runs the RFC worker-count matrix `{1, 2, 8,
      N}` where `N` is host available parallelism (deduplicated with a fallback
...
- [x] Current loom CAS/waiter model precursor:
      ...
      single-owner CAS race, and exhaustive owner-plus-two-waiter three-worker
      models cover success replay and failed-payload replay after an observed
      waiter registration; bounded combined three-racing-worker force models
      now cover success and failure paths where the owner and waiter replay
      outcomes are selected by the interleaving. Assertions pin no stranded
      waiter registration, no double body execution, no invalid/torn state-word
      decode, self-cycle progress, and acquire/release visibility of the
      pre-publish success or failure payload. This is still a model precursor:
      the full unbounded three-racing-worker combined claim/wait/replay state
...
- [x] Current parallel tree-walk Tier-A worker-storage bridge:
      scheduler-backed raw and `.drv` tree-walk workers now clone the caller's
      options through a single worker-options helper that installs both the
...
- [x] Current tree-walk parallel payload failed-replay precursor:
      ...
      before entering the serial force path, then sends misses through
      `TreeWalkParallelThunkCell::force_or_wait_with`. The adapter owns sidecar
      claim/wait and terminal publication, so only the claim winner runs the
      existing serial `ThunkCell` body and publishes either `Ok(Value)` or the
      cloned `TreeWalkError` before returning. Terminal success replays still
      perform lazy-identity cleanup and cache-hit accounting, while owner body
      execution returns the serial result directly. Tests cover pre-published
      failed sidecar replay, same-worker claimed-sidecar self-cycle handling,
      serial division-by-zero publication and replay, and suspended serial
      `ThunkCell` preservation after failed force. This is still a default-off
      payload replay precursor only: the serial `ThunkCell` remains the
      body/state owner after sidecar admission, the live scheduler wait-or-steal
      force path does not yet execute evaluator thunk bodies, and the scheduler
      park-token, lock-free waiter-list, GC-poll cancellation, and loom/Miri/TSan
      gates remain open (§6).
