...
- [x] Frontend lowering of static `builtins.<name>` saturated calls to direct IR boundaries. `IrLowerer` recognizes unshadowed top-level builtins and `builtins.<name>` selects, consults the registry's `BuiltinDirect` metadata, and lowers saturated unary/binary/ternary/direct-only forms to `IrKind::PrimOp` or `IrKind::DerivationStrict`; dynamic builtin scope, select defaults, shadowed `builtins`, and unsaturated first-class selects stay dynamic/first-class instead of pretending to be direct calls ([§4.3](#43-why-this-is-more-than-a-micro-optimization-here)) — P1.
