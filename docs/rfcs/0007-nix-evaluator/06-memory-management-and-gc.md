...
- [x] Current boundary record-owned heap-field writeback applicator:
      ...
      caches on mutated records. Tests cover copied list/attr/primop-argument
      writes, same-object copied-field staging, mixed copied/direct same-record
      staging, malformed copied and cross-branch request sets, direct old
      list/attr/primop-argument writes, stale direct-field rejection without
      mutation, permanent-field rejection, strict-path direct old-to-young
      rejection, barrier-aware direct old-to-young publication, attr symbol-slot
      stale metadata rejection, and outcome-level direct-write routing.
      Permanent shared in-place field mutation, captured environment fields,
      thunk fields, synthetic destination allocation, ABI object headers,
