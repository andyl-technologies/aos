...
| C-22 | Flakes | **Out of scope.** aos-nix targets non-flake evaluation of the AOS package set and `systems/`; flake builtins are stubbed `Unsupported` → transparent `NixCli` fallback. A future workstream if ever needed. | [23](23-scope-platform-and-modes.md) |
