...
- [x] Current force-time inline impure revalidation substrate: trace-backed
  ...
  returning a scalar, context-free string, or context-free path payload for tree-walk
  ...
  scalar/context-free string/context-free path effectful reuse only: source-less raw eval,
  ...
  retention, context-bearing strings/paths, lists/attrs and other composite
...
- [x] Current inline scalar/context-free string/context-free path value-hash substrate: `ValueHash::from_inline_value` hashes validated inline WHNF `int`/`bool`/`null`/`float` payloads in the durable BLAKE3 domain `aos-nix-inline-value-hash-v1`; floats are hashed by raw IEEE bits, so this may over-propagate relative to future Nix numeric canonicalization but cannot cut off distinct bit patterns. `ValueHash` also hashes context-free string bytes and context-free path bytes in separate durable BLAKE3 domains for the force-cache payload precursor. Context-bearing strings/paths, lists/attrs canonical serialization, functions/thunks cacheability policy, generic hash-cons value fields, `force_memoized` integration, persistence, and harness proof remain open ([§4.1](#41-the-mechanism), [§5](#5-hashing-policy)) — P2 precursor, `S-14`/`S-15`; gate: `cache::cutoff` tests.
