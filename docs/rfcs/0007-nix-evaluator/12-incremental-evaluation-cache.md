...
- [x] Current metadata-validated bundle hydration writer: `ParseCacheEntry::write_artifact_bundle_validated` decodes bundled metadata and checks `schema_version` before creating or overwriting entry files, then delegates successful writes to the existing metadata-last bundle writer. This is metadata validation only: no artifact count cross-check, keyed hydration enforcement, durable index lookup, cache-hit integration, or harness proof ([§6.1](#61-the-persistent-value-store), [§6.5](#65-storage-engine-two-engines-for-two-data-natures)) — P2 precursor, `C-13`; gate: `cache::parse` tests.
