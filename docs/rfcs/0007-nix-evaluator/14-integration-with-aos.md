...
- [ ] `name()` logged per top-level instantiation (and on fallback) via `tracing`; counters for native successes, fallbacks-by-variant, shadow divergences, verify-sample mismatches mirroring `NIX_SHOW_STATS`; self-contained divergence reports (file+attr, both `.drv` paths, ATerm byte diff); `AOS_NIX_NATIVE=0` as the one-line kill switch (§11): current seam instrumentation logs the evaluator name for top-level `eval_expr`/`instantiate`, records the native/fallback evaluator pair on fallback warnings, keeps the native gate out of Nix subprocesses, exposes grouped native success/fallback/shadow/verify counters, and attaches operation plus file/attr context to shadow/verify `.drv` divergence events when available. Remaining before completion: promote those events into the full self-contained report surface and wire the later subsystem counters as they land — **P1** seam instrumentation, `S-16` ([24](24-observability-and-diagnostics.md), [15](15-differential-testing-and-benchmarking.md) §4).
