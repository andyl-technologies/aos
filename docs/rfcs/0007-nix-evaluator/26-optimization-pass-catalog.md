...
- [~] 2.1 Inlining / beta-reduction — the keystone, re-thunking substituted args to preserve call-by-need (§2.1) — **P4**, committed `C-21`; size threshold / used-once policy `M-24`. First cut landed as `ir::InlineSingleUse` (registered behind `AOS_NIX_SIMPLIFY`): the arena-stable, same-frame, single-use `let`-inline — a binding proven `Cardinality::Once` whose one use is a same-frame `Local` is replaced in place by a copy of the binding value node (thunk or literal), leaving the binding and **frame slot layout unchanged** (elision without compaction, so JIT frame assumptions cannot shift). Byte-parity green default + engaged, JIT and serial (engaged-JIT clean — no deopt-metadata interaction); nix-bench A/B (zlib/openssl) neutral (the per-parse cardinality refresh offsets the marginal thunk-relocation win), so it stays **off by default**. Full beta-reduction of lambda applications — needing compacting rebuild/remap + de-Bruijn renumbering — is increment 8, sequenced after snapshot-design's S4b JIT-ABI work (design note §9).
...
- [x] 2.3 Case-of-known — `select`/`if`/has-attr on a statically-known value, no dynamic keys (§2.3) — **P4**, `C-21`. First cut landed as `ir::CaseOfKnown` (registered behind `AOS_NIX_SIMPLIFY`): folds `if` on a `Bool`-literal condition to its taken branch via `set_node`, inheriting the taken branch's effect and dropping the untaken branch (sound even when effectful/divergent). It composes with `ConstFold` (a folded comparison exposes a known `if`). Byte-parity green default + engaged, JIT and serial; nix-bench A/B (zlib/openssl) neutral (literal conditionals are rare in nixpkgs), so it stays **off by default**. The `Select`/`HasAttr`-on-known-`AttrSet` cases (dynamic keys, `rec`/`__overrides`, missing-attr error observability, inline-cache site accounting) remain open and mostly fire only after inlining.
- [x] 2.4 Dead-binding elimination — cardinality `Absent`, `Pure` bindings only (§2.4) — **P4**, `C-21`. Value-elision cut landed as `ir::DeadBindingElim` (registered behind `AOS_NIX_SIMPLIFY`): a binding admitted by the vetted `dead_binding_elimination_plan` (`Absent`, not proven strict, static key, omittable value edges — the exact set the tree-walk already skips) has its value replaced by `null` via `set_node`; the binding slot is retained, so **frame layout is unchanged** (elision without compaction). Sound even for effectful/divergent values (never demanded ⇒ never forced). Byte-parity green default + engaged (JIT + serial); nix-bench A/B (zlib/openssl) neutral, so it stays **off by default**. Frame-slot compaction rides with full beta-reduction (increment 8, design note §9).
