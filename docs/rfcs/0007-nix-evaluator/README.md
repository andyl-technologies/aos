...
| [31](31-substrate-optimizations.md) | Substrate optimizations below the language-level stack: evaluator heap-image snapshots (V8/CDS-style), zygote fork + CoW variant eval + shm heap sharing, a threaded-code baseline tier for the sub-promotion-threshold tail, a VFS snapshot layer for path primops, vectorized + merkle-incremental hashing, mmap'd store-validity filters, PGO/BOLT/allocator tuning, and trace-driven prewarming — each with position in the design, evidence, prerequisites, and gates |
