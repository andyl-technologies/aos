...
- [ ] Root-vs-contaminated bisection: topologically order the divergent set, report nodes with no divergent input as the roots, collapse the contaminated rest; per-root self-contained reproduction (§2.4): `DrvDiffReport` classifies root versus contaminated divergence nodes, `aos nix-diff` renders both classes, and human/JSON reports now carry the Nix file, attr, mode, eval policy/system/allowlist flags, and copyable reproduction command for the failing comparison. Remaining before completion: package the full per-root report with the exact root pair and byte/structural diff context needed to hand a single root divergence to a developer — **P1**, `C-18`.
