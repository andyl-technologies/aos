...
- [ ] Root-vs-contaminated bisection: topologically order the divergent set, report nodes with no divergent input as the roots, collapse the contaminated rest; per-root self-contained reproduction (§2.4): `DrvDiffReport` classifies root versus contaminated divergence nodes, `aos nix-diff` renders both classes, and human/JSON reports now carry the Nix file, attr, mode, eval policy/system/allowlist flags, copyable reproduction command, exact root oracle/candidate pair, and pair-local byte/structural/input-output diff context for each root divergence. Remaining before completion: include the minimal input set or direct node-level rerun artifact described in §2.4 so a developer can rerun a single root node without re-evaluating the full closure — **P1**, `C-18`.
