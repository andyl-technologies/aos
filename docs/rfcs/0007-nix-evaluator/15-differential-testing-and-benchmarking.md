...
- [x] aos-nix mirrored counters named to parallel `NIX_SHOW_STATS` (`thunks_forced`/`allocated`/`elided`, `inline_cache_hits`/`misses`, `shape_transitions`, `gc_bytes`/`gc_pause_us`, `tier_promotions`, `deopts`, `cache_hits`/`early_cutoffs`, `force_cache_hits`/`misses`/`probes`, `force_cache_memoization_admits`/`bypasses`/`demands`), surfaced through `tracing` (§4.2): `EvalStats` is returned on every owned tree-walk `EvalOutcome`, `TreeWalk::stats()` exposes live snapshots, successful public evaluation paths emit the same field names through the `aos_nix::eval::stats` tracing target, and the current P1 oracle increments thunk allocation/force/reuse counters, force-cache hit/miss/probe counters, force-cache memoization policy decision counters, parse-cache and `findFile` cache hit/miss counters, and heap chunk/reserved/used bytes from the bump arena. Later subsystem counters remain schema-stable zeroes until their subsystem lands (`early_cutoffs` **P2**, shape counters **P5**, tier counters **P6/P7**), so downstream diagnostics can diff the same field set from day one; `early_cutoffs` is the direct instrument for criterion **C4** ([24](24-observability-and-diagnostics.md) §7). Covered by `eval_outcome_reports_mirrored_stats`, `eval_stats_are_emitted_through_tracing`, and the full `aos-nix` crate test/source-size gates.
...
- [x] The benchmark corpus cold **P1** slice: full system-variant toplevel eval, toolchain-closure eval, leaf-package spread, and diagnostic microbenchmarks (§5.2). The default `aos nix-bench` corpus now discovers every `systems.<name>.build.toplevel`, overlays the same explicit bootstrap/Rust/JDK/Bazel/LLVM toolchain roots used by `aos nix-diff --all`, dynamically expands `stdenv.toolchainTiers.<tier>.<component>` derivation roots, filters leaf-package spread attrs down to attrs that exist and are derivations, and writes a deterministic ignored diagnostic corpus covering attrset access, `map`/`genList`, and deep recursion. Every recorded benchmark carries `category`, `temperature = "cold"`, file/attr, per-record eval context fingerprint, samples, summaries, and raw `NIX_SHOW_STATS`; explicit `--attr` remains an override for focused runs.
- [ ] The benchmark corpus warm split: the **cold-vs-warm split** of each workload that isolates the G2 incremental-cache win — the most-watched number — becomes meaningful once the cache exists (**P2**) and must add stable warm-cache setup/labels before this row can be checked.
