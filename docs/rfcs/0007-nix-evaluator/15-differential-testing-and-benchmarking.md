...
normalization), applies the pinned digit-normalizing `.postprocess` shape
without executing fixture shell, supports raw `eval-okay` output, XML
`eval-okay` output, and parse-mode resolution checks. The local fixture corpus
...
- [x] Parity fuzzer **P1 scaffold** (aos-nix whole vs `nix-instantiate`): structure-aware `Arbitrary`/grammar-based valid-AST generation, coverage-guided under `cargo-fuzz`/libFuzzer, source-seed passthrough for the §2.7 package/conformance corpus, and libFuzzer reduction for failures (§7.2). The scaffold lives in `fuzz/`: `parity_json` decodes either source-seed inputs (`# aos-nix-fuzz-source`) or generated valid JSON-producing ASTs through `arbitrary`, evaluates them with `NixNative::eval_expr`, and compares to pinned C++ `nix-instantiate --eval --strict --json` when `AOS_NIX_ORACLE` is configured. The checked-in seed corpus covers JSON attr/list output, attr update, reflected string context, and `derivationStrict`; `fuzz/Cargo.lock` pins the fuzz-only dependency set. Automatic population from the full §2.7 corpus remains coupled to the auto-derived corpus row, and full fuzz-budget quiescence remains the separate acceptance criterion above (§8.1) — exercised through every later phase, `R-13`.
