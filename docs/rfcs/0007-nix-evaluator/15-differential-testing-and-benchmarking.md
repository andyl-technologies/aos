...
- [x] The benchmark corpus warm split: the **cold-vs-warm split** of each workload that isolates the G2 incremental-cache win — the most-watched number — is now a first-class `aos nix-bench` record axis. The benchmark corpus expands every selected workload, including explicit `--attr` runs, into paired `category:cold:<attr>` and `category:warm:<attr>` specs; warm specs perform one unrecorded `nix-instantiate`/`NIX_SHOW_STATS` priming pass before capturing recorded samples, and history comparison keys already include the `temperature` field so cold and warm trends never cross-compare. This adds the stable warm-cache setup/labels required for P2 benchmarking; CI wiring and the broader cutover gate remain separate rows.
