...
- [x] Budgeted full generated eval-json corpus in the Linux check graph: `aos nix-diff --eval-json --time-budget <SECONDS>` bounds a corpus replay by wall clock (deterministic seed order, no in-flight abort, skipped counts reported in human/JSON output, divergences in the compared prefix still fail), `aos nix-fuzz-corpus --exclude <ATTR>` (exact attr or dot-prefix) skips eval-time-IFD attrs a sandboxed store cannot realize, and `checks.integration.aos-eval-json-corpus-full` regenerates the automatic package/toolchain/system corpus from the sandboxed repo source plus the pinned-`2.24.12` conformance seed set (unpacked from `pkgs.nix.src`) and replays it against a throwaway sandbox-local store under a 900 s budget; `checks.integration.aos-drv-parity-representative` runs the `.drv` byte gate over the fixed zlib/openssl/coreutils/bash witness set with the same pattern. Known native divergences surfaced by the first full run (hardening `concatStringsSep` thunk-force bug, `builtins.match` POSIX-ERE bracket-backslash semantics, `fromTOML` `5e22` float rendering) are excluded with `TODO(RFC-0007)` markers and tracked for burn-down in [22](22-implementation-checklist-all-phases.md) (P4 `--eval --json` rows); `C-4` stays open until they are fixed and the check is green in scheduled CI — **P1+**, `C-4` groundwork, `S-2`.
