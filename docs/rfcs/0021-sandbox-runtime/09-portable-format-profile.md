...
protocol code `u16be(1)`, protocol major `u16be(1)`, protocol minor `u16be(0)`,
...
Acquire, renew, and same-owner advance all require receipt protocol minor `0`.
Other protocol versions are non-canonical and fail closed. The claim's fixed V1
framing uses action code `3` with a mandatory exact prior generation/digest.
