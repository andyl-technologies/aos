...
| M-facts | Instance facts are an unsigned, unrecorded 4th input | Facts are a **first-class recorded input**: `facts_hash` (+ retained `facts.json`) in the manifest `inputs` and the `gen-attestation` record; "exactly one signed input" reworded to "one *operator-authored* input + a recorded instance-facts input." | trust-and-secrets.md, README.md, module-system.md |
| M-gen0key | Gen-0 SSH key seeded from unauthenticated IMDS pre-verification | **Removed** the carve-out. No `authorized_keys` is seeded from the facts channel before the stage-2 signature check; pre-eval reachability, if needed, comes only from an image-baked or operator-signed source. | provisioning.md |
| M-repart-order / locus | host.nix-derived repart runs unverified / can't reach first-boot repart | First-boot substrate runs **only** from image-baked `/usr/lib/repart.d` (idempotent, no operator input). Operator custom topologies are a **two-boot flow** (or out-of-band signed substrate spec), never applied unverified on first boot. | provisioning.md |
