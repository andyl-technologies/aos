...
- [ ] **`aos metadata` agent.** `detect` + `fetch` + `authorize` + `render-storage`; reuse `aos-net` + `security.rs` SSHSIG + `TrustStore`. Stash the exact provisioning bundle, accepted host.nix, typed plan, facts, and validation record in `/run/aos-metadata/`.
