...
| D12 | Evaluator | **AOS-packaged C++ Nix**, invoked as `nix-instantiate --store dummy:// --eval --strict --json --pure-eval` with exact authenticated paths admitted through `--aos-pure-eval-input`, `restrict-eval`, and `allow-import-from-derivation=false`, then bounded by a hardened systemd unit (`MemoryMax`/`TimeoutStartSec`). |
