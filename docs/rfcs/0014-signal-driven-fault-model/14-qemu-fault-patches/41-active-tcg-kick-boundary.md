# Capability task 0090 — Deterministic TCG kick boundary

## Purpose

Capability tasks 0088 and 0090 define the active-slice boundary with a
...
For a state-free kick in bounded sim mode, the atomic patch sets each RR
...
Capability task [`0106`](57-defer-active-slice-host-wakes.md) subsequently tightens this
...
The capability also publishes the process-local `rr_initial_wait_complete` flag
...
while all of capability task 0088's mode predicates hold. Immediate `cpu_exit()`
remains mandatory for
an admitted exact VMStop and committed stop, unplug, halted, stopped, or
...
provides wake liveness. The stateful stop, exact-VMStop, and interrupt
exceptions from capability task 0088 remain unchanged and still force the shared RR
thread out immediately.

## Files and license scope

The atomic patch modifies only MIT-licensed
...
4. Focused capability tests must prove that the state-free path publishes an atomic
...
5. Atomic-patch regeneration, ABI, license-boundary, inertness, and complete
