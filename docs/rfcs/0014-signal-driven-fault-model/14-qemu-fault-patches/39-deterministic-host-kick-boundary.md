# Capability tasks 0088/0090 — Deterministic host-kick boundary

## Purpose

Capability task `0087` removes the forced-RCU notifier as one source of host-timed
...
Capability tasks `0088` and `0090` prevent generic host latency hints from
...
plus an admitted native exact VMStop request, retain an immediate exit request
...
state, and the admitted exact VMStop is a deterministic observer action at a
...
The admission predicate deliberately excludes `rr_current_cpu` and positive
raw observed icount. The pointer is published before `cpu_can_run()` is
evaluated, so it also describes startup and other non-executing RR-loop
intervals. Raw observed icount is not updated until TCG returns, so it remains
zero for the entire first active execution interval. Wake liveness comes from
the condition broadcast, and the soft request is safe whether the RR thread is
executing a block or waiting.
...
exact VMStop is pending, or any
...
hot-unplug, exact VMStop, and interrupt wakeups thereby preserve their
...
exact-VMStop, and interrupt state still forces an immediate exit. QEMU may
...
scheduler progress. This capability therefore neither drops work nor turns
host timing into an architectural source of truth.

## Interaction with other scheduler exits

Deterministic preemption commands, exact fault boundaries, aggregate horizon
stops, guest exceptions, and guest-generated interrupts retain their existing
modeled exit paths. Capability task `0087` covers the forced-RCU notifier;
capability tasks `0088` and `0090` cover the generic host kick callback
registered by the RR accelerator. QEMU's virtual-time RR kick timer remains
disabled in sim mode by the related deterministic scheduler capabilities.
...
7. Rebuild the atomic patch and pass regeneration, ABI, license, inertness, and
...
- **[QFP-KICK-2]** Deferred host work MUST remain level triggered; an admitted
  native exact VMStop and committed control and interrupt state MUST
