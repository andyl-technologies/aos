# Capability task 0106 — Defer state-free host wakes in multi-vCPU bounded TCG

## Purpose

Capability task 0106 defines host-arrival handling with the active-slice boundary from capability task 0090. A soft
...
host-selected service point and never drops a wake. In single-vCPU mode, capability task
...
The exact VMStop request publishes its level-triggered pending state before
issuing an explicit vCPU kick. Admitted exact VMStop, committed stop, unplug,
...
The atomic patch modifies MIT-licensed `accel/tcg/tcg-accel-ops-rr.c` and
...
5. Atomic-patch regeneration, ABI, licensing, inertness, and corresponding-source
   gates pass.
