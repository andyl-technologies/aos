...
| `network.detected_frame_error` | opportunity; resolve | `kind = crc/fcs/framing/fec_uncorrectable`, `receiver_action` | severity `corrected < retry < drop < link_reset` | `network.detected-error.v1`; syndrome/class and action |
...
| `network.pause_backpressure` | persistent/state; queue | `class`, `pause_nanos` or resume event | state-machine | `network.backpressure.v1`; queue/service suspension ledger |
