...
monolithic VMState compatibility reader or restore path.
...
fingerprint hash. Version nine replaces the monolithic runtime VMState input
with each live target's exact checkpoint identity, ordered direct-plus-delta
...
matching record is rejected. Versions four through eight are rejected during
decode because they lack the complete version-nine descriptor set.
...
manifest ceiling. Earlier production closure versions are rejected during
