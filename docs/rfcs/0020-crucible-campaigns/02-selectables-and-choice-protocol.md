...
accepts only that grammar and current Checkpoint V4; noncurrent encodings fail
...
current-ABI VM-local, single-entry host-to-plugin ring. Its public envelope reuses
...
The current control protocol uses a three-descriptor `Setup` shape whose third
descriptor is `crucible.qemu-plugin.setup-plan` version 2. The descriptor is a regular memfd
...
setup-plan version 2; noncurrent encodings are rejected.
