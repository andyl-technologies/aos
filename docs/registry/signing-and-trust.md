...
| Any-active-key verification | `verify_commit_signature`/`verify_tag_signature` take `trusted_keys: &[String]`; empty set is an error. CLI tests call this production verifier directly, while the focused security and Hub interoperability tests generate stock-Git signatures to preserve the independent parser check. The Nix-only Hub test supplies its otherwise passwd-less builder identity through a repository-owned build-only preload fixture; production artifacts retain no OpenSSH or identity-shim runtime path. |
