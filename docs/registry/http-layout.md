...
| Semver, channels-as-branches, 16-partition rollout, frontier | [`versioning-and-channels.md`](versioning-and-channels.md) |
...
3. **The AOS layer is additive.** `/channel/<name>/0..f` and the thin
...
- `info/refs` is the advertised ref list, regenerated by `git update-server-info`
  ...
  and the release tags (`refs/tags/<semver>`). The 16 partition tag objects are
...
`/channel/<name>/0` … `/channel/<name>/f` are **16 signed git tag objects** served
...
- There must always be exactly **16** files (`0`–`f`); a consumer self-selects one
...
- **`HEAD` = the default channel** (e.g. `stable`); the 16 partition tag objects
...
channel rolling `1.1.0` out to 4/16 of the fleet (buckets `0`–`3` advanced,
`4`–`f` still on `1.0.0`):
...
/
  ...
  channel/
    stable/
      0  → tag(name=stable) → 1.1.0       (advanced)                        [low]
      1  → tag(name=stable) → 1.1.0       (advanced)                        [low]
      2  → tag(name=stable) → 1.1.0       (advanced)                        [low]
      3  → tag(name=stable) → 1.1.0       (advanced)                        [low]
      4  → tag(name=stable) → 1.0.0       (held back)                       [low]
      …                                                                     [low]
      f  → tag(name=stable) → 1.0.0       (held back)                       [low]
  release/
    1/0/0/objects/
...
- **An AOS host in bucket `7`** fetches `/channel/stable/7`, verifies it, sees it
  targets `1.0.0`, and stays on `1.0.0` — held back from the rollout.
- **An AOS host in bucket `1` already on `1.0.0`** fetches `/channel/stable/1`, sees
...
| `/channel/<name>/0..f` | 16 signed partition tag objects | yes | low |
...
| Rollout | (none in layout) | 16 signed `/channel/<name>/0..f` partition tags |
