...
| `apr publish <store-path> […]` | `publish` (`registry_ops.rs`) | Introspect the path, write `packages/<x>/<name>.toml`, compute + write `closures/<hash>`, upsert `ca/` trust-map entries for every runtime-closure member (refusing on a content mismatch unless `--bless`), then (unless `--no-commit`) commit the touched paths and refresh `objects/info/alternates` + `update-server-info`. |
| `apr ca bless/revoke/verify/backfill [--registry <name>] [--key \| --key-id]` | `run_ca` (`registry_ops.rs`) | Maintains the `ca/` trust map (RFC-0005): `bless` appends a realisation computed from the local Nix store (`--recursive` for the whole closure); `revoke` removes blessed content (a security event — signed, reviewable one-line diff); `verify` checks bucket health + closure coverage (`--deep` recomputes local NAR hashes); `backfill` maps every published closure so an existing registry becomes fully covered in one signed commit. |
