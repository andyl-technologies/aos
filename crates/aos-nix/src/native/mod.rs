...
impl NixNative {
    ...
    /// Returns [`NativeEvalError::Unsupported`] when the expression reaches an
    /// evaluator feature that is still outside the native tree-walk subset.
    /// Returns [`NativeEvalError::StaticDivergence`] when conservative
    /// pre-evaluation analysis proves a demanded recursive binding.
    ...
    pub fn eval_expr_with_stats(&self, expr: &str) -> Result<(String, EvalStats)> {
        crate::totality::reject_obvious_divergence_in_import_tree(expr, &self.options)?;
    ...
    fn tier1_engine_for(&self, options: &TreeWalkOptions) -> Option<Rc<dyn Tier1Engine>> {
        // RFC-0007 S4b: the tier-1 lowering emits the active carrier's value
        // ABI (two words on the baseline, one compressed word under the
        // `candidate_c_value` variant), so the engine runs on both carriers.
        // Compound shapes whose emitters still assume the two-word carrier
        // (arith trees, alloc-cons, tier-2 lambda bodies) decline at their
        // lowering entries under the variant and stay on the tree walk. See
        // design-notes/candidate-c-cutover-plan.md §6.1.
...
use error::*;
use eval_io::{json_string_from_outcome, native_source_file};
use fallback::*;
use source::*;
