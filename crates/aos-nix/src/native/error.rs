...
fn native_eval_error_impl(
    ...
    let kind = error.kind();
    if let Some(structured) = structured_config_error(&error, &kind) {
        return structured;
    }
    ...
    let resource = match kind {
        TreeWalkErrorKind::MaxCallDepthExceeded { .. } => {
            Some(crate::error::NativeResourceLimit::CallDepth)
        }
        TreeWalkErrorKind::MaxEvalStepsExceeded { .. } => {
            Some(crate::error::NativeResourceLimit::Steps)
        }
        TreeWalkErrorKind::MaxEvalDurationExceeded { .. } => {
            Some(crate::error::NativeResourceLimit::Time)
        }
        TreeWalkErrorKind::HeapMemoryBudgetExceeded { .. } => {
            Some(crate::error::NativeResourceLimit::HeapMemory)
        }
        _ => None,
    };
    if let Some(resource) = resource {
        return NativeEvalError::ResourceLimit {
            resource,
...
fn structured_config_error(
    ...
    match kind {
        TreeWalkErrorKind::MissingAttribute { .. } => {
            ...
            let expression = source.bytes().get(
                usize::try_from(error.span().start).ok()?
                    ..usize::try_from(error.span().end).ok()?,
            )?;
            let marker = expression
                .windows(b"config.".len())
                .position(|window| window == b"config.")?
                + b"config.".len();
            let root = expression[marker..]
                .iter()
                .copied()
            ...
            Some(NativeEvalError::MissingOptions {
                missing: vec![crate::NativeMissingOption {
                    path: String::from_utf8_lossy(&root).into_owned(),
                    kind: crate::NativeMissingOptionKind::AbsentRootRead,
                    source_path: Some(String::from_utf8_lossy(source.name()).into_owned()),
                }],
            })
        }
        TreeWalkErrorKind::Thrown { message, .. } => structured_throw_error(message),
        _ => None,
    }
}

fn structured_throw_error(message: &[u8]) -> Option<NativeEvalError> {
    #[derive(serde::Deserialize)]
    struct Envelope {
        #[serde(rename = "__aosEvalError")]
        error: ConfigError,
    }
    #[derive(serde::Deserialize)]
    #[serde(tag = "kind", rename_all = "snake_case")]
    enum ConfigError {
        UndeclaredWrite { missing: Vec<Missing> },
        UndefinedOption { path: String, file: Option<String> },
        Conflict { path: String, defs: Vec<Conflict> },
        Assertion { msg: String, file: Option<String> },
    }
    #[derive(serde::Deserialize)]
    struct Missing {
        path: String,
    ...
    Some(match envelope.error {
        ConfigError::UndeclaredWrite { missing } => NativeEvalError::MissingOptions {
            missing: missing
                .into_iter()
                .map(|missing| crate::NativeMissingOption {
                    path: missing.path,
                    kind: crate::NativeMissingOptionKind::UndeclaredWrite,
                    source_path: missing.file,
                })
                .collect(),
        },
        ConfigError::UndefinedOption { path, file } => NativeEvalError::UndefinedOption {
            path,
            source_path: file,
        },
        ConfigError::Conflict { path, defs } => NativeEvalError::Conflict {
            path,
            defs: defs
                .into_iter()
                .map(|definition| crate::NativeConflictDef {
                    value: definition.value,
                    source_path: definition.file,
                })
                .collect(),
        },
        ConfigError::Assertion { msg, file } => NativeEvalError::Assertion {
            message: msg,
            source_path: file,
        },
    })
...
mod tests {
    ...
    #[test]
    fn structured_undeclared_writes_are_typed() {
        let error = structured_throw_error(
            br#"{"__aosEvalError":{"kind":"undeclared_write","missing":[{"path":"firewall.port","file":"/config/module.nix"}]}}"#,
        )
        .expect("structured module error should decode");
        let NativeEvalError::MissingOptions { missing } = error else {
            panic!("expected missing options");
        };
        assert_eq!(missing.len(), 1);
        assert_eq!(missing[0].path, "firewall.port");
        assert_eq!(
            missing[0].kind,
            crate::NativeMissingOptionKind::UndeclaredWrite
        );
        assert_eq!(
            missing[0].source_path.as_deref(),
            Some("/config/module.nix")
        );
    }

    #[test]
    fn human_diagnostics_are_not_treated_as_structured_errors() {
        assert!(structured_throw_error(b"The following option(s) are not declared").is_none());
    }

    #[test]
    fn structured_conflicts_and_assertions_are_typed() {
        let conflict = structured_throw_error(
            br#"{"__aosEvalError":{"kind":"conflict","path":"aos.security.firewall.forwardPolicy","defs":[{"value":"\"accept\"","file":"/one.nix"},{"value":"\"drop\"","file":"/two.nix"}]}}"#,
        )
        .expect("structured conflict should decode");
        let NativeEvalError::Conflict { path, defs } = conflict else {
            panic!("expected conflict");
        };
        assert_eq!(path, "aos.security.firewall.forwardPolicy");
        assert_eq!(defs.len(), 2);
        assert_eq!(defs[0].source_path.as_deref(), Some("/one.nix"));

        let assertion = structured_throw_error(
            br#"{"__aosEvalError":{"kind":"assertion","msg":"must be true","file":null}}"#,
        )
        .expect("structured assertion should decode");
        assert!(matches!(
            assertion,
            NativeEvalError::Assertion { ref message, source_path: None }
                if message == "must be true"
        ));
    }
