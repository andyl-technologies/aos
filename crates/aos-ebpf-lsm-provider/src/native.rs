...
impl Executable {
    ...
    fn validate(&self, immutable_root: &Path) -> Result<()> {
        ...
        let metadata = fs::metadata(path).context("inspecting BPF-LSM loader")?;
        ensure!(metadata.is_file(), "BPF-LSM loader is not a regular file");
        ensure!(
            metadata.permissions().mode() & 0o111 != 0,
            "BPF-LSM loader is not executable"
        );
        Ok(())
    }

    pub(super) fn run(&self, arguments: &[&str], remaining_millis: u64) -> Result<Output> {
        ensure!(remaining_millis > 0, "BPF-LSM loader deadline expired");
        let mut child = Command::new(self.path())
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("starting exact BPF-LSM loader")?;
        let stdout = child
            .stdout
            .take()
        ...
        let deadline = Instant::now()
            ...
            .context("BPF-LSM loader deadline overflow")?;
        let status = loop {
            if let Some(status) = child.try_wait().context("waiting for BPF-LSM loader")? {
                break status;
            }
            if Instant::now() >= deadline {
                child.kill().context("terminating expired BPF-LSM loader")?;
                let _ = child.wait();
                let _ = join_reader(stdout_reader, "stdout");
                let _ = join_reader(stderr_reader, "stderr");
                bail!("BPF-LSM loader deadline expired");
            }
...
fn read_bounded(reader: impl std::io::Read) -> Result<Vec<u8>> {
    ...
    reader
        ...
        "BPF-LSM loader output exceeds its bound"
    );
    Ok(bytes)
}
...
fn join_reader(reader: thread::JoinHandle<Result<Vec<u8>>>, stream: &str) -> Result<Vec<u8>> {
    reader
        .join()
        .map_err(|_| anyhow::anyhow!("BPF-LSM loader {stream} reader panicked"))?
        .with_context(|| format!("reading BPF-LSM loader {stream}"))
}

pub(super) fn path_text(path: &Path) -> Result<&str> {
    path.to_str().context("BPF-LSM path is not UTF-8")
}
