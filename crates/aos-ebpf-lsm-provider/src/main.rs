//! Process entry point for the package-owned BPF-LSM policy provider.

use std::io::{self, Read as _, Write as _};

use anyhow::{Context as _, Result, bail};
use aos_ability_model::ABILITY_LIMITS_V1;
use aos_ebpf_lsm_provider::EbpfLsmProvider;
use aos_provider_protocol::{HANDLER_ABI_ARGUMENT, MAX_HANDLER_RESULT_BYTES};

fn main() {
    if let Err(error) = run() {
        eprintln!("aos-ebpf-lsm-provider: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    if arguments.len() != 2 || arguments[0] != HANDLER_ABI_ARGUMENT {
        bail!(
            "usage: aos-ebpf-lsm-provider {HANDLER_ABI_ARGUMENT} \
    ...
    let output = EbpfLsmProvider::production().handle(&arguments[1], &input)?;
