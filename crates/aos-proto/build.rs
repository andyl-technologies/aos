...
fn verify_sandbox_local_compatibility() -> Result<(), Box<dyn std::error::Error>> {
    ...
    const EXPECTED_SANDBOX_LOCAL_V1_FINGERPRINT: u64 = 0xc5d7_02e4_aa8e_dfff;
    let actual = complete_schema_fingerprint(source);
    ...
    verify_scoped_declarations(
        ...
        &[
            ...
            "bytes original_session_binding = 3;",
            "bytes original_signed_request_digest = 4;",
            "bytes archive_head = 5;",
            "bytes signed_terminal_outcome = 6;",
            "HostNoApplySettlementPhaseV2 phase = 7;",
            "bytes canonical_controller_coordinate = 8;",
            "bytes challenge = 9;",
        ],
    )?;
    verify_scoped_declarations(
        ...
        &[
            ...
            "bytes preliminary = 2;",
            "bytes floor_sealed = 3;",
            "bytes ack_retained = 4;",
        ],
    )?;
    verify_scoped_declarations(
        &source_declarations,
    ...
    verify_scoped_declarations(
        ...
        &["RequestHeader header = 1;", "bytes canonical_query = 2;"],
    )?;
    verify_scoped_declarations(
        &source_declarations,
        "message StorageExecutionCaptureCandidateV1 {",
        &["bytes canonical_candidate = 1;"],
    )?;
    verify_scoped_declarations(
        &source_declarations,
