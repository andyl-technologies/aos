...
mod tests {
    ...
    fn dropping_an_arena_returns_live_bytes_to_the_pre_arena_level() {
        let status = Command::new(std::env::current_exe().expect("test binary path resolves"))
            .args([
                "--exact",
                "heap::gauges::tests::dropping_an_arena_returns_live_bytes_to_the_pre_arena_level_child",
                "--ignored",
            ])
            .status()
            .expect("isolated gauge child starts");
        assert!(status.success(), "isolated gauge child must pass");
    }

    #[test]
    #[ignore = "subprocess target for process-global gauge isolation"]
    fn dropping_an_arena_returns_live_bytes_to_the_pre_arena_level_child() {
        ...
        assert_eq!(live_with_arena, before.live_mapped_bytes + CHUNK as u64);

        drop(arena);
        let live_after = ArenaProcessGauges::snapshot().live_mapped_bytes;
        assert_eq!(
            live_after, before.live_mapped_bytes,
