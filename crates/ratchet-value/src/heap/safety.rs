...
mod tests {
    ...
    fn reviewed_heap_unsafe_lines_keep_safety_comments_and_counts() {
        ...
        // `FlatObjectPayloadAccess`.
        // flat.rs count 6 -> 10 (packed-retirement preparation): one header
        // read validates an exact live registry entry before retirement, and
        // three allocation-free commit blocks retire one exact slot, all live
        // slots, or a validated selected subset. Each wipes the header and
        // destroys the payload after an exclusive validation token or exact-
        // slot proof. Tombstoning precedes destruction so unwinding cannot
        // double-drop; all four remain under `FlatObjectPayloadAccess`.
