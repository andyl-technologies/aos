...
impl<T> FlatObjectStore<T> {
    ...
    pub fn resolve(
        ...
        // are immutable after construction except the `Cell` epoch word.
...
impl<T> Drop for FlatObjectStore<T> {
    fn drop(&mut self) {
        for entry in &self.entries {
            // SAFETY: `entry.ptr` was placement-written by `alloc` as a
