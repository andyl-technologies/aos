...
impl FakeUnit {
    ...
    fn need_daemon_reload(&self) -> bool {
        self.needs_reload.load(std::sync::atomic::Ordering::SeqCst)
    }
}
...
#[ignore = "requires the hermetic AOS D-Bus broker"]
async fn stale_loaded_unit_is_rejected_before_start() -> Result<(), Box<dyn std::error::Error>> {
    let address = std::env::var("AOS_TEST_DBUS_ADDRESS")?;
    let (fake, _) = fake_systemd();
    let needs_reload = Arc::new(std::sync::atomic::AtomicBool::new(true));
    let manager = zbus::connection::Builder::address(address.as_str())?
        ...
        .build()
        .await?;
    manager.request_name(SYSTEMD_NAME).await?;

    let client_connection = zbus::connection::Builder::address(address.as_str())?
        .build()
        .await?;
    let pinned = PinnedSystemdManager::from_connection(client_connection).await?;
    let identity = pinned.unit_identity("example.service").await?;

    assert!(
        pinned
            .needs_daemon_reload_exact("example.service", &identity)
            .await?
    );
    assert!(matches!(
        pinned
            .start_unit_exact_current("example.service", &identity)
            .await,
        Err(Error::UnitNeedsDaemonReload { .. })
    ));

    needs_reload.store(false, std::sync::atomic::Ordering::SeqCst);
    assert!(
        !pinned
            .needs_daemon_reload_exact("example.service", &identity)
            .await?
