...
//!
//! The shared [`RpcService`](aos_hub_core::service::RpcService) serializes a
//! registry's mutable-pointer flips through the [`PublishLease`] port
//! ([`aos_hub_core::lease`]). The native hub holds the lease in process
...
//! is created by the operator CLI's schema migration (`aos-hub init
...
use aos_hub_core::lease::{PublishLease, LEASE_TTL_SECS};
