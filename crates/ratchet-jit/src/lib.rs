...
//! `ratchet-core`, [`cranelift`] records the exact Cranelift codegen crate pin
//! for the current CLIF slices, [`lower`] builds verified CLIF bodies for the
//! first literal Core-IR and constant-thunk smoke tests, [`symbols`] mirrors the
//! stable runtime-symbol manifest from `ratchet-core`, [`tier`] names the first
//! safe tier-up policy, [`warmup`] keeps the copy-and-patch hedge measurable,
