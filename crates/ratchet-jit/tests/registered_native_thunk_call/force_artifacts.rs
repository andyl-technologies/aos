...
fn registered_native_thunk_call_executes_static_select_artifact_with_candidates() {
    ...
    let candidates = [
        env_get_candidate(), force_candidate(), select_ic_candidate(),
        stack_map_enter_candidate(), stack_map_exit_candidate(),
    ];
    let invocation = unsafe {
        jit_cranelift_registered_native_thunk_call_for_artifact_with_candidates(
            artifact, &candidates, ptr::null_mut(), ptr::null_mut(),
        )
    }
    .expect("registered static select artifact can be called through native thunk ABI");
    assert_invocation(&invocation, Value::int(811), "aos_select_ic", &candidates[2]);
}
...
fn registered_native_thunk_call_executes_static_has_attr_artifact_with_candidates() {
    ...
    let candidates = [
        env_get_candidate(), force_candidate(), has_attr_candidate(),
        stack_map_enter_candidate(), stack_map_exit_candidate(),
    ];
    let invocation = unsafe {
        jit_cranelift_registered_native_thunk_call_for_artifact_with_candidates(
    ...
    assert_invocation(&invocation, Value::bool(true), "aos_has_attr", &candidates[2]);
}

#[test]
fn registered_native_thunk_call_executes_update_artifact_with_candidates() {
    let ir = update_ir(8, 9);
    let artifact = lower_update_local_slots_ir_root_thunk_body_artifact(&ir)
        .expect("update artifact lowers");
    let candidates = [
        env_get_candidate(), force_candidate(), update_candidate(),
        stack_map_enter_candidate(), stack_map_exit_candidate(),
    ];
    let invocation = unsafe {
        jit_cranelift_registered_native_thunk_call_for_artifact_with_candidates(
