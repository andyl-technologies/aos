...
const PAYLOAD_IDENTITY_AUDIT: &[PayloadIdentityAuditRow] = &[
    PayloadIdentityAuditRow {
        path: "ratchet-jit/src/cranelift/mixed_superblock.rs",
        raw_representation: 0,
        address_identity_only: 3,
        relocation_sensitive: 0,
        b2_disposition: "encode inline scalar constants into collector-free CLIF artifacts",
    },
    PayloadIdentityAuditRow {
    ...
    PayloadIdentityAuditRow {
        path: "ratchet-oracle/src/eval/heap/direct_root_rewrite.rs",
        raw_representation: 4,
        address_identity_only: 0,
        relocation_sensitive: 0,
        b2_disposition: "diagnostic stale-binding payloads only; no repair",
    },
    PayloadIdentityAuditRow {
    ...
    PayloadIdentityAuditRow {
        ...
        relocation_sensitive: 1,
        b2_disposition: "recursive-key cycle guard is collector-free; clear the advisory \
                         unhashable-value set in the live commit",
    },
    PayloadIdentityAuditRow {
    ...
    PayloadIdentityAuditRow {
        path: "ratchet-oracle/src/eval/tree_walk/genlist_elem_at.rs",
        raw_representation: 0,
        address_identity_only: 0,
        relocation_sensitive: 2,
        b2_disposition: "clear advisory generator recipes in the live moving-GC commit",
    },
    PayloadIdentityAuditRow {
        path: "ratchet-oracle/src/eval/tree_walk/maximal_laziness_census.rs",
        raw_representation: 0,
        address_identity_only: 1,
        relocation_sensitive: 0,
        b2_disposition: "compile-time-only collector-free census identity",
    },
    PayloadIdentityAuditRow {
        path: "ratchet-oracle/src/eval/tree_walk/option_access.rs",
        raw_representation: 0,
        address_identity_only: 0,
        relocation_sensitive: 2,
        b2_disposition: "rekey live option-provenance entries and drop dead young identities",
    },
    ...
    PayloadIdentityAuditRow {
        path: "ratchet-oracle/src/eval/tree_walk/relocation_identity.rs",
        raw_representation: 0,
        address_identity_only: 0,
        relocation_sensitive: 4,
        b2_disposition: "stage and apply option-provenance relocation repair",
    },
    PayloadIdentityAuditRow {
...
/// Excludes a known terminal inline test module from the production-source audit.
fn production_source<'a>(relative: &str, source: &'a str) -> &'a str {
    const MEMO_TEST_MODULE: &str = "\n#[cfg(test)]\nmod tests {";
    if relative == "ratchet-oracle/src/eval/tree_walk/memo.rs" {
        return source
            .split_once(MEMO_TEST_MODULE)
            .map(|(production, _)| production)
            .expect("memo.rs keeps its inline test module terminal");
    }
    source
}
...
fn audited_source_counts() -> BTreeMap<String, (usize, usize, usize)> {
    ...
    for path in sources {
        ...
        let relative = path
            ...
            .to_string_lossy()
            .replace('\\', "/");
        let accessor_counts = count_accessors(production_source(&relative, &source));
...
fn payload_identity_accessors_match_the_reviewed_b2_worklist() {
    ...
    // The review ledger names every production consumer separately so a new
    // identity-retaining subsystem cannot hide inside an aggregate total.
    assert_eq!(production_rows.clone().count(), 23);
    assert_eq!(
        production_rows
            .map(|row| {
                (
