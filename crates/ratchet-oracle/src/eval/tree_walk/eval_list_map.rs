...
impl TreeWalk {
    ...
    pub(super) fn eval_get_attr_primop(
        ...
        self.record_option_attr_access(attrs_value, key, selected);
        selected.ok_or_else(|| {
            TreeWalkError::new(
    ...
    pub(super) fn eval_has_attr_primop(
        ...
        self.record_option_attr_access(attrs_value, key, None);
    ...
    pub(super) fn eval_remove_attrs_primop(
        ...
        let attrs = FlatAttrs::new(entries, &self.symbols)
            .map_err(|source| TreeWalkError::new(TreeWalkErrorKind::Attr { id, source }, span))?;
        let result = self.alloc_dynamic_attrs_result_with_order_telemetry(id, span, attrs)?;
        self.propagate_option_provenance(result, &[attrs_value]);
        Ok(result)
    }
    ...
    pub(super) fn eval_intersect_attrs_primop(
        ...
        let attrs = FlatAttrs::new(entries, &self.symbols)
            .map_err(|source| TreeWalkError::new(TreeWalkErrorKind::Attr { id, source }, span))?;
        let result = self.alloc_dynamic_attrs_result_with_order_telemetry(id, span, attrs)?;
        self.propagate_option_provenance(result, &[right_value]);
        Ok(result)
    }
    ...
    pub(super) fn eval_map_attrs_primop(
        ...
        if entries.is_empty() {
            let result =
                self.alloc_dynamic_attrs_result_with_order_telemetry(id, span, FlatAttrs::empty())?;
            self.propagate_option_provenance(result, &[attrs_value]);
            return Ok(result);
        }
        ...
        let result = self.alloc_mapped_attrs(
            id,
            span,
            function_id,
            function_span,
