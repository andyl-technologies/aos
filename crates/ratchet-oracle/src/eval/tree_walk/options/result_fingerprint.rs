...
pub(super) fn result_affecting_fingerprint(options: &TreeWalkOptions) -> [u8; 32] {
    ...
    match options.max_eval_steps {
        Some(steps) => field(b"max_eval_steps", &steps.to_le_bytes()),
        None => field(b"max_eval_steps_unset", &[]),
    }
    match options.max_eval_duration {
        Some(duration) => field(
            b"max_eval_duration_nanos",
            &duration.as_nanos().to_le_bytes(),
        ),
        None => field(b"max_eval_duration_unset", &[]),
    }
    field(
        b"enforce_heap_memory_budget",
        &[options.enforce_heap_memory_budget as u8],
    );
    field(
