...
impl TreeWalkOptions {
    ...
    /// Returns the maximum number of evaluated IR nodes, when bounded.
    pub const fn max_eval_steps(&self) -> Option<u64> {
        self.max_eval_steps
    }

    /// Returns the in-engine wall-clock evaluation deadline, when bounded.
    pub const fn max_eval_duration(&self) -> Option<std::time::Duration> {
    ...
    /// Returns whether crossing the hard heap ceiling terminates evaluation.
    pub const fn enforce_heap_memory_budget(&self) -> bool {
        self.enforce_heap_memory_budget
    }
