...
/// Returns selected-child samples matching one runtime and thunk-work kind.
#[cfg(test)]
pub(super) fn recorded_genlist_selected_child_kind(
    runtime_kind: &'static str,
    thunk_kind: &'static str,
) -> u64 {
    match GENLIST_SELECTED_CHILD_CENSUS.lock() {
        Ok(guard) => guard.as_ref().map_or(0, |census| {
            census
                .iter()
                .filter(|(descriptor, _)| {
                    descriptor.runtime_kind == runtime_kind && descriptor.thunk_kind == thunk_kind
                })
