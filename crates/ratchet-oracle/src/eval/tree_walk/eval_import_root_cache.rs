...
impl TreeWalk {
    ...
    fn lookup_pure_import_root_cache(
        ...
        let early_cutoffs = self.stats.early_cutoffs;
        let value = self.lookup_forced_inline_expression_result(subject);
        if value.is_none() {
            self.stats.force_cache_misses = force_cache_misses;
        } else if self.stats.early_cutoffs == early_cutoffs {
            // A validated import-root replay is itself a dependency-granular
            // early cutoff even when the fresh process-local demand graph had
            // no dirty node to reconsider. Avoid double-counting when graph
            // reconsideration already recorded the cutoff.
    ...
    pub(super) fn eval_import_root_with_cache(
        ...
        // Capture the trace even on the first demand. A costly pure import can
        // then be materialized immediately and reused when an unrelated sibling
        // import changes on the next process invocation. Revalidation still
        // gates every replay, so the eager write changes only latency, never
        ...
        let observed = self.observe_forced_inline_expression_result_with_eval_work_units(
            ...
            crate::eval::tree_walk::eval_core::FirstDemandPersistence::AllowProfitableImportRoot,
        );
