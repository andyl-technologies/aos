...
impl TreeWalk {
    ...
    pub(super) fn bind_formal_set_argument(
        ...
        for (slot, formal) in layout.entries().iter().enumerate() {
            ...
            if self.symbols.resolve(formal.name) == Some(b"config".as_slice())
                && let Some(observer) = &self.options.option_read_observer
            {
                // Module formal binding is the semantic provenance boundary:
                // aliases and helper calls retain this value identity without
                // depending on the spelling of a later selection expression.
                observer.associate(value, Vec::new());
            }
