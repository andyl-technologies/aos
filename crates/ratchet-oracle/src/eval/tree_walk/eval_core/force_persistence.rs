...
/// Controls whether a profitable dependency boundary may be written on its
/// first observed demand.
///
/// Ordinary thunks require a demand from an earlier run before paying durable
/// serialization cost. Pure import roots are different: their source identity
/// and transitive input trace form a dependency-granular invalidation boundary,
/// and an import is intrinsically likely to be revisited when a sibling module
/// changes. For those roots, a first-run write is allowed only when the measured
/// evaluation work already exceeds the complete write cost.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::eval::tree_walk) enum FirstDemandPersistence {
    /// Requires evidence of demand from an earlier evaluator run.
    RequirePriorDemand,
    /// Allows a profitable pure import root to be persisted immediately.
    AllowProfitableImportRoot,
}
...
impl TreeWalk {
    ...
    pub(in crate::eval::tree_walk) fn observe_forced_inline_expression_result(
        ...
        self.observe_forced_inline_expression_result_with_eval_work_units(
            ...
            None,
            false,
            FirstDemandPersistence::RequirePriorDemand,
        );
    }
    ...
    pub(in crate::eval::tree_walk) fn observe_forced_inline_expression_result_with_eval_work_units(
        ...
        first_demand_persistence: FirstDemandPersistence,
    ) -> Option<DemandNodeId> {
    ...
    fn materialize_persist_forced_expression_payload(
        ...
        let costs = signals.costs();
        let profitable_first_import_demand = first_demand_persistence
            == FirstDemandPersistence::AllowProfitableImportRoot
            && costs.eval_cost() > costs.write_cost();
        let decision = if signals.decide() == MaterializationDecision::Materialize
            || profitable_first_import_demand
        {
