...
impl TreeWalk {
    /// Returns whether a subtree can emit an option-read observation.
    ///
    /// Observer side effects are intentionally excluded from force and memo
    /// caches until cache payloads carry a replayable observation trace. This
    /// keeps cold, materializing, and warm option graphs identical while still
    /// allowing unrelated pure subtrees to use the incremental cache.
    pub(in crate::eval::tree_walk::eval_core) fn subtree_contains_option_access(
        ir: &Ir,
        root: IrId,
    ) -> bool {
        ...
        while let Some(id) = stack.pop() {
            ...
            if matches!(node.kind, IrKind::Select | IrKind::HasAttr) {
                return true;
            }
...
mod tests {
    ...
    #[test]
    fn option_access_classifier_excludes_observed_but_not_pure_subtrees() {
        let selection = lower("let config = { nested = 1; }; in config.nested");
        assert!(TreeWalk::subtree_contains_option_access(
            &selection,
            selection.root
        ));

        let presence = lower("let config = { nested = 1; }; in config ? nested");
        assert!(TreeWalk::subtree_contains_option_access(
            &presence,
            presence.root
        ));

        let pure = lower("let left = 20; right = 22; in left + right");
        assert!(!TreeWalk::subtree_contains_option_access(&pure, pure.root));
    }
}
