...
impl TreeWalk {
    ...
    pub(in crate::eval::tree_walk) fn eval_attr_update(
        ...
        let result = if !self.attr_update_telemetry_enabled {
            self.merge_attr_update_fast(id, node.span, lhs, lhs_span, left, rhs, rhs_span, right)?
        } else {
        ...
        self.propagate_option_provenance(result, &[left, right]);
        Ok(result)
    }
    ...
    fn update_attr_values_with_operand_metadata(
        ...
        let result = if !self.attr_update_telemetry_enabled {
            self.merge_attr_update_fast(id, span, lhs, lhs_span, left, rhs, rhs_span, right)?
        } else {
            let left_operand = self.attr_entries_for_update(id, lhs, lhs_span, left)?;
            let right_operand = self.attr_entries_for_update(id, rhs, rhs_span, right)?;
