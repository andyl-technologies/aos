...
const DEDUP_STRING_LIST_BINDING: &str = r#"
  dedup = acc: remaining:
    if remaining == []
    then acc
    else let
      ...
      t = builtins.genList
        (i: builtins.elemAt remaining (i + 1))
        (builtins.length remaining - 1);
    in
...
#[cfg(feature = "dedup_string_list_canary")]
fn dedup_source(result: &str) -> String {
    format!("let {DEDUP_STRING_LIST_BINDING} in {result}")
}

#[test]
#[cfg(feature = "dedup_string_list_canary")]
fn dedup_string_list_canary_matches_renumbered_exact_body_and_preserves_order() {
    let source = format!(
        "let {DEDUP_STRING_LIST_BINDING} \
         in builtins.toJSON (dedup [] (builtins.attrNames {{ c = 1; a = 1; }} \
           ++ builtins.attrNames {{ c = 2; b = 2; a = 2; }}))"
    );
    let (values, admissions, executions) =
        dedup_string_list_canary::with_test_enabled(|| eval_string_bytes(&source));
    assert_eq!(values, br#"["a","c","b"]"#);
...
#[test]
#[cfg(feature = "dedup_string_list_canary")]
fn dedup_string_list_canary_empty_case_keeps_acc_lazy() {
    let source = dedup_source("builtins.isFunction (dedup (x: builtins.throw \"unused\") [])");
    let (value, admissions, executions) = dedup_string_list_canary::with_test_enabled(|| {
        let ir = lower(&source);
        let mut evaluator = TreeWalk::new(&ir);
        evaluator.eval_root()
    });
    assert_eq!(
        value
            .expect("empty result remains the accumulator")
            .as_bool(),
        Ok(true)
    );
    assert_eq!(admissions, 1);
    assert_eq!(executions, 0, "empty case returns before the nonempty loop");
}

#[test]
#[cfg(feature = "dedup_string_list_canary")]
fn dedup_string_list_canary_declines_non_string_elements_to_generic_evaluation() {
    let source = dedup_source("builtins.toJSON (dedup [] [ 1 2 1 ])");
    let (values, admissions, executions) =
        dedup_string_list_canary::with_test_enabled(|| eval_string_bytes(&source));
    assert_eq!(values, br#"[1,2]"#);
    assert_eq!(
        admissions, 1,
        "the body matches before the value guard declines"
    );
...
#[test]
#[cfg(feature = "dedup_string_list_canary")]
fn dedup_string_list_canary_rejects_nearby_structural_mutation() {
    let mutated = DEDUP_STRING_LIST_BINDING.replace("(i + 1)", "(i + 0)");
    let source = format!("let {mutated} in builtins.toJSON (dedup [] [])");
    let (value, admissions, executions) =
        dedup_string_list_canary::with_test_enabled(|| eval_string_bytes(&source));
    assert_eq!(value, b"[]");
