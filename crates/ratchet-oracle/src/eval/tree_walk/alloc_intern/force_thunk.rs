...
impl TreeWalk {
    ...
    fn force_value_with_native_census(
        ...
        let provenance = self
            ...
            .as_ref()
            .map(|observer| observer.provenance(value))
            .unwrap_or_default();
        let forced = match classify_whnf_tag_fast_path(value) {
            WhnfTagFastPath::AlreadyWhnf(value) => value,
        ...
        if let Some(observer) = &self.options.option_read_observer {
            observer.associate_all(forced, &provenance);
        }
        Ok(forced)
    }
    ...
    pub(in crate::eval::tree_walk) fn force_memoized_claimed_thunk(
        ...
        if let Some(impure_trace) = impure_trace {
            ...
            self.observe_forced_inline_expression_result_with_eval_work_units(
                ...
                crate::eval::tree_walk::eval_core::FirstDemandPersistence::RequirePriorDemand,
            );
        }
        Ok(value)
    }
