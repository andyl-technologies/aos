...
///
/// This includes the typed effect and event evidence consumed by campaign
/// inspection, so clients do not need a direct engine dependency.
pub mod engine {
    pub use crucible::model::{
        Aggregation, BoundarySelector, CohortPolicy, EffectSpecification, FaultCoordinate,
        FaultDirection, FaultOperation, FaultReplayMode, FaultResourceLimits,
    ...
    pub use crucible::{
        ...
        FailureClusteringResult, FailureFindingsLedger, FailureKind,
        ...
        FailureTimeoutBudgetKind, FailureTimeoutRecord, FailureTriageReplayEvidence,
        ...
        ObservableEventPayload, OverrideDecision, PARTITION_RECOVERY_SCENARIO_NAME, Plan,
        Predicate, PreemptionDecision, PreemptionKind, Properties, Property, QuantumLoop,
        QuantumOutcome, QuantumRequest, ReadyPoint, RecordedAssertionLog, ReplayOracleCheck,
        ReproductionArtifact, ResolvedCodePoint, ResolvedFaultTarget, ResolvedMemPlace,
        RngDecision, RngStreamId, SHMEM_ABI_VERSION, ScenarioDef, ScenarioDefForm, ScenarioFamily,
        ScenarioSelectableLimits, ScenarioSelectables, Schedule, SchedulerError,
        SchedulerEvaluationBoundaryKind, SchedulerEventLogClass, SchedulerEventLogEntry,
        SchedulerEventLogPayload, SchedulerNodeId, SchedulerQuiescence, SchedulingNodeKind,
        SchedulingPoint, SearchAssertionFinding, SearchBudget, SearchDiscoveredFailure,
