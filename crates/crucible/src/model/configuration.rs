...
impl Schedule {
    ...
    /// Version-1 artifacts must pass through [`Self::migrate_version_one`].
    pub fn from_compact_binary(bytes: &[u8]) -> Result<Self, EngineError> {
    ...
    /// Migrates one bounded version-1 artifact into the current schedule schema.
    ///
    /// Version 1 did not retain the typed declaration, opportunity, domain, and
    /// origin needed to authenticate explorable overrides, preemptions, or
    /// application randomness. The migration therefore accepts only resolved
    /// delivery order and raw RNG evidence. The returned schedule writes only
    /// version 2 and is accepted by [`Self::from_compact_binary`].
    ///
    /// # Errors
    ///
    /// Returns [`LegacyScheduleMigrationError::NotVersionOne`] when `bytes` is
    /// not explicitly version 1,
    /// [`LegacyScheduleMigrationError::UnsupportedDecision`] for a legacy
    /// decision whose typed selection basis cannot be reconstructed, or
    /// [`LegacyScheduleMigrationError::Decode`] for malformed or over-limit
    /// version-1 input.
    pub fn migrate_version_one(bytes: &[u8]) -> Result<Self, LegacyScheduleMigrationError> {
        if !bytes.starts_with(SCHEDULE_BINARY_MAGIC_V1) {
            return Err(LegacyScheduleMigrationError::NotVersionOne);
        }

        let mut reader = ScenarioBinaryReader::new(bytes, SCHEDULE_BINARY_MAGIC_V1)
            .map_err(LegacyScheduleMigrationError::Decode)?;
        let schedule =
            read_schedule_binary(&mut reader).map_err(LegacyScheduleMigrationError::Decode)?;
        reader
            .finish()
            .map_err(LegacyScheduleMigrationError::Decode)?;
        ...
        for (index, decision) in schedule.decisions.iter().enumerate() {
            let kind = match decision {
                Decision::Override(_) => Some(LegacyScheduleDecisionKind::Override),
                Decision::Preemption(_) => Some(LegacyScheduleDecisionKind::Preemption),
                Decision::AppRandom(_) => Some(LegacyScheduleDecisionKind::AppRandom),
                Decision::Selection(_) => Some(LegacyScheduleDecisionKind::Selection),
                Decision::DeliveryOrder(_) | Decision::RngDraw(_) => None,
            };
            if let Some(kind) = kind {
                return Err(LegacyScheduleMigrationError::UnsupportedDecision { index, kind });
            }
        }
...
/// A decision kind that version-1 schedules cannot migrate without guessing.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum LegacyScheduleDecisionKind {
    /// An untyped search or fuzzing override.
    Override,
    /// An untyped vCPU or interrupt preemption.
    Preemption,
    /// A served value without a retained typed selection envelope.
    AppRandom,
    /// A typed selection forged into a schema that never supported it.
    Selection,
}

impl LegacyScheduleDecisionKind {
    const fn as_str(self) -> &'static str {
        match self {
            Self::Override => "override",
            Self::Preemption => "preemption",
            Self::AppRandom => "application randomness",
            Self::Selection => "selection",
        }
    }
}

/// Failure while migrating a legacy schedule into the current schema.
#[derive(Debug)]
pub enum LegacyScheduleMigrationError {
    /// The input does not declare the exact version-1 schedule schema.
    NotVersionOne,
    /// The bounded version-1 input is malformed or violates its codec limits.
    Decode(EngineError),
    /// One decision lacks the evidence needed to reconstruct a typed selection.
    UnsupportedDecision {
        /// Zero-based decision offset in the legacy schedule.
        index: usize,
        /// Legacy decision kind found at the offset.
        kind: LegacyScheduleDecisionKind,
    },
}

impl fmt::Display for LegacyScheduleMigrationError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotVersionOne => formatter.write_str("schedule is not schema version 1"),
            Self::Decode(error) => write!(formatter, "legacy schedule decoding failed: {error}"),
            Self::UnsupportedDecision { index, kind } => write!(
                formatter,
                "legacy {} decision at index {index} has no typed migration evidence",
                kind.as_str()
            ),
        }
    }
...
impl Error for LegacyScheduleMigrationError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::Decode(error) => Some(error),
            Self::NotVersionOne | Self::UnsupportedDecision { .. } => None,
        }
    }
}
