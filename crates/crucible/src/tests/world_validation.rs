...
tag = "negative-time"
"#;
    let negative_plan_time = Plan::from_canonical_toml_for_world(&world, negative_plan_time_toml);
...
fn properties_content_address_is_orthogonal_and_validated() {
    ...
    assert_eq!(
        ...
        world
            ...
            .unwrap_or_else(|error| panic!("empty plan and properties should compose: {error}"))
    );
    assert_eq!(
        world
            .scenario_def_with_plan(&empty_plan)
    ...
    assert_ne!(
        ...
        world
            ...
            .unwrap_or_else(|error| panic!("properties should affect scenario identity: {error}"))
    );
    assert_ne!(
        world
            .scenario_def_with_plan_and_properties(&empty_plan, &properties)
...
fn scenario_builder_keeps_authoring_layers_structurally_orthogonal() {
    ...
    let scenario = ScenarioBuilder::new()
        .node(
            "a",
            NodeTemplate::fixed_icount(Icount { retired: 11 }).white_box(WhiteBoxPolicy::Disabled),
        )
        .node_like("b", "a")
        .link_with_transport(
            "b",
    ...
    let manual_properties = Properties::from_assertions_for_world(&manual_world, vec![property])
...
fn serializable_scenario_form_round_trips_and_rejects_host_paths() {
    ...
    let root_image_ref = ContentAddressedBlobRef::from_hash(ContentHash::from_canonical_material(
        "crucible.test.blob",
        "root-image",
    ));
    ...
    let wrong_hash = ContentHash::from_canonical_material("crucible.test.scenario-form", "wrong");
...
fn scenario_family_pins_concrete_validated_instances() {
    ...
    let tiny_space = FamilySpace::new(
        ...
        FaultDensityRange::new(
            zero_density,
            FaultDensity::from_millionths(1)
                .unwrap_or_else(|error| panic!("one-millionth density should be valid: {error}")),
        )
        .unwrap_or_else(|error| panic!("tiny density range should be valid: {error}")),
        TopologySizeRange::new(1, 2)
            .unwrap_or_else(|error| panic!("tiny topology size range should be valid: {error}")),
    ...
    let zero_faults = family
        ...
        .unwrap_or_else(|error| panic!("zero-density family params should instantiate: {error}"));
    ...
    let random_zero_faults = family
        ...
        .unwrap_or_else(|error| panic!("random zero-density topology should instantiate: {error}"));
    let random_half_faults = family
        .instantiate(FamilyParams {
            topology_shape: TopologyShape::Random,
            ..params
        })
        .unwrap_or_else(|error| panic!("random half-density topology should instantiate: {error}"));
    let sampled = family
...
fn reproduction_artifact_is_self_contained_and_replay_checked() {
    ...
    let pinned_genesis_artifact = ReproductionArtifact::from_pinned_configuration(&pinned_genesis)
...
fn canonicalization_hashes_meaning_not_authoring_spelling() {
    ...
    let changed_kernel = ContentAddressedBlobRef::from_hash(ContentHash::from_canonical_material(
        "crucible.test.canonicalization.blob",
        "changed-kernel",
    ));
    ...
    let density_family = ScenarioFamily::new(
        FamilySpace::new(
            ...
            FaultDensityRange::new(FaultDensity::ZERO, density)
                .unwrap_or_else(|error| panic!("density range should be valid: {error}")),
            TopologySizeRange::new(4, 4)
                .unwrap_or_else(|error| panic!("density topology range should be valid: {error}")),
            vec![TopologyShape::Ring],
        )
    ...
    assert!(
        authored_plan
            .to_compact_binary()
            .starts_with(b"crucible.plan.v3\0")
    );
    assert!(
        authored_properties
            .to_compact_binary()
    ...
    assert!(
        authored_form
            .to_compact_binary()
            .starts_with(b"crucible.scenario-def-form.v3\0")
    );
    assert_eq!(
...
pub(super) fn tag(name: &str) -> FaultTag {
...
pub(super) fn marker_id(name: &str) -> MarkerId {
    MarkerId::from_name(name)
}

pub(super) fn named_predicate(name: &str, nodes: &[&str]) -> Predicate {
    Predicate::Named {
        name: name.to_owned(),
...
pub(super) fn assertion(name: &str, message: &str, property: Property) -> AssertionDef {
    AssertionDef {
