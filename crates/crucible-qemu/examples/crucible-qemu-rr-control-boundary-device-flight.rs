...
fn exercise_block_completion(
    ...
    if console_marker_count(&priming_events) != 0 {
    ...
    {
        return Err(format!(
            "block flight did not return one exact completed pause: baseline={settled_time:?}, observation={observation:?}"
        )
        .into());
    }
    let events = SimulationBackend::drain_observable_events(node)?;
    let marker_count = console_marker_count(&events);
    if marker_count != 1 {
        return Err(format!(
...
#[cfg(target_os = "linux")]
fn console_marker_count(events: &[crucible::ObservableEvent]) -> usize {
    let console = events
        .iter()
