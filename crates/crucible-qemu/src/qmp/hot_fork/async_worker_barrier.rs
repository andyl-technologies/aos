//! Retained QEMU-owned asynchronous-worker barrier.
...
/// QMP command name used for the reversible asynchronous-worker barrier.
///
/// Schema 3 covers AioContexts, AioHandlers, and coroutines under the canonical
/// async-worker barrier command.
pub const QMP_HOT_FORK_ASYNC_WORKER_BARRIER_COMMAND: &str =
    "crucible-hot-fork-async-worker-barrier";
/// Version of the QEMU-owned asynchronous-worker barrier contract.
pub const QMP_HOT_FORK_ASYNC_WORKER_BARRIER_SCHEMA_VERSION: u32 = 3;

/// Exact state of QEMU's retained asynchronous-worker barrier.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct QmpHotForkAsyncWorkerBarrierState {
    generation: u64,
    owner_thread_id: i64,
    held: bool,
    complete: bool,
...
impl QmpHotForkAsyncWorkerBarrierState {
    ...
    /// Returns whether every bounded asynchronous-worker inventory was complete.
    #[must_use]
...
pub(crate) fn parse_hot_fork_async_worker_barrier_state(
    value: &Value,
) -> Result<QmpHotForkAsyncWorkerBarrierState, QmpError> {
    parse_hot_fork_async_worker_barrier_state_for(QmpCommandKind::HotForkAsyncWorkerBarrier, value)
}

pub(super) fn parse_hot_fork_async_worker_barrier_state_for(
    ...
    let expected_quiescent = held
        ...
        && active_aio_handler_callbacks == 0
        && active_timer_callbacks == 0;
    let valid = schema_version == u64::from(QMP_HOT_FORK_ASYNC_WORKER_BARRIER_SCHEMA_VERSION)
    ...
    Ok(QmpHotForkAsyncWorkerBarrierState {
        generation,
        owner_thread_id,
        held,
        complete,
