...
#[test]
fn production_envelope_rejects_retired_validation_bearing_version() {
    let snapshot = snapshot_fixture("retired-validation-version");
    let mut bytes = snapshot
        .to_canonical_bytes()
        .unwrap_or_else(|error| panic!("encode current snapshot: {error}"));
    bytes[..MAGIC.len()].copy_from_slice(b"crucible.qemu-vm-snapshot.v3\0");
