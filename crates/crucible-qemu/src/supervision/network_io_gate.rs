...
use crate::{
    CrucibleShmemNetworkDevice, LaunchProfileCandidate, LaunchProfileError, QemuHostPluginSetup,
    ...
    QemuNodeChannelError, QemuNodeChild, QemuPluginIpcControlChannel, QemuQmpChannelConfig,
    QemuQmpVmStateControlChannel, QemuQuantumShmemConfig, QemuShmemHotPathChannel,
    QemuVmLaunchConfig, QmpError, complete_qemu_host_plugin_setup,
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct NetworkDeterministicProjection {
    protocol_frames: Vec<(u32, Vec<u8>)>,
    reply_latency_icount: Option<u64>,
}

fn deterministic_projection(outcome: &NetworkIoRunOutcome) -> NetworkDeterministicProjection {
...
fn probe_emit_icount(outcome: &NetworkIoRunOutcome) -> Option<u64> {
    protocol_frames(outcome).first().map(|frame| frame.0)
}

fn acknowledgement_offset_icount(outcome: &NetworkIoRunOutcome) -> Option<u64> {
...
fn vm_launch_config(config: &QemuLiveNetworkIoGateConfig) -> QemuVmLaunchConfig {
