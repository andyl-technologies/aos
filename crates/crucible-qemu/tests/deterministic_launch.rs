...
fn launch_command_builder_adds_plugin_and_hashes_full_argv() {
    ...
    let plugin_argument = format!(
        "/nix/store/22222222222222222222222222222222-crucible-qemu-plugin/lib/libcrucible_qemu_plugin.so,simfd=3,slot=0,fault_node_hash={fault_hash},process_generation=1,shmemfd=4,wakefd=5,whitebox=off,coverage=off"
    );
    ...
    let expected_plugin_args = format!(
        "simfd=3,slot=2,fault_node_hash={fault_hash},process_generation=1,shmemfd=4,wakefd=5,whitebox=on,coverage=on,whitebox_setup=x86-port-00e7-unclaimed-v1"
    );
