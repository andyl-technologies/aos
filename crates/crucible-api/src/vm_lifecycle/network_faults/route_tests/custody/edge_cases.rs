...
fn completed_contact_ledgers_fold_into_the_settled_cursor() {
    ...
    state.contact_services.insert(
        ...
        NetworkContactServiceState {
            settled_cursor_ticks: 100_000,
            service_cursor_ticks: 112_000,
            served_bundles: 1,
            served_bytes: 1,
            reservations: vec![NetworkContactServiceReservation {
                ...
                start_ticks: 110_000,
                finish_ticks: 112_000,
                arrival_ticks: 112_000,
                bytes: 1,
            }],
        },
    );

    prune_network_contact_services(&mut state, 112_000);
    let service = state
        .contact_services
        .values()
        .next()
    ...
    assert_eq!(service.settled_cursor_ticks, 112_000);
    assert_eq!(service.service_cursor_ticks, 112_000);
...
fn direct_contact_counter_overflow_fails_before_mutation() {
    ...
    for (served_bundles, served_bytes) in [(u64::MAX, 0), (0, u64::MAX)] {
        ...
        let service = state
            ...
            .get(&key)
            .unwrap_or_else(|| panic!("contact service"));
        assert_eq!(service.service_cursor_ticks, 100_000);
...
fn custody_priority_arbitrates_equal_contact_release_coordinates() {
    ...
    for (sequence, action, priority, effects) in [
        ...
        assert_eq!(waiting.defer_until, Some(110_000));
    }
    ...
    assert_eq!(critical_service.defer_until, Some(112_000));
    assert_eq!(bulk_service.defer_until, Some(113_000));
}
...
fn custody_expiry_precedes_an_unreachable_future_contact() {
    ...
    assert_eq!(waiting.defer_until, Some(1_000_000));
...
fn custody_overflow_executes_every_closed_disposition() {
    for disposition in [
        ...
        let second = opportunity_at(2, 1_000);
        ...
        match disposition {
            ...
            crucible::model::NetworkPolicyOverflow::Timeout => {
                assert_eq!(second_application.defer_until, Some(26_000));
...
fn custody_removal_releases_the_real_pending_frame_at_the_boundary() {
    ...
    removal.coordinate.virtual_ticks = 510_000;
    assert!(
        apply_network_custody_removals(&mut state, &mut pending, &[removal], 510_000)
