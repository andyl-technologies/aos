...
const JIT_CONTEXT_DECODER: &str =
    concat!("pub(crate) ", "uns", "afe fn with_native_jit_context<R>(");
...
const JIT_EVALUATOR_CONTEXT_DECODER: &str = concat!(
    "pub(crate) ",
    "uns",
    "afe fn with_native_jit_evaluator_context<R>("
);
...
const JIT_EVALUATOR_CAST: &str =
    concat!("let eval = ", "uns", "afe { jit_context.eval.as_mut() };");
...
const EXIT_TYPE: &str = concat!(
    "pub type RuntimeJitStackMapExitNativeFn = ",
    "uns",
    "afe ",
    "ext",
...
const ENTER_FN: &str = concat!(
    ...
    "afe ",
    "ext",
    "ern \"C\" fn aos_jit_stack_map_enter("
);
...
const BOUND_HEADER_READ: &str = concat!("let bound_header = ", "uns", "afe { binding.as_ref() };");
const VALUES_ADDRESS: &str = concat!("let values = ", "uns", "afe {");
const VALUE_READ: &str = concat!(
    "let value = ",
    "uns",
    "afe { values.add(index as usize).read() };"
);
const BOUND_VALUE_READ: &str = concat!("let value = ", "uns", "afe { pointer.as_ptr().read() };");
const BOUND_VALUE_WRITE: &str = concat!("uns", "afe { pointer.as_ptr().write(slot.value()) };");
const BOUND_VALUE_ADDRESS: &str = concat!("let pointer = ", "uns", "afe {");
const ENTER_DECODE: &str = concat!(
    "uns",
    "afe { // aos_jit_stack_map_enter runtime-context decode"
);
const EXIT_DECODE: &str = concat!(
    "uns",
    "afe { // aos_jit_stack_map_exit runtime-context decode"
);
    ...
    if source_path == source_root.join("context.rs") {
        return token == UNSAFE_TOKEN
            && matches!(
                ...
                JIT_CONTEXT_DECODER
                    | JIT_CONTEXT_CAST
                    | JIT_EVALUATOR_CONTEXT_DECODER
                    | JIT_EVALUATOR_CONTEXT_CAST
                    | JIT_EVALUATOR_CAST
            );
    }
...
pub(super) fn assert_reviewed(source_root: &Path) {
    ...
    for (text, expected, message) in [
        ...
        assert_eq!(
            trimmed_line_occurrences(input, text),
            expected,
            "{message} changed"
        );
    }
