...
pub(super) fn is_allowed_token(
    ...
    match token {
        UNSAFE_TOKEN => [ENTER_TYPE, EXIT_TYPE, EXPORT_ATTR, ENTER_FN, EXIT_FN, HEADER_READ,
            VALUES_ADDRESS, VALUE_READ, ENTER_DECODE, EXIT_DECODE, TEST_CALL].contains(&trimmed),
...
pub(super) fn assert_reviewed(source_root: &Path) {
    ...
    for line in [ENTER_FN, EXIT_FN, HEADER_READ, VALUES_ADDRESS, VALUE_READ, ENTER_DECODE,
        EXIT_DECODE, TEST_CALL] {
        assert_eq!(trimmed_line_occurrences(&source, line), 1, "reviewed line changed");
    }
    let lines = source.lines().collect::<Vec<_>>();
    for line in [HEADER_READ, VALUES_ADDRESS, VALUE_READ, ENTER_DECODE, EXIT_DECODE, TEST_CALL] {
