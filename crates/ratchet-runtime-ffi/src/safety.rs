...
"#,
    ...
    fn assert_has_safety_comment_before(lines: &[&str], expected: &str, message: &str) {
        ...
        // Reviewed boundaries may carry a `rustfmt::skip` marker immediately
        // before the exact audit line so rustfmt cannot split its identity.
