...
pub fn run_context_finalized_native_lambda_call(
    ...
    #[rustfmt::skip]
    let lambda_dispatched = unsafe { // shared-context finalized lambda call
        jit_cranelift_call_context_finalized_lambda_entry(body, rt, env, argument)
    };
...
pub fn run_context_finalized_native_chain_call(
    ...
    #[rustfmt::skip]
    let chain_call = unsafe { // shared-context finalized chain call
        jit_cranelift_call_context_finalized_lambda_argv_entry(body, rt, env, argv)
    };
    match chain_call {
        Ok(value) => {
            let trap = scope.take_trap();
...
pub fn run_context_finalized_native_filter_loop(
    ...
    for (index, element) in elements.iter().copied().enumerate() {
        ...
        #[rustfmt::skip]
        let filter_step = unsafe { // shared-context finalized filter step
            jit_cranelift_call_context_finalized_lambda_argv_entry(body, rt, env, &argv)
        };
...
pub fn run_context_finalized_native_all_any_loop(
    ...
    for (index, element) in elements.iter().copied().enumerate() {
        ...
        #[rustfmt::skip]
        let step = unsafe { // shared-context finalized all-any step
            jit_cranelift_call_context_finalized_lambda_argv_entry(body, rt, env, &argv)
        };
...
fn run_native_fold_loop(
    ...
    for index in 0..source.len() {
        ...
        #[rustfmt::skip]
        let fold_step = unsafe { // shared-context finalized fold step
            jit_cranelift_call_context_finalized_lambda_argv_entry(body, rt, env, &argv)
        };
...
fn run_native_fold_loop_i64acc(
    ...
    for index in 0..source.len() {
        ...
        #[rustfmt::skip]
        let fold_step = unsafe { // shared-context finalized fold-step i64acc
            jit_cranelift_call_context_finalized_fold_step_i64acc_entry(
                body,
                rt,
                env,
