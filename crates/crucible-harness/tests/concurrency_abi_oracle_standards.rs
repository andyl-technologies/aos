...
fn boundary_abi_owner_failures(targets: &[GateTargetSpec]) -> Vec<String> {
...
fn spsc_ring_unsafe_without_model_failures(
    root: &Path,
    targets: &[GateTargetSpec],
...
fn concurrent_primitive_before_model_failures(
    source_label: &str,
    file_name: &str,
    content: &str,
...
fn advanced_standard_regression_failures() -> Vec<String> {
    let mut failures = Vec::new();
    let broken_targets = [
        GateTargetSpec {
            gate: "gate:layer1-injection",
