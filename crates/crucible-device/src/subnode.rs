...
const IO_CORE_SNAPSHOT_MAGIC: &[u8] = b"crucible.io-core-snapshot.v1\0";
...
pub enum IoCoreSnapshotCodecError {
    ...
    /// A queue or frame payload exceeds its hard bound.
    #[error("I/O-core snapshot `{field}` exceeds hard limit {hard}")]
    Limit {
...
fn validate_io_core_snapshot(snapshot: &IoCoreSnapshot) -> Result<(), IoCoreSnapshotCodecError> {
    ...
    for (field, capacity, length) in [
        ...
        {
            return Err(IoCoreSnapshotCodecError::Limit {
                field,
                hard: HARD_IO_CORE_CHECKPOINT_ENTRIES,
            });
        }
    }
    if snapshot.inflight.len() > HARD_IO_CORE_CHECKPOINT_ENTRIES {
        return Err(IoCoreSnapshotCodecError::Limit {
            field: "in-flight responses",
            hard: HARD_IO_CORE_CHECKPOINT_ENTRIES,
        });
    }
...
fn write_io_count(
    ...
    bytes.extend_from_slice(
        &u32::try_from(count)
            .map_err(|_| IoCoreSnapshotCodecError::Limit {
                field,
                hard: HARD_IO_CORE_CHECKPOINT_ENTRIES,
            })?
            .to_le_bytes(),
