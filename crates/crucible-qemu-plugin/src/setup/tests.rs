...
fn plugin_handshake(slot_index: u32, node_count: u32) -> PluginControlHandshake {
    let args = PluginArgs::parse(&format!("simfd=3,slot={slot_index},fault_node_hash=1111111111111111111111111111111111111111111111111111111111111111,process_generation=1"))
...
fn owned_callbacks(
    ...
    let args = PluginArgs::parse(&format!("simfd=3,slot={slot},fault_node_hash=1111111111111111111111111111111111111111111111111111111111111111,process_generation=1"))
        .unwrap_or_else(|error| panic!("test plugin args should parse: {error}"));
