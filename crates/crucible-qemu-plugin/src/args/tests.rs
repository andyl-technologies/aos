...
fn plugin_args_parse_terminal_state_dump_as_complete_group() {
    let args = PluginArgs::parse(
        "simfd=4,slot=1,fingerprint=on,state_dump_target=4000001,state_dump_path=/tmp/dump.bin",
    )
...
fn plugin_args_reject_incomplete_or_unscoped_state_dump() {
    assert_eq!(
        PluginArgs::parse("simfd=4,slot=1,fingerprint=on,state_dump_target=1"),
        Err(PluginArgsParseError::IncompleteStateDump)
    );
    assert_eq!(
        PluginArgs::parse("simfd=4,slot=1,state_dump_target=1,state_dump_path=/tmp/dump.bin"),
        Err(PluginArgsParseError::StateDumpWithoutFingerprint)
    );
    assert!(matches!(
        PluginArgs::parse(
            "simfd=4,slot=1,fingerprint=on,state_dump_target=1,state_dump_path=relative"
        ),
...
fn plugin_args_require_whitebox_setup_attestation_exactly_when_enabled() {
    ...
    assert_eq!(
        PluginArgs::parse("simfd=3,slot=0,whitebox=on,whitebox_setup=x86-port-00e8-unclaimed-v1"),
    ...
    assert_eq!(
        PluginArgs::parse("simfd=3,slot=0,whitebox=off,whitebox_setup=x86-port-00e7-unclaimed-v1"),
