...
//!
//! Verifies that [`seed_dev`](aos_registry_hub::seed::seed_dev) populates a
...
use aos_registry_hub::auth::password::verify_password;
use aos_registry_hub::db::Database;
use aos_registry_hub::seed::{seed_dev, SeedOutcome, DEMO_EMAIL, DEMO_ORG, DEMO_PASSWORD};

#[tokio::test]
async fn seed_creates_browsable_registry_and_login() {
    let root = tempfile::tempdir().unwrap();
    let db = Database::open(&root.path().join("hub.db")).unwrap();
    ...
    let org = db.org_by_slug(DEMO_ORG).unwrap().expect("demo org exists");
    assert_eq!(db.list_projects(org.id).unwrap().len(), 1);
    let user = db
        .user_by_email(DEMO_EMAIL)
        .unwrap()
        .expect("demo user exists");
    assert_eq!(db.list_storage_bindings(org.id).unwrap().len(), 1);
    ...
    assert!(!db.list_releases(registry.id).unwrap().is_empty());
    assert!(!db.list_channels(registry.id).unwrap().is_empty());
...
async fn re_seeding_is_a_safe_no_op() {
    ...
    let org = db.org_by_slug(DEMO_ORG).unwrap().unwrap();
    assert_eq!(db.org_registry_count(org.id).unwrap(), 1);
}
