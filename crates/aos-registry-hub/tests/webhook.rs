...
use aos_registry_hub::db::{Database, DueDelivery, TokenAuth};
use aos_registry_hub::domain::{Permission, Principal, Scope};
use aos_registry_hub::server::{router, AppState};
use aos_registry_hub::webhook::{self, WebhookEvent};
use axum::body::Body;
...
async fn metrics_renders_counters() {
    ...
    assert!(text.contains("# TYPE aos_registry_hub_registries_total gauge"));
