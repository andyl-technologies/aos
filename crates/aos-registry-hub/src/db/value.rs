...
//!
//! The hub's [`Database`](crate::db::Database) speaks to three SQL engines
//! (sqlite, postgres, mysql) through one [`Backend`](crate::db::backend::Backend)
//! trait. That trait moves data in and out as a single common shape so the
//! per-engine drivers stay thin: a [`Value`] is the union of the column types
//! the hub actually stores, and a [`Row`] is a positional vector of them with
//! a typed [`Row::get`] accessor.
//!
//! # Value taxonomy
//!
...
//!
//! ```
//! use aos_registry_hub::db::value::{Row, Value};
//!
