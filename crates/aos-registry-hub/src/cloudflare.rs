...
pub struct DeployConfig {
    ...
    /// A custom domain to bind the Worker to (e.g. `aos.andyl.org`), emitted as a
    /// `custom_domain` route. `None` serves on `*.workers.dev` only. The domain's
...
pub fn render_wrangler_toml(cfg: &DeployConfig) -> String {
    ...
    // A custom-domain route binds the Worker to e.g. aos.andyl.org; `wrangler
    // deploy` provisions the domain (DNS record + cert) when the zone is on the
    // account. Absent, the Worker serves on *.workers.dev only.
    let routes = match &cfg.custom_domain {
        Some(domain) => format!(
            "[[routes]]\npattern = {}\ncustom_domain = true\n\n",
            toml_string(domain)
        ),
    ...
    format!(
        "# Generated by `aos-registry-hub worker` — do not edit by hand.\n\
         name = {name}\n\
         main = \"shim.mjs\"\n\
...
mod tests {
    ...
    fn rendered_toml_has_bindings_vars_and_no_build() {
        let cfg = DeployConfig {
            ...
            custom_domain: Some("aos.andyl.org".into()),
        };
