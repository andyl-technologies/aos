...
//! ([`RpcService::put_machine_path`](aos_registry_core::service::RpcService::put_machine_path)
//! / [`head_machine_path`](aos_registry_core::service::RpcService::head_machine_path)),
...
//! requires [`Permission::Publish`](aos_registry_core::domain::Permission::Publish)
//! on the registry's canonical [`Scope`](aos_registry_core::domain::Scope).
...
use aos_registry_core::service::{FacadeWrite, RpcService};

use crate::server::AppState;

/// The native, process-local publish lease backing the upload facade.
///
/// Relocated to shared core ([`aos_registry_core::lease`]) in RFC-0004 Phase 5
...
pub use aos_registry_core::lease::InMemoryLease as LeaseMap;
...
pub use aos_registry_core::service::MAX_UPLOAD_BYTES;
...
///
/// A thin shim over the shared
/// [`RpcService::head_machine_path`](aos_registry_core::service::RpcService::head_machine_path)
...
pub async fn cache_serve_file(root: &std::path::Path, rel: &str, headers: &HeaderMap) -> Response {
    ...
    let ct = aos_registry_core::keymap::content_type(rel);
    let cc = aos_registry_core::keymap::cache_control(rel);
