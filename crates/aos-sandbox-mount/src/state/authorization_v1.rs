//! Authenticated mount-authorization records stored in the common journal.
//!
...
//! mount broker's authorization-bearing values. The node-local MAC covers the
...
const AUTHENTICATED_MAGIC: &[u8; 8] = b"AOSMAJ\0\0";
const AUTHENTICATED_VERSION: u16 = 1;
const AUTHENTICATED_FIXED_BYTES: usize = 8 + 2 + 1 + 1 + 16 + 4 + 32;
const AUTHENTICATION_DOMAIN: &[u8] = b"aos.mount.journal-authentication.v1\0";
...
/// Reports malformed or unauthenticated mount authorization journal state.
#[derive(Clone, Copy, Debug, Eq, PartialEq, thiserror::Error)]
pub(crate) enum AuthorizationRecordError {
    ...
    #[error("invalid durable mount authorization payload")]
    InvalidPayload,
}
...
/// itself secret.
pub(crate) struct NodeJournalMacKey {
    key_id: [u8; 16],
...
pub(crate) struct MountAuthorizationFenceV1 {
...
impl MountAuthorizationFenceV1 {
    ...
    #[must_use]
    pub(crate) const fn ownership_authority(&self) -> &KeyReference {
        &self.ownership_authority
    }

    /// Returns the nested node-local lease record.
    #[must_use]
    pub(crate) const fn local_lease_record(&self) -> &LocalLeaseRecord {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum MountEffectStatusV2 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct MountEffectIntentV2 {
    status: MountEffectStatusV2,
    request_id: [u8; 16],
...
impl MountEffectIntentV2 {
    ...
    pub(crate) fn pending(
        ...
        let intent = Self {
            status: MountEffectStatusV2::Pending,
    ...
    pub(crate) fn complete(mut self, receipt: Vec<u8>) -> Result<Self, AuthorizationRecordError> {
        if self.status != MountEffectStatusV2::Pending
            || receipt.is_empty()
            || receipt.len() > MAXIMUM_RECEIPT_BYTES
        {
            return Err(AuthorizationRecordError::InvalidPayload);
        }
        self.status = MountEffectStatusV2::Complete;
    ...
    #[must_use]
    pub(crate) const fn status(&self) -> MountEffectStatusV2 {
        self.status
    }
    ...
    #[must_use]
    #[cfg(test)]
    pub(crate) const fn maximum_request_bytes(&self) -> u32 {
    ...
    #[must_use]
    pub(crate) const fn admitted_wall_seconds(&self) -> i64 {
        self.admitted_wall_seconds
    }

    /// Returns the `CLOCK_BOOTTIME` nanoseconds sampled at admission.
    #[must_use]
    pub(crate) const fn admitted_boottime_nanoseconds(&self) -> u64 {
    ...
    fn validate(&self) -> Result<(), AuthorizationRecordError> {
        ...
        let receipt_valid = match self.status {
            MountEffectStatusV2::Pending => self.receipt.is_empty(),
            MountEffectStatusV2::Complete => {
...
/// payload bound.
pub(crate) fn seal_authorization_fence(
    ...
    fence.validate()?;
    let payload = encode_fence(fence)?;
    seal(
        mac_key,
        namespace,
        journal_key,
...
pub(crate) fn open_authorization_fence(
    mac_key: &NodeJournalMacKey,
    namespace: RecordNamespace,
    journal_key: &[u8],
    bytes: &[u8],
) -> Result<MountAuthorizationFenceV1, AuthorizationRecordError> {
    let payload = open(
        mac_key,
        namespace,
        journal_key,
    ...
    decode_fence(payload)
}

/// Encodes and authenticates one durable mount effect intent.
///
/// # Errors
///
/// Returns [`AuthorizationRecordError`] for an invalid intent, journal key, or
/// payload bound.
pub(crate) fn seal_effect_intent(
    mac_key: &NodeJournalMacKey,
    namespace: RecordNamespace,
    journal_key: &[u8],
    intent: &MountEffectIntentV2,
) -> Result<Vec<u8>, AuthorizationRecordError> {
    intent.validate()?;
    let payload = encode_effect(intent)?;
    seal(
        mac_key,
        namespace,
        journal_key,
...
/// Authenticates and decodes one durable mount effect intent.
///
/// Authentication covers the exact namespace and journal key and completes
...
/// Returns [`AuthorizationRecordError`] for malformed, unauthenticated,
/// misplaced, unknown-kind, or semantically invalid bytes.
pub(crate) fn open_effect_intent(
    mac_key: &NodeJournalMacKey,
    namespace: RecordNamespace,
    journal_key: &[u8],
    bytes: &[u8],
) -> Result<MountEffectIntentV2, AuthorizationRecordError> {
    let payload = open(
        mac_key,
        namespace,
        journal_key,
...
fn seal(
    ...
    bytes.extend_from_slice(AUTHENTICATED_MAGIC);
...
fn encode_fence(fence: &MountAuthorizationFenceV1) -> Result<Vec<u8>, AuthorizationRecordError> {
...
fn decode_fence(bytes: &[u8]) -> Result<MountAuthorizationFenceV1, AuthorizationRecordError> {
    let mut decoder = Decoder::new(bytes);
    if decoder.take::<8>()? != *FENCE_MAGIC || decoder.u16()? != FENCE_VERSION {
        return Err(AuthorizationRecordError::InvalidPayload);
    }
...
fn encode_effect(intent: &MountEffectIntentV2) -> Result<Vec<u8>, AuthorizationRecordError> {
    ...
    bytes.push(match intent.status {
        MountEffectStatusV2::Pending => 0,
        MountEffectStatusV2::Complete => 1,
    });
    bytes.push(verb_code(intent.verb));
...
fn decode_effect(bytes: &[u8]) -> Result<MountEffectIntentV2, AuthorizationRecordError> {
    let mut decoder = Decoder::new(bytes);
    if decoder.take::<8>()? != *EFFECT_MAGIC || decoder.u16()? != EFFECT_VERSION {
        return Err(AuthorizationRecordError::InvalidPayload);
    }
    let status = match decoder.u8()? {
        0 => MountEffectStatusV2::Pending,
        1 => MountEffectStatusV2::Complete,
    ...
    decoder.finish()?;
    let intent = MountEffectIntentV2 {
        status,
        request_id,
        transport_request_digest,
        request_digest,
...
const fn verb_code(verb: BrokerVerb) -> u8 {
    match verb {
        BrokerVerb::MountCreate => 1,
        BrokerVerb::MountInstall => 2,
        BrokerVerb::MountReplace => 3,
        BrokerVerb::MountDetach => 4,
        BrokerVerb::MountRelease => 5,
        _ => 0,
    }
}

fn decode_verb(code: u8) -> Result<BrokerVerb, AuthorizationRecordError> {
    match code {
        1 => Ok(BrokerVerb::MountCreate),
        2 => Ok(BrokerVerb::MountInstall),
        3 => Ok(BrokerVerb::MountReplace),
        4 => Ok(BrokerVerb::MountDetach),
        5 => Ok(BrokerVerb::MountRelease),
        _ => Err(AuthorizationRecordError::InvalidPayload),
    }
}
...
mod tests {
    ...
    fn mac_key() -> NodeJournalMacKey {
        NodeJournalMacKey::new([90; 16], [91; 32]).unwrap_or_else(|error| panic!("key: {error}"))
    }
    ...
    fn fence() -> MountAuthorizationFenceV1 {
        MountAuthorizationFenceV1::new(
            assignment(),
    ...
    fn wrong_secret_key_id_truncation_and_oversize_fail_closed() {
        ...
        let bytes =
            seal_authorization_fence(&key, RecordNamespace::DesiredState, b"fence-key", &fence())
                .unwrap_or_else(|error| panic!("seal: {error}"));
        let wrong_secret = NodeJournalMacKey::new([90; 16], [92; 32])
        ...
        let wrong_id = NodeJournalMacKey::new([89; 16], [91; 32])
            .unwrap_or_else(|error| panic!("key: {error}"));
        assert_eq!(
            open_authorization_fence(
                &wrong_id,
    ...
    fn payload_decoder_rejects_unknown_tags_trailing_bytes_and_bounds() {
        let valid_fence = encode_fence(&fence()).unwrap_or_else(|error| panic!("fence: {error}"));
        let mut trailing = valid_fence.clone();
        trailing.push(0);
        assert_eq!(
            decode_fence(&trailing),
            Err(AuthorizationRecordError::InvalidPayload)
        );
        ...
        assert_eq!(
            decode_effect(&oversized_receipt),
            Err(AuthorizationRecordError::InvalidPayload)
        );
    }
    ...
    fn sample_effect() -> Vec<u8> {
        ...
        encode_effect(&intent).unwrap_or_else(|error| panic!("effect: {error}"))
    }

    #[test]
    fn effect_round_trip_retains_intersection_and_receipt_fields() {
        let pending =
            decode_effect(&sample_effect()).unwrap_or_else(|error| panic!("decode: {error}"));
        assert_eq!(pending.status(), MountEffectStatusV2::Pending);
    ...
    fn invalid_keys_and_semantic_substitution_are_rejected() {
        assert!(matches!(
            NodeJournalMacKey::new([0; 16], [1; 32]),
            Err(AuthorizationRecordError::InvalidKey)
        ));
        assert!(matches!(
            NodeJournalMacKey::new([1; 16], [0; 32]),
            Err(AuthorizationRecordError::InvalidKey)
        ));
