...
use aos_sandbox_mount::spawn::{
    ATTACHMENT_ANCHOR_FD, DescriptorMapping, MOUNT_NAMESPACE_FD, OBSERVATION_FD, PLAN_FD,
    TARGET_ROOT_FD, TARGET_SLOT_FD, run_helper_status,
};
...
#[test]
fn launcher_rejects_a_missing_attachment_anchor() {
    let file = tempfile::tempfile().unwrap_or_else(|error| panic!("temporary file: {error}"));
    let mappings = [
        DescriptorMapping {
            target: PLAN_FD,
    ...
    assert!(run_helper_status(executable, &mappings).is_err());
}
