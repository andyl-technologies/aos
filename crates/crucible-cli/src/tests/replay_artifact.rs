...
"#,
    ...
    for retired in [
        ...
        "builtin:fault-campaign.fam",
    ] {
        let error = plan_fuzz_invocation(
            &FuzzArgs {
                family: Some(retired.to_owned()),
                ..FuzzArgs::default()
            },
            &seed_plan,
            temp.path(),
        ...
        .err()
        .ok_or("retired fuzz family alias unexpectedly resolved")?;
        assert!(matches!(error, CliError::InvalidScenario(_)));
        assert!(resolve_builtin_example_scenario(retired)?.is_none());
    }
