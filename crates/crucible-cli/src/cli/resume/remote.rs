//! Remote resume, run, save, and verification workflows.
...
pub(in super::super) fn finish_run_workflow_outcome(
    ...
    let captures_interactive_session = run_plan.execution_mode == RunExecutionMode::Interactive
        && report.execution_owner == RunExecutionOwner::Session;
    if (outcome.status.is_non_passing() || captures_interactive_session)
        && backend_plan.target == BackendExecutionTarget::Local
    {
        let producer = match (
            backend_plan.resolved_backend.as_ref(),
            report.execution_owner,
        ) {
            (Some(ResolvedLocalBackend::Qemu { .. }), RunExecutionOwner::Session) => "run",
            (_, RunExecutionOwner::Session) => "run",
            (_, RunExecutionOwner::Campaign) => "campaign-run",
        };
        ...
        let artifact = run_reproduction_artifact_bytes(
            artifact_seed,
            backend_plan.resolved_backend.as_ref(),
            producer,
            run_plan,
...
pub(in super::super) fn finish_verify_workflow_outcome(
    ...
    for witness in &report.witnesses {
        ...
        outcome.stdout.push(format!(
            "verify-run\tindex={}\trun={}\tprofile={}\tworkers={}\tcores={}\tscheduling_seed={}\tclock_skew_ms={}\tclock_coarsening_ms={}\tclock_backstep_every={}\tdeadline_backstep_applied={}\thost_io_stall_ms={}\tcanonical_log={}\tfingerprint={}\tartifact={}\tsamples={}\teffect_applied={}\tapplied_fault_bindings={}\tassertion_evaluated={}\tevaluated_assertions={}\tassertion_state_changed={}\tassertion_transitions={}",
            witness.reduction.index,
            witness.reduction.run_index,
            witness.reduction.host_profile.label(),
            witness.reduction.host_profile.executor_workers,
            witness.reduction.host_profile.logical_cores,
            witness.reduction.host_profile.scheduling_seed,
            witness.reduction.host_profile.wall_clock_skew_ms,
            witness.reduction.host_profile.wall_clock_coarsening_ms,
            witness.reduction.host_profile.wall_clock_backstep_every,
            witness
                .reduction
                .host_profile
                .applies_deadline_backstep(),
            witness.reduction.host_profile.host_io_stall_ms,
        ...
        outcome.canonical_log.push(CanonicalLogEntry {
            ...
            summary: format!(
                "index={} run={} profile={} workers={} cores={} scheduling_seed={} clock_skew_ms={} clock_coarsening_ms={} clock_backstep_every={} deadline_backstep_applied={} host_io_stall_ms={} canonical_log={} fingerprint={} artifact={} samples={} effect_applied={} applied_fault_bindings={} assertion_evaluated={} evaluated_assertions={} assertion_state_changed={} assertion_transitions={}",
                witness.reduction.index,
                witness.reduction.run_index,
                witness.reduction.host_profile.label(),
                witness.reduction.host_profile.executor_workers,
        ...
        if let Some(preemption) = witness.host_scheduler_preemption {
            outcome
                .host_scheduler_preemption
                .push(HostSchedulerPreemptionEvidence {
                    reduction_index: witness.reduction.index,
                    run_index: witness.reduction.run_index,
                    host_profile: witness.reduction.host_profile.label().to_owned(),
                    applied: preemption.applied,
                    pending_quantum_certified: preemption.pending_quantum_certified,
                    perturbations: preemption.perturbations,
                    requested_stopped_milliseconds: preemption.requested_stopped_milliseconds,
                });
        }
    }
