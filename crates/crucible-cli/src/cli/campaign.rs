...
pub(super) fn run_campaign_invocation(cli: &Cli, args: &CampaignArgs) -> Result<(), CliError> {
                ...
                let launch_identity = match (&worked.qemu, &worked.plugin) {
                    ...
                    _ => {
                        return Err(usage_error(
                            "worked-network QEMU and plugin must be supplied together",
                        ));
                    }
                };
    ...
    if let CampaignCommand::Debug(debug) = &args.command {
        ...
        let snapshot = CampaignSnapshotId::parse(&debug.snapshot)
            .map_err(|error| usage_error(format!("invalid campaign snapshot: {error}")))?;
        let finding = crucible_campaign::FindingId::parse(&debug.finding)
            .map_err(|error| usage_error(format!("invalid finding identity: {error}")))?;
        let request = crucible_daemon::OpenCampaignDebugSessionRequest::new(
            principal.clone(),
            campaign,
            snapshot,
            finding,
        )
        .map_err(|error| usage_error(format!("campaign debug request is invalid: {error}")))?;
        let response = service
            .open_campaign_debug_session(&request)
            .map_err(|error| {
                backend_error(format!("campaign debug session allocation failed: {error}"))
            })?;
        response
        ...
        let relay_args = DebugArgs {
            ...
            record_transcript: None,
            guest_idle_timeout: None,
            verb: Some(DebugVerbArgs::AttachGdb),
        };
        let plan = plan_debug_invocation(cli, &relay_args)?;
