...
#[test]
fn live_qemu_contract_rejects_previous_schema_without_compatibility() {
    let encoded = interactive_contract().encode();
    let stale = String::from_utf8(encoded)
        .unwrap_or_else(|error| panic!("contract fixture should be UTF-8: {error}"))
        .replacen(
            "crucible.live-qemu-replay-contract.v4",
            "crucible.live-qemu-replay-contract.v3",
            1,
        );
    let error = match LiveQemuReplayContract::decode(stale.as_bytes()) {
        Err(error) => error,
        Ok(_) => panic!("the previous live contract schema must fail closed"),
    };
