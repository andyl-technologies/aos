//! Process-boundary flight for packed-store transformation porcelain.
//!
...
#[test]
fn public_packed_transform_rewrites_sparse_pack_and_keeps_open_reader_valid()
-> Result<(), Box<dyn Error>> {
    ...
    let planned = fixture.run_transform("plan", &fixture.journal)?;
    ...
    let applied = fixture.run_transform("apply", &fixture.journal)?;
    ...
    let replayed = fixture.run_transform("apply", &fixture.journal)?;
    assert_eq!(replayed["plan"], planned["plan"]);
    assert_eq!(replayed["replayed"], true);

    let stale_journal = fixture.root.join("stale-journal");
    fixture.run_transform("plan", &stale_journal)?;
    ...
    let stale = fixture
        .transform_command("apply", &stale_journal)
        .output()?;
    assert!(!stale.status.success());
    assert!(String::from_utf8_lossy(&stale.stderr).contains("packed repack apply failed"));
...
impl PackedTransformFixture {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        let journal = root.join("repack-journal");
        fs::write(
            &store,
            format!(
...
"#
    ...
    fn transform_command(&self, operation: &str, journal: &Path) -> Command {
        ...
        command
            ...
            .arg(&self.store)
            .args(["--node", "packed", "--journal"])
            .arg(journal)
            .arg(operation);
        command
    }

    fn run_transform(&self, operation: &str, journal: &Path) -> Result<Value, Box<dyn Error>> {
        let output = self.transform_command(operation, journal).output()?;
...
fn parse_json_output(output: Output, operation: &str) -> Result<Value, Box<dyn Error>> {
    if !output.status.success() {
        return Err(format!(
            "store transform packed {operation} failed with {}; stdout=`{}` stderr=`{}`",
            output.status,
    ...
    let line = lines
        .next()
        .ok_or_else(|| format!("store transform packed {operation} returned empty stdout"))?;
    if lines.next().is_some() {
        return Err(
            format!("store transform packed {operation} returned multiple JSONL records").into(),
        );
    }
