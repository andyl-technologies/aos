...
fn assert_machine_readable_jsonl(
    ...
    assert_machine_readable_entries(&entries, expected_kinds, "jsonl stdout")?;
    Ok(())
}
...
fn assert_machine_readable_json(
    ...
    assert_machine_readable_entries(entries, expected_kinds, "json stdout")
}
...
fn assert_machine_readable_entries(
    ...
    assert!(
        final_summary.contains("exit_code=0"),
        "{context} final outcome summary must include exit_code=0, got `{final_summary}`",
    );
    Ok(())
}
