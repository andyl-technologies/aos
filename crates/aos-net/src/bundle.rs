...
//! registry git cache.
//!
//! Also supports uploading bundles to S3, HTTP, or filesystem destinations.
...
pub async fn download_bundle(
    ...
    tracing::info!("Downloading bundle: {}", entry.uri);
...
// ---------------------------------------------------------------------------
// Bundle upload support
// ---------------------------------------------------------------------------

/// Upload a bundle file to a destination URL.
///
/// Supports `s3://`, `https://`, `http://`, and `file://` destinations.
/// For S3, uses the S3 backend's `put_object` method.
/// For HTTP/HTTPS, uses PUT with the file content.
/// For file://, copies the bundle to the destination path.
pub async fn upload_bundle(
    bundle_path: &Path,
    destination: &str,
    auth: &AuthOptions,
) -> Result<()> {
    let content = tokio::fs::read(bundle_path)
        .await
        .with_context(|| format!("reading bundle {}", bundle_path.display()))?;

    let parsed = url::Url::parse(destination)
        .map_err(|e| anyhow::anyhow!("invalid destination URL '{destination}': {e}"))?;
    ...
    match parsed.scheme() {
        "s3" => {
            ...
            let s3 = crate::backend::s3::S3Backend::new(bucket, "", auth).await?;
            s3.put_object(key, &content, "application/octet-stream").await?;
        ...
        "http" | "https" => {
            ...
            let resp = req
                .send()
                .await
                .with_context(|| format!("uploading bundle to {destination}"))?;

            let status = resp.status();
            if !status.is_success() {
                let body = resp.text().await.unwrap_or_default();
                bail!("bundle upload failed (HTTP {status}): {body}");
            }

            tracing::info!("Uploaded bundle to {destination}");
        }
        "file" => {
            let dest_path = parsed
                .to_file_path()
                .map_err(|_| anyhow::anyhow!("invalid file URL: {destination}"))?;
            ...
            tokio::fs::write(&dest_path, &content)
                .await
                .with_context(|| format!("writing bundle to {}", dest_path.display()))?;

            tracing::info!("Copied bundle to {}", dest_path.display());
        }
        other => bail!("unsupported bundle upload scheme: {other}"),
    }
