...
fn target_label(target: &aos_proto_types::OperationTarget) -> String {
    ...
    match target.target.as_ref() {
        ...
        Some(Target::NetworkBoundaryId(id)) => format!("boundary:{id}"),
        Some(Target::DeliveryEndpointId(id)) => format!("endpoint:{id}"),
        Some(Target::StorageGatewayId(id)) => format!("gateway:{id}"),
        Some(Target::DeliveryRouteId(id)) => format!("route:{id}"),
        ...
        Some(Target::StorageBindingId(id)) => format!("storage-binding:{id}"),
