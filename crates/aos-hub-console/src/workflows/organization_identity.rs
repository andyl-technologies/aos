...
fn ServiceAccounts(client: ApiClient, organization: String) -> impl IntoView {
    ...
    view! {
        <div class="workflow-stack">
            ...
            <ServiceAccountCreate client=client organization=create_org/>
        </div>
    }
}
...
fn ServiceAccountCard(
    ...
    view! {
        <details class="binding-card"><summary><div><span class="resource-kind">"service account"</span><h3>{account.name}</h3><code>{format!("principal id {}", account.id)}</code></div><StatusBadge state="active".to_string() positive=true/></summary><div class="binding-details"><div class="resource-identity"><div><span>"Version"</span><code>{account.resource_version}</code></div></div><div class="subworkflow-grid"><section class="subworkflow"><h4>"Rename"</h4><form class="stacked-form" on:submit=on_update><label><span>"New name"</span><input required prop:value=move || new_name.get() on:input=move |event| new_name.set(event_target_value(&event))/></label><button class="secondary-button" type="submit" disabled=move || busy.get()>"Review rename"</button></form><PlanReview pending=update_pending error=error busy=busy on_apply=update_apply/></section><section class="subworkflow danger-subworkflow"><h4>"Delete service account"</h4><p>"Deletion also removes this principal's direct memberships."</p><button class="danger-button" type="button" disabled=move || busy.get() on:click=on_delete>"Review deletion"</button><PlanReview pending=delete_pending error=error busy=busy on_apply=delete_apply/></section></div></div></details>
    }
}
...
fn OrganizationMembers(client: ApiClient, organization: String) -> impl IntoView {
    view! {
        <div class="workflow-stack">
            <MembershipLookup client=client.clone() organization=organization.clone()/>
            <Invitations client=client organization=organization/>
        </div>
    }
}

#[component]
fn MembershipLookup(client: ApiClient, organization: String) -> impl IntoView {
    ...
    view! {
        <section class="panel editor-panel"><p class="section-kicker">"Exact grants"</p><h2>"Memberships"</h2><p>"Load one direct principal/scope grant before replacing or removing it."</p><form class="editor-form" on:submit=on_lookup><label><span>"Principal kind"</span><select prop:value=move || principal_kind.get() on:change=move |event| principal_kind.set(event_target_value(&event))><option value="user">"User"</option><option value="service_account">"Service account"</option></select></label><label><span>"Principal reference"</span><input required prop:value=move || principal_ref.get() on:input=move |event| principal_ref.set(event_target_value(&event))/></label><label><span>"Canonical scope"</span><input prop:value=move || scope.get() on:input=move |event| scope.set(event_target_value(&event))/></label><div class="form-actions"><button class="button" type="submit" disabled=move || busy.get()>"Load membership"</button></div></form>{move || error.get().map(|detail| view! { <InlineError detail=detail/> })}{move || loaded.get().map(|membership| view! { <MembershipEditor client=client.clone() membership=membership/> })}</section>
    }
}
...
fn Invitations(client: ApiClient, organization: String) -> impl IntoView {
    ...
    view! {
        <section class="panel resource-panel">
            ...
            <InvitationCreate client=client organization=create_org/>
        </section>
    }
}
...
#[component]
fn InvitationRow(client: ApiClient, invitation: aos_proto_types::Invitation) -> impl IntoView {
    let pending = RwSignal::new(None::<PendingPlan>);
    let error = RwSignal::new(None::<String>);
    let busy = RwSignal::new(false);
    let cancellable = invitation.state == "pending";
