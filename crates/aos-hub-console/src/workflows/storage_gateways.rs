...
//!
//! Gateways bind one storage binding to an exact delivery-endpoint generation.
...
impl GatewayScope {
    ...
    fn binding_ref(&self, binding_name: &str) -> aos_proto_types::StorageBindingRef {
        use aos_proto_types::storage_binding_ref::Target;
        ...
        aos_proto_types::StorageBindingRef {
            target: Some(target),
        }
    }
}
                        ...
                        Suspend::new(async move {
                            match inventory.await.as_ref() {
                                Ok(groups) if groups.iter().all(|group| group.gateways.is_empty()) => {
                                    view! { <p class="muted">"No storage gateways in this scope."</p> }.into_any()
                                }
                                Ok(groups) => view! {
...
fn GatewayCreateForm(
    ...
    let on_plan = move |event: SubmitEvent| {
        ...
        let request = aos_proto_types::PlanStorageGatewayMutationRequest {
        ...
        spawn_local(async move {
            let result = client
                .call::<_, aos_proto_types::TopologyPlanResponse>(
                    aos_proto_types::DELIVERY_SERVICE_PLAN_CREATE_STORAGE_GATEWAY_PATH,
                    &request,
                )
                .await
                .map_err(|failure| failure.to_string())
    ...
    let on_apply = Callback::new(move |()| {
        ...
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::StorageGatewayResponse>(
                    aos_proto_types::DELIVERY_SERVICE_CREATE_STORAGE_GATEWAY_PATH,
                    &reviewed.storage_gateway_apply(),
                )
                .await
            {
                Ok(_) => reload(),
...
#[component]
fn GatewayCard(client: ApiClient, gateway: aos_proto_types::StorageGateway) -> impl IntoView {
        ...
        <details class="binding-card">
            <summary>
                <div>
                    ...
                    <code>{format!("{} → {}@{}", desired.storage_binding_id, desired.endpoint_id, desired.endpoint_generation)}</code>
                </div>
...
#[component]
fn GatewayUpdate(client: ApiClient, gateway: aos_proto_types::StorageGateway) -> impl IntoView {
    ...
    let binding_id = desired.storage_binding_id;
    ...
    let on_plan = move |event: SubmitEvent| {
        ...
        spawn_local(async move {
            let result = client
                .call::<_, aos_proto_types::TopologyPlanResponse>(
                    aos_proto_types::DELIVERY_SERVICE_PLAN_UPDATE_STORAGE_GATEWAY_PATH,
                    &request,
                )
                .await
                .map_err(|failure| failure.to_string())
    ...
    let on_apply = Callback::new(move |()| {
        ...
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::StorageGatewayResponse>(
                    aos_proto_types::DELIVERY_SERVICE_UPDATE_STORAGE_GATEWAY_PATH,
                    &reviewed.storage_gateway_apply(),
                )
                .await
            {
...
#[component]
fn GatewayGrants(client: ApiClient, gateway: aos_proto_types::StorageGateway) -> impl IntoView {
    ...
    let on_plan = move |event: SubmitEvent| {
        ...
        spawn_local(async move {
            let result = client
                .call::<_, aos_proto_types::TopologyPlanResponse>(
                    aos_proto_types::DELIVERY_SERVICE_PLAN_GRANT_STORAGE_GATEWAY_SCOPE_PATH,
                    &request,
                )
                .await
                .map_err(|failure| failure.to_string())
        ...
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::ConsumerScopeGrantResponse>(
                    aos_proto_types::DELIVERY_SERVICE_GRANT_STORAGE_GATEWAY_SCOPE_PATH,
...
fn GatewayGrantRow(client: ApiClient, grant: aos_proto_types::ConsumerScopeGrant) -> impl IntoView {
    ...
    let on_plan = move |_| {
        ...
        spawn_local(async move {
            let result = client
                .call::<_, aos_proto_types::TopologyPlanResponse>(
                    aos_proto_types::DELIVERY_SERVICE_PLAN_REVOKE_STORAGE_GATEWAY_SCOPE_PATH,
                    &request,
                )
                .await
                .map_err(|failure| failure.to_string())
    ...
    let on_apply = Callback::new(move |()| {
        ...
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::ConsumerScopeGrantResponse>(
                    aos_proto_types::DELIVERY_SERVICE_REVOKE_STORAGE_GATEWAY_SCOPE_PATH,
                    &reviewed.consumer_grant_apply(),
                )
                .await
            {
...
#[component]
fn GatewayState(client: ApiClient, gateway: aos_proto_types::StorageGateway) -> impl IntoView {
    let pending = RwSignal::new(None::<PendingPlan>);
    let error = RwSignal::new(None::<String>);
    let busy = RwSignal::new(false);
    ...
    let on_plan = move |_| {
        ...
        let path = if enabling {
            aos_proto_types::DELIVERY_SERVICE_PLAN_ENABLE_STORAGE_GATEWAY_PATH
        } else {
            aos_proto_types::DELIVERY_SERVICE_PLAN_DISABLE_STORAGE_GATEWAY_PATH
        };
        busy.set(true);
        error.set(None);
        spawn_local(async move {
    ...
    let on_apply = Callback::new(move |()| {
        ...
        let client = client.clone();
        let path = if enabling {
            aos_proto_types::DELIVERY_SERVICE_ENABLE_STORAGE_GATEWAY_PATH
        } else {
            aos_proto_types::DELIVERY_SERVICE_DISABLE_STORAGE_GATEWAY_PATH
        };
        busy.set(true);
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::StorageGatewayResponse>(path, &reviewed.delete_apply())
                .await
            {
                Ok(_) => reload(),
                Err(failure) => error.set(Some(failure.to_string())),
...
#[component]
fn GatewayDelete(client: ApiClient, gateway: aos_proto_types::StorageGateway) -> impl IntoView {
    let pending = RwSignal::new(None::<PendingPlan>);
    let error = RwSignal::new(None::<String>);
    let busy = RwSignal::new(false);
    let stable_id = gateway.stable_id;
    ...
    let on_plan = move |_| {
        ...
        spawn_local(async move {
            let result = client
                .call::<_, aos_proto_types::TopologyPlanResponse>(
                    aos_proto_types::DELIVERY_SERVICE_PLAN_DELETE_STORAGE_GATEWAY_PATH,
                    &request,
                )
                .await
                .map_err(|failure| failure.to_string())
        ...
        spawn_local(async move {
            match client
                .call::<_, aos_proto_types::DeleteTopologyResourceResponse>(
                    aos_proto_types::DELIVERY_SERVICE_DELETE_STORAGE_GATEWAY_PATH,
...
fn gateway_revision(
...
) -> Result<aos_proto_types::StorageGatewayRevisionSpec, String> {
    ...
    Ok(aos_proto_types::StorageGatewayRevisionSpec {
        storage_binding_id: required(storage_binding_id.to_string(), "Storage binding ID")?,
...
fn gateway_grant_request(
    ...
    aos_proto_types::PlanConsumerScopeGrantRequest {
        resource_kind: "storage_gateway".to_string(),
