    ...
    view! {
        <section class="panel resource-panel">
            ...
            <form class="editor-form" on:submit=on_submit>
                ...
                <label><span>"Target"</span><select prop:value=move || filters.target.get() on:change=move |event| filters.target.set(event_target_value(&event))><option value="">"Any target"</option><option value="bare-metal">"Bare metal"</option><option value="qemu-kvm">"QEMU/KVM"</option><option value="openstack">"OpenStack"</option><option value="vmware">"VMware"</option><option value="hyper-v">"Hyper-V"</option></select></label>
