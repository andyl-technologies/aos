    ...
    pub fn decode(
        ...
        if operation_id == creation_operation_id {
            return Err(ProtocolValidationError::InvalidField(
                "guest root effect operation",
            ));
        }

        let mut canonical_bytes = [0_u8; CANONICAL_BYTES];
        canonical_bytes[..8].copy_from_slice(MAGIC);
        canonical_bytes[8..24].copy_from_slice(fence.sandbox_id());
        canonical_bytes[24..40].copy_from_slice(fence.incarnation_id());
        canonical_bytes[40..48].copy_from_slice(&fence.assignment_epoch().to_be_bytes());
        canonical_bytes[48..56].copy_from_slice(&fence.desired_generation().to_be_bytes());
        canonical_bytes[56..88].copy_from_slice(fence.assignment_digest());
        canonical_bytes[88..104].copy_from_slice(&operation_id);
        canonical_bytes[104..136].copy_from_slice(&workspace_handle);
        canonical_bytes[136..152].copy_from_slice(&creation_operation_id);
