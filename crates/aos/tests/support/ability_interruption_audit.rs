...
struct MatrixMethod {
    method: String,
    required_target_access: String,
}

#[derive(Deserialize)]
struct Applicability {
    schema: String,
    applicable_cell_ids: Vec<String>,
    inapplicable_cells: Vec<Value>,
}
...
fn checked_plan(
    ...
    let mut selected_interfaces = vec![interface.clone()];
    for method in interface.interface.methods.values() {
        ...
        let target = interfaces
            .values()
            .find(|candidate| candidate.interface.name == method.target_resource)
            .with_context(|| format!("missing target interface {}", method.target_resource))?;
        if !selected_interfaces.contains(target) {
            selected_interfaces.push(target.clone());
        }
    }
...
fn schema_accepts_literal(schema: &ValueSchema, candidate: &Value) -> Result<bool> {
