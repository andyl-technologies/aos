...
pub(super) async fn run(printer: &Printer, command: &HubDeliveryCmd) -> Result<()> {
    match command {
        ...
        HubDeliveryCmd::Show { access, workflow } => {
            ...
            topology_read(printer, &client, hub_rpc::GetDeliveryWorkflow,
                &pb::GetDeliveryWorkflowRequest { workflow_id: workflow.clone() }).await
        }
        HubDeliveryCmd::List { access, surface, pagination } => {
            let client = hub_client(&access.hub, access.token.as_deref())?;
            topology_read(printer, &client, hub_rpc::ListDeliveryWorkflows,
        ...
        HubDeliveryCmd::Resume { request, workflow, if_version } => {
            let client = hub_client(&request.access.hub, request.access.token.as_deref())?;
            topology_read(printer, &client, hub_rpc::ResumeDeliveryDestination,
        ...
        HubDeliveryCmd::Activate { command } => match command {
            HubDeliveryActivationCmd::Plan { request, workflow, if_version } => {
                let client = hub_client(&request.access.hub, request.access.token.as_deref())?;
                topology_read(printer, &client, hub_rpc::PlanActivateDeliveryDestination,
...
async fn apply_reviewed(printer: &Printer, apply: &HubReviewedApplyArgs, activate: bool) -> Result<()> {
    ...
    if activate {
        topology_read(printer, &client, hub_rpc::ActivateDeliveryDestination, &request).await
    } else {
        topology_read(printer, &client, hub_rpc::ApplyDeliveryDestination, &request).await
    }
}
...
fn read_intent(path: &Path) -> Result<pb::DeliveryDestinationIntent> {
    ...
    anyhow::ensure!(bytes.len() as u64 <= MAX_BYTES, "delivery intent exceeds 1 MiB");
