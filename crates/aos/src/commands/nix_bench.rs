...
pub(super) fn fresh_isolated_candidate(
    ...
    // populated and reused within this run. Enable the in-process content-memo
    // stack and the worker-local Ready directory, but detach persistent and
    // additive disk/network locations. Serial evaluation constructs L0 and
    // Ready state only; L1 becomes active when the evaluation has a shared
    // parallel-demand context. Durable cache population is a separate benchmark
    // axis: including it here measures cross-run serialization and writeback
    // rather than the evaluator's legal same-run reuse.
