...
//! mysql case also interrupts the production baseline at several DDL and seed
//! boundaries, then reopens it through the normal migration path.
//!
...
/// Recovers interrupted baseline installation at table, index, and seed boundaries.
#[cfg(feature = "mysql")]
async fn assert_mysql_baseline_crash_replay_and_concurrent_start(url: &str) {
    use aos_hub_core::db::migration_statements;
    ...
    let first_index = statements
        .iter()
        .position(|sql| sql.starts_with("CREATE INDEX ") || sql.starts_with("CREATE UNIQUE INDEX "))
        .expect("baseline indexes");
    let first_seed = statements
        .iter()
        .position(|sql| sql.starts_with("INSERT INTO "))
        .expect("baseline seed data");
    let cuts = [
        1,
        first_index,
        first_index + 1,
    ...
    for cut in cuts {
        ...
        for statement in &statements[..cut] {
            let translated = Dialect::Mysql.translate(statement).unwrap();
            sqlx::query(&translated.sql)
                .execute(&mut connection)
                .await
                .unwrap_or_else(|error| {
                    panic!("baseline interruption at {cut}: {error}; {statement}")
                });
        }
        drop(connection);
        ...
        let recovered = Database::connect(url)
            .await
            .unwrap_or_else(|error| panic!("baseline recovery at {cut}: {error:#}"));
        drop(recovered);
        drop(
            Database::connect(url)
                .await
                .expect("reopen recovered baseline"),
        );
    }

    reset_mysql_schema(url).await;
    let (first, second) = tokio::join!(Database::connect(url), Database::connect(url));
    first.expect("first concurrent baseline installer");
    second.expect("second concurrent baseline installer");

    // Both markers commit together; a missing portable marker is corruption,
    // not an interrupted DDL state that a later startup may silently adopt.
    let mut connection = MySqlConnection::connect(url).await.unwrap();
    sqlx::query("DELETE FROM schema_version")
        .execute(&mut connection)
        .await
        .unwrap();
    let error = Database::connect(url)
        .await
        .err()
        .expect("missing portable marker must fail closed");
    assert!(
        error
            .to_string()
            .contains("portable schema version marker is missing"),
        "{error:#}"
    );
}
