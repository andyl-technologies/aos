...
fn metrics_measure_synchronous_and_deferred_host_latency() {
    let (synchronization, controller) = OperationSynchronization::start();
    let child = Arc::new(MemoryBlobBackend::new("latency-child", 1_024));
    let synchronized = Arc::new(SynchronizedMetricsBackend {
        child: child.clone(),
        synchronization: Arc::clone(&synchronization),
    });
    let (store, state) = MetricsStore::new("latency-metrics", synchronized.clone());
    ...
    let snapshot = state.snapshot();
    assert!(snapshot.contains_elapsed_nanoseconds > 0);
    assert!(snapshot.read_elapsed_nanoseconds > 0);
    assert!(snapshot.read_stream_open_elapsed_nanoseconds > 0);
    assert!(snapshot.read_stream_read_elapsed_nanoseconds > 0);
    assert!(snapshot.put_elapsed_nanoseconds > 0);

    drop(store);
    drop(synchronized);
    drop(synchronization);
    controller.join().expect("join operation synchronization");
}
