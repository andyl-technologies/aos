...
fn stopped_owner_repairs_one_physical_copy_from_an_independent_source() {
    ...
    let repaired = admin
        .repair_physical_copy(&source, &target, id, 1_024)
    ...
    assert_eq!(
        ...
        StorePhysicalRepairDisposition::ReplacedCorrupt
    );
    assert_eq!(
    ...
    assert_eq!(
        replay.disposition(),
        StorePhysicalRepairDisposition::AlreadyAuthenticated
    );
    assert!(matches!(
        admin.repair_physical_copy(&source, &source, id, 1_024),
