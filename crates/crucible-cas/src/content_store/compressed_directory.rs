...
//!
//! Each logical object retains the ordinary digest-shard path and full content
//! identity filename while its physical file uses this private representation:
//!
//! ```text
//! +------------------+----------------------+-----------------------+
...
fn visit_compressed_inventory(
    ...
    visit_directory_objects(backend.root(), &mut |path, id| {
        let (_, header) = open_compressed_object(path, id, backend.maximum_logical_object_bytes)?;
