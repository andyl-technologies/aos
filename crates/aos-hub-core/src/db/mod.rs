...
pub struct TopologyOperationTargetRecord {
    ...
    pub control_permission: crate::domain::Permission,
...
pub struct IndexSnapshot {
    ...
    /// ([`crate::stack::StackNode::to_json`]), or `None` when the registry has
...
impl Database {
    ...
    pub(super) fn topology_event_insert_statement(event: &NewTopologyEvent<'_>) -> Statement {
        Statement::new(
    ...
    pub(super) fn topology_event_statement(event: &NewTopologyEvent<'_>) -> CheckedStatement {
    ...
    fn operational_webhook_event_insert_statement(
        ...
        anyhow::ensure!(
            crate::webhook::is_supported_event_type(event_name)
                && crate::webhook::is_safe_event_header_value(event_name),
    ...
    pub async fn register_registry(
        ...
        let id = (incarnation.as_u128() & i64::MAX as u128) as i64;
    ...
    async fn apply_snapshot_transaction(
        ...
        if let Some(stack_json) = snapshot.cache_stack.as_deref() {
            let stack = aos_registry_surface::stack::StackNode::from_json(stack_json)?;
    ...
    async fn assert_registry_index_mutation_source(
        &self,
        registry_id: i64,
        indexed_placement_id: Option<i64>,
    ) -> Result<()> {
    ...
    async fn channel_floor_guard_statements(
        &self,
        registry_id: i64,
        channels: &[ChannelSummary],
    ...
    pub async fn set_registry_mirror(
        ...
        crate::url_guard::is_safe_remote_url(source_url)
    ...
    /// Lists coordinated boundary activations eligible for a controller claim.
    ///
    /// # Errors
    ///
    /// Returns an error on database failure.
    ...
    async fn numeric_operation_target_scope(
        &self,
        table: &str,
        stable_id: &str,
    ) -> Result<Option<String>> {
    ...
    pub async fn topology_operation_targets(
        ...
        let mut targets = vec![TopologyOperationTargetRecord {
            ...
            control_permission: crate::auth::permission_from_str(&operation.control_permission)
        ...
        let mut secondary = self
            ...
            .map(|row| {
                Ok(TopologyOperationTargetRecord {
                    ...
                    control_permission: crate::auth::permission_from_str(&row.get::<String>(5)?)
    ...
    /// Claims a pending or stale-running coordinated boundary activation.
    ///
    /// # Errors
    ///
    /// Returns an error for an invalid lease or database failure.
    pub async fn claim_network_boundary_coordination_operation(
        ...
        if lease_seconds <= 0 {
            bail!("boundary coordination claim lease must be positive");
        }
        let now = unix_now();
        let changed = self
            .backend
    ...
    fn registry_image_presence_statements(
        registry_id: i64,
        placement_id: i64,
        objects: &[VerifiedRegistryImageObject],
        observed_at: i64,
    ...
    async fn system_image_delivery_ready(
        &self,
        registry_id: i64,
    ...
    /// plan conflict.
    pub(crate) async fn create_project_from_plan(
        &self,
        org_id: i64,
        path: &str,
        name: &str,
    ...
    async fn create_project_with_plan(
        &self,
        org_id: i64,
        path: &str,
        name: &str,
        ...
        // idempotency identity; the positive 63-bit integer is only the local
        // relational key.
        let project_id = (incarnation.as_u128() & i64::MAX as u128) as i64;
    ...
    ///
    /// Returns an error on database failure.
    pub(crate) async fn project_matches_creation_plan(
        &self,
        project_id: i64,
        plan_id: &str,
    ) -> Result<bool> {
    ...
    pub async fn authorization_context(
        &self,
        scope_key: &str,
    ) -> Result<Option<crate::domain::iam::AuthorizationContext>> {
        let Some(target) = crate::domain::Scope::try_parse(scope_key) else {
            return Ok(None);
        };
        let rows = self
            .backend
        ...
        let stored = rows
            ...
            .map(|row| {
                ...
                let scope = crate::domain::Scope::try_parse(&raw)
        ...
        while let Some(key) = cursor {
            ...
            let parsed = crate::domain::Scope::try_parse(&key)
        ...
        let context = crate::domain::iam::AuthorizationContext::try_new(target, ancestors)
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn delete_project_at_version(
        &self,
        org_id: i64,
        project_id: i64,
        expected_version: i64,
    ...
    async fn upsert_live_membership_role(
        &self,
        principal_kind: &str,
        principal_id: i64,
        scope: &str,
    ...
    pub async fn grant_membership(
        ...
        if crate::domain::PrincipalKind::parse(principal_kind).is_none() {
        ...
        if role != crate::domain::Role::Owner.as_str() {
            return self
    ...
    pub async fn bootstrap_root(&self, email: &str, plaintext: &str) -> Result<(String, i64)> {
        ...
        self.grant_membership(
            "user",
            user_id,
            crate::domain::Scope::root().as_str(),
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn create_managed_registry_from_plan(
        &self,
        org_id: i64,
        project_path: &str,
        name: &str,
    ...
    #[allow(clippy::too_many_arguments)]
    async fn create_managed_registry_with_plan(
        &self,
        org_id: i64,
        project_path: &str,
        name: &str,
    ...
    pub async fn create_binary_cache(
        ...
        let incarnation = uuid::Uuid::new_v4();
        let cache_id = (incarnation.as_u128() & i64::MAX as u128) as i64;
    ...
    pub async fn set_stable_topology_defaults(
        ...
        let valid_scope_shape = match (scope_kind, org_id) {
            ...
            ("organization", Some(_)) => {
                crate::domain::Scope::is_canonical(scope_key) && scope_key.starts_with("org:")
            }
            _ => false,
        };
    ...
    pub async fn set_storage_binding_credential_revision(
        ...
        crate::secret_version::validate_secret_version_ref(secret_version_ref)?;
        anyhow::ensure!(
    ...
    /// [`SecretSealer`](crate::auth::seal::SecretSealer), then stored as the
    ...
    pub async fn create_token(
        ...
        if permissions.is_empty()
            || permissions
                .iter()
                .any(|permission| !crate::domain::iam::allow(&grants, *permission, &context))
        {
    ...
    pub async fn validate_token(&self, secret: &str) -> Result<Option<TokenAuth>> {
        ...
        let permissions: Vec<_> = parse_permission_names(&perms_json)
            .into_iter()
            .filter(|permission| crate::domain::iam::allow(&grants, *permission, &context))
            .collect();
        if permissions.is_empty() {
            return Ok(None);
        }
    ...
    pub async fn approve_device(
        ...
        _approver_grants: &[(crate::domain::Scope, crate::domain::Role)],
    ) -> Result<bool> {
        let now = unix_now();
        if !self
        ...
        let granted: Vec<crate::domain::Permission> = requested
            .into_iter()
            .filter(|perm| crate::domain::iam::allow(&current_grants, *perm, &requested_context))
            .collect();
        if granted.is_empty() {
            return Ok(false);
        }
    ...
    /// `expected_version`.
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn apply_registry_configuration_change(
        &self,
        registry_id: i64,
        expected_version: i64,
        visibility: &str,
    ...
    pub async fn seed_webhook_for_test(
        ...
        crate::url_guard::is_safe_remote_url(url).context("rejecting webhook URL")?;
        crate::secret_version::validate_secret_version_ref(secret_version_ref)?;
        anyhow::ensure!(
            credential_fingerprint.len() == 64
                && credential_fingerprint
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn create_webhook_from_plan(
        &self,
        org_id: i64,
        url: &str,
        secret_version_ref: &str,
        ...
        let org_scope: String = self
            ...
            .get(0)?;
        let id = (uuid::Uuid::new_v4().as_u128() & i64::MAX as u128) as i64;
        let now = unix_now();
    ...
    ///
    /// Returns an error on database failure.
    pub(crate) async fn webhook_by_creation_plan(
        &self,
        plan_id: &str,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn delete_webhook_at_version(
        &self,
        id: i64,
        expected_version: i64,
        change_id: &str,
    ...
    pub async fn seed_delivery_for_test(
        ...
        anyhow::ensure!(
            crate::webhook::is_safe_event_header_value(event),
    ...
    pub async fn materialize_topology_events_with_delivery_ids(
        ...
        for row in rows {
            ...
            anyhow::ensure!(
                crate::webhook::is_safe_event_header_value(&event.event_name)
                    && crate::webhook::is_supported_event_type(&event.event_name),
    ...
    async fn delivery_for_claim(&self, id: i64, claim_token: &str) -> Result<Option<DueDelivery>> {
        let row = self
            .backend
            .query_opt(
                "SELECT d.id, d.delivery_id, d.webhook_id, d.event, d.payload, d.attempts, w.url,
...
fn operation_target_control_permission(
    target_kind: &str,
    primary_permission: crate::domain::Permission,
...
"#,
    ...
    fn signed_image_release_snapshot(
        package: &aos_registry_surface::manifest::PackageToml,
        release: &str,
        commit: &str,
        tag_oid: &str,
