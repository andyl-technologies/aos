            ...
            Statement::new(
                "INSERT INTO oci_quota_reservations
                 ...
                 WHERE repository.id = ?4 AND repository.registry_id = ?5
                   ...
                   AND (CAST(?6 AS TEXT) IS NULL OR EXISTS (SELECT 1 FROM oci_publication_sessions publication
...
fn release_upload_statements(
    ...
    let ownership = if writer_id.is_some() {
        ...
        "AND CAST(?2 AS TEXT) IS NULL AND CAST(?3 AS TEXT) IS NULL AND CAST(?4 AS BIGINT) IS NULL"
    };
