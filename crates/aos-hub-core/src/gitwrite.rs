...
/// Propose a change request adding or removing one advertised cache URL in a
/// registry's committed `registry.toml` `[[caches]]` list.
///
/// This is the write-through that makes a managed cache's *advertise* link
/// reach consumers (RFC-0004): the committed `[[caches]]` list is the single
/// source of truth a consumer resolves, so advertising a cache means proposing
/// a `[[caches]]` edit through the same change-request flow as
/// [`propose_config_change`] (a maintainer promotes it with `apr change merge`).
///
/// With `advertise = true` the cache's `url` is added; with `false` it is
/// removed. The edit is computed against the registry's current committed
/// `registry.toml` (read through `fetch` at the last-indexed commit) by
/// [`crate::web::config_form::add_cache_to_toml`] /
/// [`remove_cache_from_toml`](crate::web::config_form::remove_cache_from_toml),
/// which preserve `[registry]` metadata, an advanced `[cache_stack]`, and any
/// unmodeled keys.
///
/// **Idempotent:** returns `Ok(None)` when the committed file already reflects
/// the desired state (the URL is already present for an add, or already absent
/// for a remove), so no redundant change request is created.
///
/// # Errors
///
/// Returns an error when the registry has no indexed base commit to branch
/// from, when reading the committed file fails, when the rebuilt file fails
/// schema validation, or when writing the change request fails.
#[allow(clippy::too_many_arguments)]
pub async fn propose_cache_advertisement(
    db: &Database,
    sealer: &dyn SecretSealer,
    fetch: &dyn SurfaceFetch,
    writer: &dyn SurfaceWrite,
    ...
    let base_commit_hex = db
        ...
        .await?
        .and_then(|status| status.last_indexed_commit);
    let Some(base_commit_hex) = base_commit_hex else {
        // No committed config yet. There is nothing to de-advertise from, so a
        // removal is a clean no-op; advertising, though, needs a base commit to
        // branch the change request from.
        if advertise {
            bail!(
                "registry '{}' has no indexed config to advertise a cache into; \
                 publish or index it first",
                registry.slug
            );
        }
        return Ok(None);
    ...
    let base_commit = Oid::from_hex(&base_commit_hex)?;
    let current = crate::git::load_committed_file(fetch, base_commit, "registry.toml")
        .await?
        .unwrap_or_default();

    // The committed [registry].name fallback for a (defensively handled) bare
    // file — an indexable registry already has one, so this rarely applies.
    let name_fallback = registry.slug.rsplit('/').next().unwrap_or(&registry.slug);
    let rebuilt = if advertise {
        crate::web::config_form::add_cache_to_toml(&current, name_fallback, cache_url)?
    } else {
        crate::web::config_form::remove_cache_from_toml(&current, name_fallback, cache_url)?
    };
    let Some(new_contents) = rebuilt else {
        // Already in the desired state — nothing to propose.
        return Ok(None);
    };

    let proposed = propose_config_change(
        db,
        sealer,
        fetch,
        writer,
