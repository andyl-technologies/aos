//! Command entry point for one-time storage provisioning resources.
...
use aos_block_storage_provider::storage_provisioning::StorageProvisioningBackend;
use aos_provider_protocol::{HANDLER_ABI_ARGUMENT, MAX_HANDLER_RESULT_BYTES};

fn main() {
    if let Err(error) = run() {
        eprintln!("aos-storage-provisioning-provider: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    let output = if arguments[1] == "observe-root" {
        let response = observe_root(BlockStorageRootRole::Provisioning, &input)?;
        aos_contract::canonical::canonical_json(&serde_json::to_value(response)?)?
    } else {
        Provider::new(StorageProvisioningBackend).handle(&arguments[1], &input)?
    };
