//! Command entry point for OpenZFS storage-pool resources.
...
use aos_block_storage_provider::zfs_pool::ZfsPoolBackend;
use aos_provider_protocol::{HANDLER_ABI_ARGUMENT, MAX_HANDLER_RESULT_BYTES};

fn main() {
    if let Err(error) = run() {
        eprintln!("aos-zfs-pool-provider: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    let output = Provider::new(ZfsPoolBackend).handle(&arguments[1], &input)?;
