//! Command entry point for encrypted block-device mappings.

use std::io::{self, Read as _, Write as _};

use anyhow::{Context as _, Result, bail};
use aos_ability_model::ABILITY_LIMITS_V1;
use aos_block_storage_provider::cryptsetup::CryptsetupBackend;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("aos-cryptsetup-provider: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    let output = Provider::new(CryptsetupBackend).handle(&arguments[1], &input)?;
