...
use crate::execution::CancelAttemptExecutionRequest;
use crate::{
    AssignmentId, AttemptResourceLimits, CampaignCommandId, CancelAttemptExecutionDisposition,
    CheckpointAttemptExecutionDisposition, CheckpointAttemptExecutionRequest, ExactCheckpointId,
    ExecutionId, ExecutionRetentionIntent, ExecutorClient, ExecutorClientError,
    ExecutorControlService, ExecutorRejection, ExecutorResumeService, ExecutorStatusService,
    GetAttemptExecutionDisposition, GetAttemptExecutionRequest, GetAttemptExecutionResponse,
    ResumeAttemptExecutionDisposition, ResumeAttemptExecutionRequest, SavepointCaptureOutcome,
    SavepointCaptureRequest, SavepointCaptureResolution,
};
...
impl<S> CampaignExecutorDriver<S> {
    ...
    fn request_for(
        ...
        let request = match source {
            Some(source) => SubmitAttemptRequest::new(
                assignment,
                ...
                self.resources,
                self.retention,
                crate::AttemptRetentionPolicyDisposition::Required(retention_policy_basis),
            )
            .and_then(|assignment| {
        ...
        S: ExecutorResumeService,
    {
        ...
        let resumed_assignment = SubmitAttemptRequest::new(
            ...
            self.resources,
            self.retention,
            prior_request.retention_policy(),
        )?;
        let request =
