...
//! and decomposed PUCT input so an authority-free planner can validate its
//! interpretation and derive the score using the by-value active policy.
...
impl PlannerCandidateGuidance {
    ...
    /// with the PUCT predicates, finding counts are empty or oversized, or the
    /// canonical record exceeds 64 KiB.
    ...
    /// closed finding weights, reward sum, or PUCT score input disagrees.
    pub fn validate_for(
        &self,
        offer: &Proposal,
    ...
    pub(crate) fn score_for_policy(
        ...
        let reward = reward.min(u128::from(i64::MAX.unsigned_abs())) as i64;
        if reward != self.statistics.reward_sum_micros() {
            return Err(CampaignCodecError::InvalidValue {
                reason: "planner candidate guidance reward disagrees with finding evidence",
            });
        }
...
impl Canonical for PlannerCandidateGuidance {
    ...
    fn decode(decoder: &mut Decoder<'_>) -> Result<Self, CampaignCodecError> {
        ...
        Self::new(
            ...
            PuctEdgeStatistics::decode(decoder)?,
            u64::decode(decoder)?,
