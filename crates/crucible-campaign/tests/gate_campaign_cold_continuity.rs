...
const CAMPAIGN_NAME: &str = "campaign-cold-continuity";
const RESTORED_CAMPAIGN_NAME: &str = "campaign-cold-continuity-restored";
...
-> Result<(), Box<dyn Error>> {
    ...
    let source_blobs = Arc::new(DirectoryBlobBackend::new(
        "campaign-cold-continuity-source",
    ...
    let destination = CampaignRepository::new(
        Arc::new(DirectoryBlobBackend::new(
            "campaign-cold-continuity-destination",
...
#[test]
#[ignore = "spawned by the campaign-cold-continuity process fixture"]
...
fn publish_observed_branch(
    ...
    let opportunity = ChoiceOpportunity::new(
        ...
        "campaign-cold-continuity-branch",
        None,
    )?;
    ...
    let signature = FindingSignature::new(
        ...
        fingerprint,
        None,
        String::from("campaign.continuity"),
...
fn stable_projection(
    ...
    let principal = CampaignPrincipal::new("campaign-cold-continuity-gate")?;
...
fn exact_checkpoint_closure(
    ...
    let leaf = ContentEnvelope::new(
        "crucible.gate.campaign-cold-continuity.trace",
        1,
        BTreeSet::new(),
...
fn directory_repository(root: &Path) -> CampaignRepository {
    CampaignRepository::new(
        Arc::new(DirectoryBlobBackend::new(
            "campaign-cold-continuity-helper",
            root.join("objects"),
        )),
...
fn authenticate_checkpoint_closure(
    ...
    let blobs = DirectoryBlobBackend::new(
        "campaign-cold-continuity-checkpoint-reader",
        root.join("objects"),
    );
...
fn command_id(label: &str) -> CampaignCommandId {
    CampaignCommandId::from_hash(CampaignHash::derive(
        "gate.campaign-cold-continuity.command",
        label.as_bytes(),
    ))
}

fn hash(label: &str) -> CampaignHash {
    CampaignHash::derive("gate.campaign-cold-continuity", label.as_bytes())
}

fn hash_bytes(label: &str, bytes: &[u8]) -> CampaignHash {
    CampaignHash::derive(
        &format!("gate.campaign-cold-continuity.projection.{label}"),
        bytes,
    )
}
