...
##! after the complete fixed point evaluates.
{
  ...
  pamServiceContributionType = abilityTypes.record {
    fields = {
      unixAuth = abilityTypes.boolean;
      startSession = abilityTypes.boolean;
      setLoginUid = abilityTypes.boolean;
    };
  };
  wrapperContributionType = abilityTypes.record {
    fields = {
      source = abilityTypes.artifactPathReference;
      owner = abilityTypes.deferredResult abilityTypes.principalName;
      group = abilityTypes.deferredResult abilityTypes.groupName;
      mode = abilityTypes.fileMode;
      maximumSizeBytes = abilityTypes.integer {
        minimum = 1;
        maximum = abilityTypes.limits.maxSafeInteger;
      };
    };
  };
  kernelParameterType = abilityTypes.string {
    maxLength = 4096;
    syntax = null;
  };
  runtimeCheckType = abilityTypes.record {
    fields = {
      ...
      script = abilityTypes.string {
        maxLength = abilityTypes.limits.maxDocumentBytes;
        syntax = null;
      };
    };
  };
  ...
  filesystemTreeContributionType = abilityTypes.record {
    fields = {
      target = abilityTypes.relativePath;
      source = abilityTypes.artifactPathReference;
    };
  };
in {
  ...
  options.aos.contributions = {
    pamServices = mkOption {
      type = contributionMap pamServiceContributionType;
      default = {};
      contributable = true;
      description = "Package-owned PAM service policies consumed by the host authentication module.";
    };
    wrappers = mkOption {
      type = contributionMap wrapperContributionType;
      default = {};
      contributable = true;
      description = "Package-owned privileged executable declarations consumed by the wrapper materializer.";
    };
    kernelParameters = mkOption {
      type = contributionMap (abilityTypes.list {
        element = kernelParameterType;
        maxItems = 256;
        unique = true;
        canonicalOrder = true;
      });
    ...
    runtimeChecks = mkOption {
      type = contributionMap runtimeCheckGroupType;
      default = {};
      contributable = true;
      description = "Package-owned runtime qualification groups consumed by the system test plan.";
    };
    filesystemTrees = mkOption {
      type = abilityTypes.list {
        element = filesystemTreeContributionType;
        maxItems = 4096;
      };
      default = [];
      contributable = true;
      description = "Package-owned immutable directory trees exposed beneath /etc.";
    };
  };
