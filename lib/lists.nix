...
rec {
  ...
  zipLists = a: b: let
    len =
      if builtins.length a < builtins.length b
  ...
  ## Wrap a single value in a single-element list. Distinct from `toList`:
  ## `singleton` always creates a new list, regardless of whether the
  ## argument is already a list. Matches nixpkgs' `lib.singleton`.
  ## # Type
  ## `a -> [a]`
  singleton = x: [x];
}
