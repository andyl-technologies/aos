...
rec {
  ...
  mapAttrs' = f: attrs:
    builtins.listToAttrs (mapAttrsToList f attrs);
  ...
  mapAttrsRecursive = f: attrs:
    let
      go = path: set:
        builtins.mapAttrs (name: value:
          let newPath = path ++ [ name ];
          in if builtins.isAttrs value && !(value ? _type)
             then go newPath value
             else f newPath value
        ) set;
    in go [] attrs;

  # mapAttrsRecursiveCond :: (attrset -> bool) -> ([string] -> a -> b) -> attrset -> attrset
  # Like mapAttrsRecursive but only recurse into attrsets satisfying a predicate.
  mapAttrsRecursiveCond = cond: f: attrs:
    let
      go = path: set:
        builtins.mapAttrs (name: value:
          let newPath = path ++ [ name ];
          in if builtins.isAttrs value && cond value
             then go newPath value
             else f newPath value
        ) set;
    in go [] attrs;
  ...
  filterAttrsRecursive = pred: attrs:
    builtins.listToAttrs (
        ...
        mapAttrsToList (name: value:
          if !pred name value then []
            ...
            value =
              ...
              then filterAttrsRecursive pred value
              else value;
          }]
        ) attrs
      )
      ...
      uniqueNames = let
        go = acc: remaining:
          if remaining == [] then acc
            ...
            t = builtins.genList (i: builtins.elemAt remaining (i + 1))
                  (builtins.length remaining - 1);
          in if builtins.any (x: x == h) acc then go acc t
             else go (acc ++ [ h ]) t;
        in go [] allNames;
    in builtins.listToAttrs (builtins.map (name: {
      inherit name;
      value = builtins.foldl' (acc: attrs:
        if builtins.hasAttr name attrs
        then f attrs.${name} acc
        else acc
      ) init listOfAttrs;
    }) uniqueNames);
  ...
  foldlAttrs = f: init: attrs:
    builtins.foldl' (acc: name: f acc name attrs.${name})
      ...
      in {
        ...
        value =
          if lhsHas && rhsHas then
            if builtins.isAttrs lhsVal && builtins.isAttrs rhsVal
               && !(lhsVal ? outPath) && !(rhsVal ? outPath)
            then recursiveUpdate lhsVal rhsVal
            else rhsVal
          else if rhsHas then rhsVal
          else lhsVal;
      }
    ) allNames);
          ...
          allNames = let
            ...
            dedup = acc: remaining:
              if remaining == [] then acc
                ...
                t = builtins.genList (i: builtins.elemAt remaining (i + 1))
                      (builtins.length remaining - 1);
              in if builtins.any (x: x == h) acc then dedup acc t
                 else dedup (acc ++ [ h ]) t;
          in dedup [] combined;
        in builtins.listToAttrs (builtins.map (name:
          let
          ...
          in {
            ...
            value =
              if lHas && rHas then
                if builtins.isAttrs lVal && builtins.isAttrs rVal && !pred newPath lVal rVal
                then go newPath lVal rVal
                else rVal
              else if rHas then rVal
              else lVal;
          }
        ) allNames);
    in go [] lhs rhs;
  ...
  collect = pred: attrs:
    if pred attrs then [ attrs ]
      ...
      builtins.concatLists (
        builtins.map (name: collect pred attrs.${name})
  ...
  collectLeaves = attrs:
    if !builtins.isAttrs attrs then [ attrs ]
    else builtins.concatLists (
      builtins.map (name: collectLeaves attrs.${name})
        (builtins.attrNames attrs)
    );
  ...
  zipAttrs = listOfAttrs:
    ...
    in builtins.listToAttrs (builtins.map (name: {
      ...
      value = builtins.concatLists (
        builtins.map (attrs:
          if builtins.hasAttr name attrs then [ attrs.${name} ] else []
        ) listOfAttrs
      );
    }) uniqueNames);
  ...
  zipAttrsWith = f: listOfAttrs:
    builtins.mapAttrs f (zipAttrs listOfAttrs);
    ...
    let
      go = remaining: current:
        if remaining == [] then current
          ...
          t = builtins.genList (i: builtins.elemAt remaining (i + 1))
                (builtins.length remaining - 1);
        in if builtins.isAttrs current && builtins.hasAttr h current
           then go t current.${h}
           else default;
    in go path attrs;
  ...
  attrsToList = attrs:
    builtins.map (name: { inherit name; value = attrs.${name}; })
      (builtins.attrNames attrs);
