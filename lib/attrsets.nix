...
rec {
  ...
  mapAttrsRecursiveCond = cond: f: attrs: let
    go = path: set:
      builtins.mapAttrs (
        name: value: let
          newPath = path ++ [name];
  ...
  foldAttrs = f: init: listOfAttrs: let
      ...
      builtins.map (
        ...
        in {
          ...
          value =
            if lhsHas && rhsHas
            then
              if builtins.isAttrs lhsVal && builtins.isAttrs rhsVal && !(lhsVal ? outPath) && !(rhsVal ? outPath)
  ...
  collect = pred: attrs:
    ...
    then builtins.concatLists (builtins.map (name: collect pred attrs.${name}) (builtins.attrNames attrs))
    else [];
  ...
  collectLeaves = attrs:
    ...
    then [attrs]
    else builtins.concatLists (builtins.map (name: collectLeaves attrs.${name}) (builtins.attrNames attrs));
  ...
  ## # Type
  ## `[string] -> a -> attrset -> a`
  attrByPath = path: default: attrs: let
    go = remaining: current:
      ...
      then current
        ...
        t = builtins.genList (i: builtins.elemAt remaining (i + 1)) (builtins.length remaining - 1);
      in
        if builtins.isAttrs current && builtins.hasAttr h current
        then go t current.${h}
        else default;
  in
    go path attrs;
