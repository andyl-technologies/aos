...
{ trivial, lists, attrsets, strings, types }:
...
let
  ...
  mkOption = { type ? types.anything
    ...
    {
      _type = "option";
      inherit type default description example readOnly apply visible;
    };
  ...
  resolveIfs = value:
    if isMkIf value then
      if value._condition then resolveIfs value._value
      else {}
    else if builtins.isAttrs value then
      let
        names = builtins.attrNames value;
        resolved = builtins.listToAttrs (builtins.concatLists (builtins.map (name:
          let v = value.${name};
          in if isMkIf v then
               if v._condition then [{ inherit name; value = resolveIfs v._value; }]
               else []
             else [{ inherit name; value = resolveIfs v; }]
        ) names));
      in resolved
    else value;
    ...
    if builtins.isAttrs lhs && builtins.isAttrs rhs then
      ...
      in builtins.listToAttrs (builtins.map (name: {
        ...
        value =
          ...
          in if lHas && rHas then deepMerge lhs.${name} rhs.${name}
             else if rHas then rhs.${name}
             else lhs.${name};
      }) allNames)
    else rhs;
  ...
  evalModule = { config, pkgs, lib, extraArgs }: mod:
    let
      # Determine the file path for error messages
      file =
        if builtins.isPath mod then builtins.toString mod
        else if builtins.isString mod then mod
        else if builtins.isAttrs mod && mod ? _file then mod._file
      ...
      args = { inherit config pkgs lib; } // extraArgs;
      evaluated =
        if builtins.isFunction loaded then
          let
            ...
            callArgs =
              if fArgs == {} then args  # No formals, pass everything (variadic)
  ...
  evalModules = { modules
    ...
    let
      ...
      result = let
        ...
        collectModules = mods:
          builtins.concatLists (builtins.map (mod:
            let
              evaled = evalModule {
                config = finalConfig;
                pkgs = pkgs;
        ...
        optionMap = builtins.foldl' (acc: decl:
        ...
        mergedOptions = builtins.listToAttrs (builtins.map (key:
          let
            decl = optionMap.${key};
        ...
        finalConfig = builtins.foldl' (acc: key:
          let entry = mergedOptions.${key};
          in deepMerge acc (setPath entry.path entry.finalValue)
